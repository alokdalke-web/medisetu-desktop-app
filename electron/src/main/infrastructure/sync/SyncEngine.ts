...
import { ClinicSymptomSyncService } from './services/ClinicSymptomSyncService.js';
import { PatientSyncService } from './services/PatientSyncService.js';
import { AppointmentSyncService } from './services/AppointmentSyncService.js';
...
export class SyncEngine {
  ...
  public async triggerSync(): Promise<void> {
    // We removed the isHost() check here because ALL computers on the network
    // need to cache Master Data (Medicines, Doctors, Templates, etc.) into their own local DB.
    // Unlike pushing P2P events, metadata fetching isn't strictly reserved for the elected Host.
