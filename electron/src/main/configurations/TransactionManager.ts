...
export class TransactionManager {
  ...
  public static async run<T>(
    ...
    try {
      // Trigger sync after successful transaction to update UI immediately
      const { PushSyncEngine } = require('../sync/SyncEngine');
      if (PushSyncEngine.getInstance) {
        PushSyncEngine.getInstance().triggerImmediateSync();
      }
    } catch (err) {
      console.error('[TransactionManager] Failed to trigger immediate sync:', err);
    }
