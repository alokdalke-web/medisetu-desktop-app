...
export function runMigrations(db: Database.Database) {
  ...
  const migrations = [
    ...
    },
    {
      version: 20,
      up: `
        ALTER TABLE event_log ADD COLUMN p2p_sync TEXT DEFAULT 'pending';
      `
    },
    {
      version: 21,
      up: `
        ALTER TABLE event_log ADD COLUMN next_retry_at DATETIME;

        CREATE INDEX IF NOT EXISTS idx_event_log_lamport
          ON event_log(lamport_clock);

        CREATE INDEX IF NOT EXISTS idx_event_log_sync_status
          ON event_log(synced_to_cloud, retry_count, status);

        CREATE INDEX IF NOT EXISTS idx_event_log_node_id
