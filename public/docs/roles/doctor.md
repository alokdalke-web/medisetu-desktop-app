...
![Doctor consultation workspace](/docs/images/roles/role-doctor.svg)


Your Infinity MediSetu workspace is built around clinical workflow — managing appointments, conducting consultations, writing prescriptions, ordering lab tests, reviewing results, and controlling your availability.

> Tip: **At a glance** — your day flows as: **see your schedule** → **start a consultation** (vitals, diagnosis, prescription, lab orders) → **review results** → **set follow-ups**. The sections below follow that order.
...
- **Queue Summary** — Checked-in patients physically waiting (live "Waiting Now")
- **Recent Patients** — Quick access to your latest consultations
...
Whether payment/revenue figures are shown in the app is controlled by the clinic admin via **Profile → Security & Access → Payment History Visibility**. When it is turned off, revenue amounts are hidden and users see a notice instead of payment data.
...
| **Profile → Services & Price** | Your consultation types, duration, and fees |
| **Profile → Doctor Availability** | Working schedule — start/end times, slot duration, gap between appointments, max patients/day, unavailable days |
...
> Note: Account security (password, active sessions), clinic settings, referrals, and notification preferences are managed by your clinic admin — they are not part of the doctor menu.
