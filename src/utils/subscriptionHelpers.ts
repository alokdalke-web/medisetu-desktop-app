...
export const generateInvoiceHTML = (data: InvoiceData): string => {
  ...
  const discountAmount = Number.isFinite(Number(data.discount))
    ? Math.max(0, Number(data.discount))
    : 0;
  const hasDiscount = discountAmount > 0;
  const discountedGrandTotal = Math.max(0, grandTotal - discountAmount);
      ...
      <table>
        ...
        <tbody>
          ...
          <tr>
            ...
            <td
              style="line-height: 20px; text-transform: uppercase; letter-spacing: 1px; white-space: nowrap;"
            >
              <p style="margin: 0;">+ ${safeFormatMoney(cgst, data.currency)}</p>
              <p style="margin: 0;">+ ${safeFormatMoney(sgst, data.currency)}</p>
            </td>
          </tr>
          ${hasDiscount ? `
          <tr>
            ...
            <td style="text-align:right; white-space:nowrap;">
              ${safeFormatMoney(subtotal+cgst+sgst, data.currency)}
            </td>
          </tr>
          <tr>
            <td colspan="5" style="text-align:right; font-size:12px;">
            ...
            <td style="text-align:right; white-space:nowrap;">
              - ${safeFormatMoney(discountAmount, data.currency)}
            </td>
          </tr>
          ` : ""}
          <tr class="grand-total">
            <td colspan="5" style="text-align:right; font-weight:bold; text-transform: uppercase; letter-spacing: 1px;">Grand Total</td>
            <td style="font-weight:bold; text-transform: uppercase; letter-spacing: 1px;">${safeFormatMoney(discountedGrandTotal, data.currency)}</td>
          </tr>
          <tr>
            <td colspan="6" style="border-top: none;">
              <p style="font-size: 10px; margin-bottom: 5px; text-transform: uppercase; letter-spacing: 1px;">Total in Words:</p>
              <p style="font-size: 12px; font-weight: bold; text-transform: uppercase; letter-spacing: 1px;">${amountInWords(discountedGrandTotal)}</p>
            </td>
          </tr>
        </tbody>
      </table>
