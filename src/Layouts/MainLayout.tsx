...
const SidebarSkeleton: React.FC<{ collapsed?: boolean }> = ({
  ...
  return (
    <aside className="hidden xl:block xl:row-span-2 xl:col-start-1 border-r border-line bg-surface">
...
const MainLayout: React.FC = () => {
  ...
  const approvalRequestSent =
    (clinicProfile as OnboardingProfile)?.approvalRequestSent ??
    (user as OnboardingProfile)?.approvalRequestSent ??
    false;
  ...
  const shouldEnforceOnboarding = isAdmin || (isDoctor && !isPendingApproval);
  ...
  // ✅ Doctor Pending Guard: Keep pending doctors inside setup-safe paths
  if (isDoctor && isPendingApproval) {
    const allowedPaths = [
      "/dashboard",
      "/profile/security",
      "/profile/services",
    ...
    const isPathAllowed = allowedPaths.some(p => location.pathname === p || location.pathname.startsWith(p));
    if (!isPathAllowed) {
      return <Navigate to="/dashboard" replace />;
    }
  }

  // ✅ Pharmacist Pending Guard: Lock navigation for pending pharmacists to setup paths
  const isPharmacist = user?.userType === "Pharmacist";
  if (isPharmacist && isPendingApproval) {
    const allowedPaths = [
      "/pharmacy/dashboard",
      "/pharmacy/medicines",
      "/pharmacy/suppliers",
  ...
  // ✅ Receptionist Pending Guard: Lock navigation for pending receptionists to setup paths
  const isReceptionist = user?.userType === "Receptionist";
  if (isReceptionist && isPendingApproval) {
    const allowedPaths = [
      "/dashboard",
      "/patient/new",
      "/patients",
    ...
    const isPatientEditPath = /^\/patient\/[^/]+\/edit$/.test(location.pathname);
    const isPathAllowed = isPatientEditPath || allowedPaths.some(p => location.pathname === p || location.pathname.startsWith(p));
    if (!isPathAllowed) {
      return <Navigate to="/dashboard" replace />;
    }
  }

  // ✅ Lab Assistant Pending Guard: Keep pending lab assistants inside setup-safe paths
  const isLabAssistant = user?.userType === "Lab_Assistant";
  if (isLabAssistant && isPendingApproval) {
    const allowedPaths = [
      "/lab/dashboard",
      "/lab/queue",
      "/profile/security",
