...
interface UseAppointmentRealtimeOptions {
  ...
  doctorId?: string;
  /**
   * Called (debounced, same as the "Appointment" tag invalidation) whenever a
   * relevant appointment event arrives. Lets callers invalidate cache tags on
   * OTHER API slices (e.g. dashboardApi's "DoctorDashboard") that don't share
   * appointmentApi's "Appointment" tag but still need to refresh in response
   * to the same events.
   */
  onAppointmentEvent?: () => void;
}
...
export const useAppointmentRealtime = (
  ...
  const invalidateAppointments = useCallback(() => {
    ...
    debounceRef.current = setTimeout(() => {
      ...
      optionsRef.current.onAppointmentEvent?.();
