...
export interface GetPatientsArgs {
  ...
  startDate?: string;
  endDate?: string;
  sortBy?: "createdAt" | "name" | "visitCount";
  sortOrder?: "asc" | "desc";
}
...
export const patientApi = createApi({
  ...
  endpoints: (builder) => ({
      ...
      queryFn: async (args) => {
        try {
          ...
          const queryParams = {
            ...
            userStatus: status,
            patientId,
            // additional filter params sent only if defined
            ...(gender && { gender }),
            ...(city && { city }),
    ...
    getPatientById: builder.query<CreatePetientDto, string>({
      queryFn: async (patientId, api) => {
        try {
          // 1. Resolve cloud mapping from Local DB if inside Electron
          let targetId = patientId;
          const isElectron = typeof window !== 'undefined' && (window as any).ipcAPI;
          if (isElectron) {
            try {
              const localRes = await (window as any).ipcAPI.patient.getProfile(patientId);
              if (localRes && localRes.cloud_id) {
                targetId = localRes.cloud_id;
              }
            } catch (err) {
              // Ignore local error, proceed with original ID
            }
          }

          // 2. Fetch directly from Cloud using the resolved ID
          ...
          const resultData = {
            ...response.data,
            _source: response.meta.source
          };
          return { data: resultData as CreatePetientDto, meta: { source: response.meta.source } };
        } catch (error: any) {
          return { error: { status: 'CUSTOM_ERROR', error: error.message } };
        }
      },
    ...
    >({
      queryFn: async ({
        ...
        try {
          ...
          const response = await TransportLayer.execute<any>({
            ipcMethod: 'patient.getReportCards',
            ipcPayload: { patientId, pageSize, pageNumber, searchBy, typeOfPaginations },
            preferCloud: true,
            restConfig: {
              url: `/reports/card/all/${cloudId || patientId}`,
              method: 'GET',
              params
            }
          });
          ...
          let data: ReportCardsByPatientIdResult;
          switch (typeOfPaginations) {
            case "Appointments":
              data = {
                ...base,
          ...
          return { data, meta: { source: response.meta.source } };
        } catch (error: any) {
          return { error: { status: "CUSTOM_ERROR", error: error.message } };
        }
      },
