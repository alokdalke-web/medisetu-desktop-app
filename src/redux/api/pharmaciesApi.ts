...
export type GetAdminMedicinesArgs = GetAllMedicinesArgs & {
  pharmacyId: string;
};

export type AddAdminMedicineArgs = {
  pharmacyId: string;
  body: AddMedicineRequest;
};

export type UpdateAdminMedicineArgs = {
  pharmacyId: string;
  medicineId: string;
  body: UpdateMedicineRequest;
};
...
export type GetSalesReturnsArgs = {
  pageNumber?: number;
  pageSize?: number;
  saleId?: string;
  startDate?: string;
...
export type GetSalesReturnsResponse = {
  success: boolean;
  data: SalesReturn[];
  pagination: {
    totalRecords: number;
...
export const pharmaciesApi = createApi({
  ...
  tagTypes: ["Supplier", "Medicine", "HSN", "Category", "Brand", "Prescription", "Stock", "Sale", "Setting", "Subscription", "Dashboard", "SalesOverview", "CategoryRevenue", "TopPerformers", "AiPrediction", "Tag", "SalesReturn"],
  endpoints: (builder) => ({
    /* ----------------------------- Suppliers ----------------------------- */
    ...
    // Admin variants: view/manage any pharmacy's inventory by pharmacyId
    // (self-service endpoints above resolve pharmacyId from the logged-in
    // pharmacist instead). Tag id is namespaced per pharmacy so it doesn't
    // cross-invalidate the self-service "LIST" tag.
    getAdminMedicinesByPharmacyId: builder.query<
      MedicineListResponse,
      GetAdminMedicinesArgs
    >({
      query: ({ pharmacyId, pageNumber = 1, pageSize = 10, search = "", category, form, status, hsnId, stockStatus, tag }) => ({
        url: `/pharmacies/admin-medicine/${pharmacyId}/get-all-medicines`,
        method: "GET",
        params: {
          pageNumber,
          pageSize,
      ...
      providesTags: (_res, _err, arg) => [
        { type: "Medicine", id: `PHARMACY-${arg.pharmacyId}` },
      ],
    }),

    addAdminMedicine: builder.mutation<
      PharmaciesApiResponse<any>,
      AddAdminMedicineArgs
    >({
      query: ({ pharmacyId, body }) => ({
        url: `/pharmacies/admin-medicine/${pharmacyId}/add-medicine`,
        method: "POST",
        body,
      }),
      invalidatesTags: (_res, _err, arg) => [
    ...
    updateAdminMedicine: builder.mutation<
      PharmaciesApiResponse<any>,
      UpdateAdminMedicineArgs
    >({
      query: ({ pharmacyId, medicineId, body }) => ({
        url: `/pharmacies/admin-medicine/${pharmacyId}/update-medicine/${medicineId}`,
        method: "PUT",
        body,
      }),
      invalidatesTags: (_res, _err, arg) => [
    ...
    >({
      query: (body) => ({
        url: "/pharmacies/sales/create-sales-return",
        method: "POST",
        body,
      }),
      invalidatesTags: [
    ...
    getSalesReturns: builder.query<GetSalesReturnsResponse, GetSalesReturnsArgs>({
      query: ({
        pageNumber = 1,
        pageSize = 10,
        saleId,
        ...
        url: "/pharmacies/sales/get-sales-returns",
        method: "GET",
        params: {
          pageNumber,
          pageSize,
