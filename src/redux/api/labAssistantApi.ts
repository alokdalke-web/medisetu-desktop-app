...
function normalizeApiEndpointUrl(value: unknown) {
  ...
  try {
    const basePath = new URL((typeof window !== 'undefined' && window.localStorage?.getItem('API_BASE_URL') || import.meta.env.VITE_API_BASE_URL)).pathname.replace(
      /\/$/,
      "",
    );
