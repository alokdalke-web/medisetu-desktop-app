...
export type PaymentTransaction = {
  ...
  // Per-row payment status: "Paid" | "Pending" (Pay Later) | "Already Paid" | "Refunded"
  paymentStatus?: string | null;
  paymentMode?: string | null;
...
export type PaymentStatus = "Paid" | "Pending" | "Already Paid" | "Refunded";
      ...
      queryFn: async (args) => {
        try {
          ...
          const response = await TransportLayer.execute<any>({
            ipcMethod: "dashboard.getPaymentTransactions",
            ipcPayload: params,
            restConfig: {
          ...
          return { data: { data, metadata, summary }, meta: { source: response.meta?.source } };
        } catch (error: any) {
    ...
    getAllPlans: b.query<PlansResponse, void>({
      ...
      transformResponse: (res: any): PlansResponse => {
        ...
        const plans: Plan[] = (Array.isArray(rawPlans) ? rawPlans : []).map((p: any) => {
          ...
          // Show all enabled features, with marketing features first.
