...
import type {
  SetDoctorOnlineBookingPayload,
  GetDoctorOnlineBookingResponse,
  SetDoctorOnlineBookingResponse,
...
export const doctorApi = createApi({
  ...
  endpoints: (builder) => ({
    ...
    updateService: builder.mutation<
      ...
      { serviceId: string; body: { serviceName?: string; price?: number; currency?: string; durationDays?: number; additionalServices?: string; canBeBookedByPatient?: boolean } }
    >({
    ...
    getDoctorOnlineBooking: builder.query<
      GetDoctorOnlineBookingResponse,
      string
    >({
      query: (doctorId) => `/clinic/doctors/${doctorId}/online-booking`,
      providesTags: ["Doctor"],
    }),

    setDoctorOnlineBooking: builder.mutation<
      SetDoctorOnlineBookingResponse,
      SetDoctorOnlineBookingPayload
    >({
      query: ({ doctorId, enabled, maxAdvanceBookingDays }) => ({
        url: `/clinic/doctors/${doctorId}/online-booking`,
        method: "PATCH",
        body: { enabled, maxAdvanceBookingDays },
      }),
      invalidatesTags: ["Doctor"],
    }),
    ...
    /* ✅ GET /clinic/clinic-symptom?search=Fever */
    ...
    // GET /doctor/single/:doctorId/services — services offered by a doctor
    getDoctorServices: builder.query<
      { success: boolean; message: string; data: DoctorServiceItem[] },
      string
    >({
      query: (doctorId) => `/doctor/single/${doctorId}/services`,
      providesTags: ["Doctor"],
    }),

    // GET /doctor/single/:doctorId/activity — a doctor's recent appointment activity
    getDoctorActivity: builder.query<
      { success: boolean; message: string; data: DoctorActivityItem[] },
      string
    >({
      query: (doctorId) => `/doctor/single/${doctorId}/activity`,
      providesTags: ["Doctor"],
    }),

    // GET /doctor/single/:doctorId/availability — a doctor's weekly availability
    getDoctorAvailabilityById: builder.query<
      { success: boolean; message: string; data: DoctorAvailabilityItem[] },
      string
    >({
      query: (doctorId) => `/doctor/single/${doctorId}/availability`,
      providesTags: ["Doctor"],
    }),
  }),
});
...
export const {
  ...
  useGetDoctorActivityQuery,
  useGetDoctorAvailabilityByIdQuery,
