...
const GoogleAuthButton: React.FC<GoogleAuthButtonProps> = ({
  ...
  const handleElectronLogin = async () => {
    ...
    try {
      ...
      const response = await (window as any).ipcAPI.auth.googleLogin(clientId);
      if (response.success && response.idToken) {
        onSuccess({ credential: response.idToken } as CredentialResponse);
      } else {
    ...
    <div ref={buttonRef} className={`relative mx-auto h-[40px] w-full overflow-hidden rounded-xl sm:h-[42px] lg:h-[34px] lg:rounded-lg ${className}`}>
      {isElectron ? (
        <button
          ...
          disabled={isLoading}
          className="absolute inset-0 z-10 h-full w-full w-full cursor-pointer opacity-0"
