...
const RoleSwitcherDropdown: React.FC = () => {
  ...
  const isPendingOnboardingUser =
    (actualUserType === "Admin" || actualUserType === "Doctor") &&
    normalizeStatus((user as any)?.userStatus) === "pending";
  const shouldSkipRoleLookups =
    location.pathname === "/clinic-setup" || isPendingOnboardingUser;
  const { data: clinics } = useGetAllClinicsQuery(undefined, {
    skip: actualUserType !== "Admin" || shouldSkipRoleLookups,
  });
  ...
  return (
    <div className="relative" ref={dropdownRef}>
      <button
        ...
        className="cursor-pointer inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg border border-gray-200 bg-gray-50 hover:bg-gray-100 transition-colors text-sm font-medium text-gray-700"
