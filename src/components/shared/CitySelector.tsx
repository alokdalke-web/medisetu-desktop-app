...
interface CitySelectorProps {
  ...
  label?: string;
  /**
   * State that belongs to an externally-set city (the map filling the address).
   * Only used for display — a city filled from the map has no dropdown
   * selection behind it, so without this the field reads "Indore" where a
   * hand-picked city reads "Indore, Madhya Pradesh".
   */
  stateValue?: string;
  size?: "sm" | "md" | "lg";
  isRequired?: boolean;
...
export default function CitySelector({
  ...
  /**
   * What the closed field reads: "City, State" when a state is known — from the
   * dropdown selection, or from `stateValue` when the caller filled the city
   * (the map) — and the bare city otherwise.
   */
  const displayValue = [
    (field.value as string) || "",
    ((field.value && (lastSelectedStateRef.current || stateValue)) || "").trim(),
  ]
    .filter(Boolean)
    .join(", ");
  ...
  return (
    ...
    >
      <Autocomplete
        ...
        isClearable
        /* The value can come from outside the list (the map auto-fills it), so
           HeroUI must not discard input text it can't match to an item. */
        allowsCustomValue
        ...
        inputProps={{
          ...
          classNames: {
            input:
              "!text-text placeholder:text-text-muted truncate",
            inputWrapper:
        ...
        inputValue={search || displayValue}
        selectedKey={allItems.find((c) => c.city === field.value)?.id}
        menuTrigger="focus"
        allowsEmptyCollection
        onClear={handleClear}
        classNames={{
          /* HeroUI's Autocomplete root is `inline-flex`, so it sits on the text
             baseline of its cell and renders a few pixels lower than a plain
             `Input` beside it in the same grid row — the City & State field was
             always dropping below Address/Pincode. `flex` makes it block-level
        ...
        onSelectionChange={(key) => {
          if (!key) {
            ...
            // A value set from outside (the map auto-filling the address) never
            // matches a key in `allItems` — the list is empty until someone
            // searches — so HeroUI reports "no selection" and this used to wipe
            // it straight back out, leaving the field blank after every pin
            // drop. Clearing belongs to the × button (`onClear`) and to
            // emptying the input (`onInputChange`), both handled separately.
        ...
        onInputChange={(value) => {
          ...
          // HeroUI also echoes the current text back on close/blur. That is not
          // the user typing, and treating it as a search would strip the comma
          // out of "Indore, Madhya Pradesh" and then clear the city for not
          // matching itself.
          if (raw.trim() === displayValue.trim()) {
            return;
          }
