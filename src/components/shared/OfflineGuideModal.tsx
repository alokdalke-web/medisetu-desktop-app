...
export const OfflineGuideModal: React.FC = () => {
  ...
  useEffect(() => {
    if (isOffline) {
      // Show the icon after 5.5 seconds to let the offline banner toast finish
      const timer = setTimeout(() => setShowIcon(true), 5500);
  ...
  return (
    <>
      <motion.button
        ...
        className="relative inline-flex items-center justify-center gap-2 px-3 sm:px-4 h-8 sm:h-10 rounded-full bg-red-50 dark:bg-red-900/30 border border-red-200 dark:border-red-800 transition-colors cursor-pointer"
