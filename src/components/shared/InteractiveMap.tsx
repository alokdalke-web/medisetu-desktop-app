...
import {
  getAddressDetailsFromGooglePlace,
...
const InteractiveMap: React.FC<InteractiveMapProps> = ({
  ...
  const [markerPosition, setMarkerPosition] = useState<InteractiveMapLocation>(
    initialLocation || defaultCenter
  );
  const [mapCenter, setMapCenter] = useState<InteractiveMapLocation>(
    initialLocation || defaultCenter
  );
  ...
  const handlePlacesChanged = useCallback(() => {
    ...
    const addr = getAddressDetailsFromGooglePlace(place);
    setMarkerPosition({ lat, lng });
  ...
  /**
   * `GoogleMap` is rendered with controlled `center`/`zoom`, so whatever those
   * state values hold is re-applied on every render. Panning and pinch-zooming
   * happen inside the Google widget and never touched them, so the next render
   * — one `setValue` in the parent form is enough — snapped the map back to the
   * last programmatic view: the pin stayed put while the viewport jumped to a
   * stale centre at a stale zoom. Syncing on idle keeps the two in step.
   */
  ...
  return (
    <div
      className="clinic-location-map flex w-full flex-col bg-white font-outfit dark:bg-slate-800"
      style={{ height }}
    >
      ...
      {!readOnly && (
        <div className="flex gap-2 bg-white px-3 py-2 dark:bg-slate-800">
          ...
          <button
            ...
            className={[
              "flex h-9 items-center gap-1.5 rounded-lg px-2.5 text-[12px] font-bold transition-all whitespace-nowrap select-none sm:px-3 cursor-pointer",
              isGettingLocation
      ...
      <div className="relative flex-1 min-h-0">
        <GoogleMap
          ...
          center={mapCenter}
          zoom={zoom}
          options={{ ...mapOptions, draggable: !readOnly, disableDefaultUI: true }}
      ...
      <style>{`
        ...
        .clinic-location-map .gm-err-container,
        .clinic-location-map .gm-err-content,
        .clinic-location-map .gm-style .gm-err-autocomplete,
        .clinic-location-map .gm-style .gm-err-title,
        .clinic-location-map .gm-style .gm-err-message {
