...
const baseTriggerClass =
  "flex h-8 w-full items-center justify-between gap-2 rounded-lg border border-line bg-surface px-3 text-left text-xs font-semibold text-text outline-none transition hover:border-primary/40 focus:border-primary/45 focus:ring-2 focus:ring-primary/10 disabled:cursor-not-allowed disabled:opacity-60";

const baseMenuClass =
  "absolute left-0 top-[calc(100%+6px)] z-[70] w-full overflow-hidden rounded-xl border border-line bg-surface p-1 shadow-xl shadow-black/10";
...
export default function CompactSelectDropdown<T extends string | number>({
  ...
  menuClassName = "",
  optionClassName = "",
  selectedClassName = "bg-primary/10 text-primary",
  unselectedClassName = "text-text hover:bg-surface-muted hover:text-primary",
