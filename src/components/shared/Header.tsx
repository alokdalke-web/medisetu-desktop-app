...
const Header: React.FC<HeaderProps> = ({ onOpenLeft, onOpenRight }) => {
  ...
  const { data: clinics } = useGetAllClinicsQuery(undefined, {
    skip: isLabAssistant || isSuperAdmin,
  });
  ...
  const isOffline = connectivityState === 'island' || connectivityState === 'lan_sync';
  ...
  useEffect(() => {
    if (!(window as any).ipcAPI?.appMenu) return;
    
    const unsubscribe = (window as any).ipcAPI.appMenu.onAction((action: string) => {
      switch (action) {
        case 'file:new-patient':
          navigate('/patient/new');
          break;
        case 'file:new-appointment':
          // Ideally open appointment modal or navigate to /appointment
          navigate('/appointment');
          break;
        case 'file:settings':
          navigate('/profile/clinic-settings');
          break;
        case 'view:toggle-sidebar':
          if (onOpenLeft) onOpenLeft();
          break;
        case 'view:toggle-theme':
          toggleTheme();
          break;
        case 'view:docs':
          window.open(`/app/guidelines?from=${location.pathname}`, '_blank');
          break;
        case 'clinic:sync-now':
          if ((window as any).ipcAPI?.pushSync) {
            (window as any).ipcAPI.pushSync.trigger().catch(console.error);
          }
          break;
        case 'clinic:sync-details':
          document.getElementById('sync-status-hidden-trigger')?.click();
          break;
        case 'clinic:pair':
          // Assuming pairing opens a modal or navigates
          // if there is a route for this, add here. 
          break;
        default:
          console.log('Unhandled app menu action:', action);
      }
    });
    return unsubscribe;
  }, [navigate, onOpenLeft, toggleTheme, location.pathname]);

  return (
    <>
      <header
        style={isElectron ? { WebkitAppRegion: 'drag' } as any : undefined}
        className={`flex items-center justify-between px-2 sm:px-6 h-12 sm:h-[60px] bg-surface border-b border-gray-200 relative ${isElectron ? "pr-[140px] sm:pr-[150px]" : ""}`}
      >
        ...
        {/* Center Section: Centered flex wrapper for Offline Warning to prevent overlap */}
        <div className="flex shrink-0 justify-center px-2">
          <OfflineGuideModal />
        </div>

        {/* Right Section */}
        <div className="flex flex-1 justify-end shrink-0 items-center gap-1.5 sm:gap-3">
          {onOpenRight && (
            <button
              type="button"
              onClick={onOpenRight}
          ...
          <div className="hidden sm:block" style={isElectron ? { WebkitAppRegion: 'no-drag' } as any : undefined}>
            <FullscreenToggle />
          </div>
          {/* Dark mode toggle */}
          <div className="relative group inline-block" style={isElectron ? { WebkitAppRegion: 'no-drag' } as any : undefined}>
            <button
              type="button"
              onClick={toggleTheme}
              className="cursor-pointer inline-flex items-center justify-center h-8 w-8 sm:h-10 sm:w-10 rounded-full bg-gray-100 hover:bg-gray-200 transition-colors"
              aria-label={
    ...
    whitespace-nowrap bg-surface text-text text-xs shadow-md border border-line
    ...
    transition duration-200 pointer-events-none"
          ...
          <div className="relative group inline-block" style={{ WebkitAppRegion: 'no-drag' } as any}>
            <button
              onClick={() =>
                window.open(
    ...
    transition duration-200 z-50"
          ...
          {/* Notifications */}
          <div className="relative" ref={notifRef} style={isElectron ? { WebkitAppRegion: 'no-drag' } as any : undefined}>
            <button
              type="button"
              className="cursor-pointer relative inline-flex items-center justify-center h-8 w-8 sm:h-10 sm:w-10 rounded-full bg-gray-100 hover:bg-gray-200 transition-colors"
            ...
            {notifOpen && (
              <div className={notifPanelClass}>
                <div className="w-full rounded-2xl bg-surface shadow-xl border border-gray-100 overflow-hidden">
