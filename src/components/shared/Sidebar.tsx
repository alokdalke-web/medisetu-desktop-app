  ...
  return (
    <img
      ...
      className={`h-4 w-4 sm:h-[18px] sm:w-[18px] object-contain shrink-0 block transition-all duration-200 ${
        active
          ? "brightness-0 invert opacity-100" // active => white icon
          : "brightness-0 opacity-70 dark:brightness-0 dark:invert dark:opacity-100" // inactive => dark gray in light, white in dark
      }`}
      onError={(e) => {
...
const Sidebar: React.FC<SidebarProps> = ({
  ...
  const isSuperAdmin = actualUserType === "Super_Admin";

  const { data: clinics } = useGetAllClinicsQuery(undefined, {
    skip: !isAdmin,
  });

  // Active banner count for the "Broadcast Hub" nav badge — only fetched for Super Admins.
  const { data: bannerCountData } = useGetBannersQuery(
    { pageNumber: 1, pageSize: 1, isActive: true },
    { skip: userType !== "Super_Admin" },
  );
  const activeBannerCount = bannerCountData?.pagination.total ?? 0;

  const [showCallOptions, setShowCallOptions] = useState(false);
  const { data: userClinics } = useGetAllClinicsQuery(undefined, {
    skip: isSuperAdmin,
  });
  ...
  const isApprovalLocked =
    ...
    (isSetupAdmin ||
      isSetupDoctor ||
      setupRole === "Pharmacist" ||
      setupRole === "Receptionist" ||
      setupRole === "Lab_Assistant");
  ...
  const isRouteActive = (itemKey: string, to: string, end?: boolean) => {
    ...
    if (itemKey === "labAssigned") {
      return (
        path === "/lab/assigned" ||
        path.startsWith("/lab/assigned/") ||
    ...
    return (
      <aside
        className={`
          ...
          bg-surface xl:border border-border-color
  ...
  // const isLabAssistantRole = role === "Lab_Assistant";git 
  const isPendingLabAssistant =
    isPendingApproval && setupRole === "Lab_Assistant";
  const homeTo = isPendingLabAssistant
    ? "/lab/dashboard"
    : isAdminSetupLocked
      ? "/dashboard"
      : role === "Receptionist"
        ...
        : role === "Pharmacist"
          ...
          : role === "Lab_Assistant"
            ? "/lab/dashboard"
            : "/dashboard";
  const approvalEnabledItemKeys = new Set([
    "dashboard",
    "pharmacyDashboard",
    "labDashboard",
    ...(isPendingLabAssistant ? ["labQueue"] : []),
  ]);
  ...
  return (
    ...
    >
      {/* ✅ Header */}
      <header className={`shrink-0 ${isElectron ? 'pt-3' : 'pt-5 sm:pt-8'}`} style={isElectron ? { WebkitAppRegion: 'drag' } as any : undefined}>
        <div
          className={`
        ...
        >
          ...
          {isElectron && (
            <button
              type="button"
              className="inline-flex h-8 w-8 shrink-0 items-center justify-center text-gray-700 p-0 self-start"
              aria-label="App Menu"
              style={isElectron ? { WebkitAppRegion: 'no-drag' } as any : undefined}
              onClick={(e) => {
                if ((window as any).ipcAPI?.appMenu?.showMenu) {
                  const rect = e.currentTarget.getBoundingClientRect();
                  (window as any).ipcAPI.appMenu.showMenu(rect.left, rect.bottom);
                }
              }}
            >
      ...
      >
        {visibleItems.map((item) => {
          if (item.key === "labBarcodeLookup") {
            const isBarcodeLookupDisabled =
              isApprovalLocked && !approvalEnabledItemKeys.has(item.key);

            if (isBarcodeLookupDisabled) {
              if (isCollapsed) {
                return (
                  <Tooltip
                    key={item.key}
                  ...
                  >
                    <div
                      ...
                      className={`${baseItem} ${baseItemCollapsed} ${disabledItem} w-full`}
                    >
              ...
              return (
                ...
                >
                  <div
                    ...
                    aria-disabled="true"
                    title="Available after account approval"
                    className="flex min-w-0 cursor-not-allowed items-center gap-2.5 rounded-xl border border-slate-200 bg-slate-50 px-3 py-2 opacity-50 dark:border-slate-700 dark:bg-slate-800 sm:px-4"
                  >
                    <span className={`${iconBox} ${iconBoxIdle} shrink-0`}>
                      {item.icon}
                    </span>
                    <span className="w-full truncate bg-transparent text-[14px] font-semibold text-slate-400 dark:text-slate-500 sm:text-[15px]">
                      Barcode Lookup
                    </span>
                    <span className="flex h-7 w-7 shrink-0 items-center justify-center rounded-lg bg-slate-200/50 text-slate-500 dark:bg-slate-700/50 dark:text-slate-500">
          ...
          const isOfflineDisabled = isOffline && ["reports", "subscribedPatients", "configuration"].includes(item.key);
          const isDisabled =
            (isAdminSetupLocked && !approvalEnabledItemKeys.has(item.key)) || isOfflineDisabled;
          ...
          if (isDisabled) {
            const tooltipContent = isOfflineDisabled ? (
              <div className="max-w-[220px] px-1 py-0.5">
                ...
                <p className="mt-1 text-[11px] leading-snug text-slate-500 dark:text-slate-300">
                  {item.key === "subscribedPatients" ? "Billing and payments require an internet connection." : `${item.label} require an internet connection.`}
                </p>
              </div>
          ...
          if (item.children && item.children.length > 0) {
            const isGroupActive =
              ...
              item.children.some((child) => {
                ...
                return (
                  childPath === childTarget || childPath.startsWith(`${childTarget}/`)
                );
              });
          ...
          return (
            ...
            >
              ...
              >
                ...
                {item.key === "banners" && activeBannerCount > 0 && (
                  <span
                    className={`inline-flex items-center justify-center rounded-full bg-[#f0ebff] text-[#7c3aed] shadow-sm dark:bg-[#282043] dark:text-[#c8b6ff] transition-all duration-300 ease-in-out ${
                      isCollapsed
                        ? "w-0 max-w-0 opacity-0 p-0 border-0 scale-0 select-none pointer-events-none overflow-hidden"
                        : "w-auto max-w-[32px] opacity-100 px-2 py-[3px] text-[10px] font-semibold leading-none"
                    }`}
                  >
                    {activeBannerCount}
                  </span>
          ...
          <div className={`mt-2 ${isCollapsed ? "px-2" : ""}`}>
            {isCollapsed && !callStatus ? (
              ...
              >
                ...
                <PopoverContent className="rounded-xl border border-line bg-surface p-0 shadow-lg">
      ...
      >
        <Tooltip
          content={
            isOffline ? (
              <div className="max-w-[220px] px-1 py-0.5">
                ...
                <p className="mt-1 text-[11px] leading-snug text-slate-500 dark:text-slate-300">
                  My Profile requires an internet connection.
                </p>
              </div>
            ) : isAdminSetupLocked ? (
              approvalLockedTooltip
          ...
          isDisabled={!isCollapsed && !isAdminSetupLocked && !isOffline}
          showArrow={isAdminSetupLocked || isOffline}
          placement="right"
          closeDelay={0}
        >
          {isAdminSetupLocked || isOffline ? (
            <div className={profileCardClassName} style={{ opacity: isOffline ? 0.5 : 1, cursor: 'not-allowed' }}>{profileCardContent}</div>
          ) : (
            <NavLink
