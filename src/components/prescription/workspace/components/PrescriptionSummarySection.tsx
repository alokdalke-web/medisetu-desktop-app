...
import { buildDosePattern, buildScheduleText, calcTotalDoses } from "../helpers/doseHelpers";
import { EmptyPrescriptionSummary } from "../../PrescriptionWorkspaceUi";
import type { Dose, SelectedMed } from "../types";
import MedicineTable from "./summary/MedicineTable";
import DentalChart from "./dental/DentalChart";
import { isDentistSpeciality } from "../helpers/dentalChart";
import {
...
const PrescriptionSummarySection: React.FC<{
  ...
  favouriteDialogOpen?: boolean;
  onFavouriteDialogOpenChange?: (open: boolean) => void;
  doctorSpeciality?: string | null;
  additionalInformation: Record<string, string>;
  onAdditionalInformationChange: (next: Record<string, string>) => void;
}> = ({
  selectedMeds,
  ...
  favouriteDialogOpen,
  onFavouriteDialogOpenChange,
  doctorSpeciality,
  additionalInformation,
  onAdditionalInformationChange,
}) => {

    // Owned by the workspace so the trigger can live in the search row while
    // the dialog stays here with the favourite-prescription logic.
    const showFavoriteInput = favouriteDialogOpen ?? false;
    const setShowFavoriteInput = onFavouriteDialogOpenChange ?? (() => {});
    ...
    return (
      <div className="flex min-h-0 min-w-0 flex-1 flex-col overflow-hidden bg-surface">
        {/* Only rendered when it has something to show. The "Prescription
            Summary / N medicines added" title was removed as redundant (the
            table lists the medicines, the footer bar shows the count) and its
            star moved to the search row — leaving this bar empty, and an empty
            bordered strip above the table reads as a rendering bug. */}
            ...
            <div className="flex min-w-0 flex-wrap items-center gap-2">
              {favouritePrescriptions.length > 0 && (
                ...
                >
                  ...
                  {isFavouriteDropdownOpen && (
                    <div className="absolute right-0 z-50 mt-2 max-h-64 w-full overflow-y-auto rounded-xl border border-line bg-surface p-1.5 shadow-xl shadow-slate-200/70 dark:shadow-black/30">
                      <button
                        type="button"
              ...
              {selectedMeds.length > 0 && (
                <>
                  {favoritePrescriptionName.trim() && (
                    <div className="hidden max-w-[180px] items-center gap-1.5 rounded-full border border-amber-100 bg-warning/15 px-3 py-2 text-[11px] font-bold text-amber-700 shadow-sm sm:inline-flex">
                      <span className="truncate">{favoritePrescriptionName}</span>
                    </div>
                  )}

                  {/* Star moved to the search row, beside the history button. */}
                </>
              )}
        ...
        <div className="no-scrollbar min-h-0 min-w-0 flex-1 overflow-y-auto overflow-x-hidden px-4 py-4 pb-24 sm:px-5">
          {selectedMeds.length === 0 ? (
            <>
              <div className="min-h-[200px] overflow-hidden rounded-2xl border border-dashed border-teal-100 bg-surface px-2 py-4 dark:border-[#46beae]/30 sm:px-4 sm:py-5">
                <EmptyPrescriptionSummary imageSrc={emptyPrescriptionImg} />
              </div>

              {isDentistSpeciality(doctorSpeciality) && (
                <DentalChart
            ...
            <>
              <div className="min-w-0 py-1">
                <MedicineTable
                  ...
                  isMedicineFavorite={isMedicineFavorite}
                  onToggleFavorite={onToggleMedicineFavorite}
                  ...
                  <button
                    ...
                    onClick={() => {
                      const searchInput = document.getElementById(
                        MEDICINE_SEARCH_INPUT_ID,
                      ) as HTMLInputElement | null;
                      if (!searchInput) return;

                      // Only scroll when the field is actually off-screen.
                      // Scrolling unconditionally (and to `center`) yanked the
                      // whole page even though the search box was already
                      // visible a few rows above, which read as the layout
                      // jumping on its own.
                      ...
                      // Focus after the smooth scroll settles — focusing
                      // mid-scroll makes the browser snap and cancels it.
        ...
        >
          <ModalContent className="overflow-hidden rounded-[28px]">
            {(onClose) => (
              <>
                {/* One accent, the brand one. This dialog previously ran an
                    amber→white→emerald gradient header, an amber "note" box, an
                    emerald focus ring and a raw `teal-600` button — four
                    competing colours in a 400px dialog, none of them the
                    screen's primary. */}
                  ...
                  <div className="flex items-start gap-3">
                    <div className="grid h-10 w-10 shrink-0 place-items-center rounded-xl bg-primary/10 text-primary dark:text-primary-hover">
                      <FiStar
                        size={18}
                        className={
                          favoritePrescriptionName.trim() ? "fill-current" : ""
                        }
                      />
                    </div>
                    ...
                    <div className="min-w-0">
                      ...
                      <p className="text-[12px] font-medium leading-4 text-text-muted">
                        Reuse this set of medicines on a future visit
                      </p>
                    </div>
                  </div>
                </ModalHeader>
                ...
                <ModalBody className="px-5 py-4">
                  {/* Informational, not a warning: nothing is wrong, the save
                      simply happens later. Amber-on-amber read as an error. */}
                  <div className="flex items-start gap-2 rounded-xl border border-line bg-surface-muted px-3 py-2.5">
                    <FiInfo className="mt-0.5 h-3.5 w-3.5 shrink-0 text-text-muted" />
                  ...
                  <div className="mt-4">
                    <label
                      htmlFor="favourite-prescription-name"
                    ...
                    <input
                      id="favourite-prescription-name"
                      type="text"
                      ...
                      placeholder="e.g. Fever prescription, Cold & cough"
                      maxLength={50}
                      disabled={!canEditPrescription}
                      autoFocus
                      className="h-11 w-full rounded-xl border border-line bg-surface px-3.5 text-sm font-medium text-text outline-none transition placeholder:text-text-subtle hover:border-primary/40 focus:border-primary focus:ring-2 focus:ring-primary/20 disabled:cursor-not-allowed disabled:bg-surface-muted disabled:text-text-subtle"
                    />

                    <div className="mt-1.5 text-right text-[11px] font-medium text-text-subtle">
                      {favoritePrescriptionName.length}/50
                    </div>
                  </div>
                </ModalBody>
                ...
                <ModalFooter className="border-t border-line px-5 py-4">
                  ...
                  <Button
                    ...
                    onPress={onClose}
                    isDisabled={
                      !canEditPrescription || !favoritePrescriptionName.trim()
                    }
                  >
                    Done
                  </Button>
        ...
        >
          <ModalContent>
            {(onClose) => (
              <>
                <ModalHeader className="text-text">
