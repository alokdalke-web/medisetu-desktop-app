...
import {
  ...
  ScheduleDoseButton,
  TimingOptionButton,
} from "./editor/DoseControls";
import {
  ALL_SCHEDULE_PATTERNS,
  FOOD_TIMING_OPTIONS,
  isStandardFoodTiming,
  toSuggestions,
} from "../helpers/schedulePattern";

// This editor keeps "0-0-0" (rendered as "None") as a selectable option, unlike
// the picker's list — an already-added medicine can be set to no scheduled dose.
const SCHEDULE_PATTERN_OPTIONS = ALL_SCHEDULE_PATTERNS;
const SCHEDULE_PATTERN_SUGGESTIONS = toSuggestions(SCHEDULE_PATTERN_OPTIONS);

type FoodTimingMode = (typeof FOOD_TIMING_OPTIONS)[number] | "Custom";
...
const PrescriptionEditorCard: React.FC<{
  ...
  onCollapse,
    ...
    const scheduleSuggestionsMenu =
      showScheduleSuggestions && canEdit && scheduleSuggestionsStyle ? (
        <div
          ...
          className="fixed z-[9999] min-w-[140px] overflow-hidden rounded-xl border border-line bg-surface p-1.5 shadow-xl shadow-slate-200/70 dark:shadow-black/30"
                  ...
                  <button
                    ...
                    className={[
                      ...
                      isSelected
                        ...
                        : "text-text hover:bg-slate-50 dark:hover:bg-[#151c2d]",
                    ].join(" ")}
                  >
    ...
    const medicineHeader = (
      ...
      >
        {typeof index === "number" && (
          <span className="grid h-6 w-6 shrink-0 place-items-center rounded-lg bg-success/10 text-sm font-bold text-emerald-700 dark:bg-emerald-950/40 dark:text-emerald-400">
            {index}
          </span>
        )}

        <div className="min-w-0">
          <div className="flex min-w-0 flex-wrap items-baseline gap-x-2 gap-y-1 text-[15px] font-bold text-text sm:text-[16px]">
        ...
        {onCollapse && (
          <span className="ml-1 mt-0.5 shrink-0 text-text-subtle" aria-hidden>
    ...
    if (shouldUseRouteLayout) {
      return (
        <div className="rounded-xl border border-line bg-surface px-4 py-4 shadow-[0_1px_3px_0_rgb(0_0_0/0.04)] dark:shadow-[0_1px_3px_0_rgb(0_0_0/0.2)]">
            ...
            <div className="flex shrink-0 items-center gap-2">
              {onToggleFavorite && (
                <button
                  ...
                  className={[
                    ...
                    isFavorite
                      ? "border-warning/40 bg-warning/15 text-warning dark:border-amber-700 dark:bg-amber-950/40"
                      : "border-line bg-surface text-text-subtle hover:border-warning/40 hover:bg-warning/15 hover:text-warning dark:hover:border-amber-700 dark:hover:bg-amber-950/30",
                  ].join(" ")}
                  aria-label={
              ...
              {canRemove && (
                <button
                  ...
                  disabled={!canEdit}
                  onClick={onRemove}
                  className="grid h-8 w-8 place-items-center rounded-full bg-danger/10 text-rose-500 transition hover:bg-danger/15 dark:bg-rose-500/15 dark:text-rose-200 dark:hover:bg-rose-500/25 disabled:cursor-not-allowed disabled:opacity-50"
          ...
          <div className="mt-3 rounded-lg bg-slate-50/60 p-2.5 dark:bg-[#0b1321]/50 sm:p-3">
            <div className="grid grid-cols-1 gap-2.5 sm:grid-cols-2 md:grid-cols-[minmax(140px,162px)_minmax(110px,128px)_minmax(88px,96px)_minmax(160px,1fr)]">
              <div className="grid h-9 grid-cols-2 overflow-hidden rounded-lg border border-slate-200/70 bg-surface dark:border-[#273244]/70">
                <button
                  type="button"
                  disabled={!canEdit}
                  onClick={() => changeFreq("daily")}
                  className={[
                    "border-r border-line text-sm font-medium transition",
                    compactFrequency === "daily"
                      ? "bg-success/10 text-emerald-700 dark:bg-emerald-950/40 dark:text-emerald-400"
              ...
              <div className="flex h-9 min-w-0 items-center rounded-lg border border-slate-200/70 bg-surface px-3 transition focus-within:border-emerald-400 focus-within:ring-2 focus-within:ring-emerald-100 focus-within:z-10 dark:border-[#273244]/70 dark:focus-within:border-[#46beae]/60 dark:focus-within:ring-[#46beae]/20">
                <input
                  type="number"
                  min={1}
                  max={
                  ...
                  className="min-w-0 flex-1 bg-transparent text-sm font-semibold text-text outline-none disabled:cursor-not-allowed disabled:text-slate-400 dark:disabled:text-slate-500"
                  aria-label="Duration"
                />
                <span className="shrink-0 text-xs font-medium text-text-muted">
                  {durationUnit}
              ...
              <div ref={routeDropdownRef} className="relative min-w-0">
                ...
                {isRouteDropdownOpen && canEdit && (
                  <div
                    ...
                    className="fixed z-[9999] min-w-[128px] overflow-hidden rounded-xl border border-line bg-surface p-1.5 shadow-xl shadow-slate-200/70 dark:shadow-black/30"
        ...
        <div className="rounded-xl border border-line bg-surface px-4 py-4 shadow-[0_1px_3px_0_rgb(0_0_0/0.04)] dark:shadow-[0_1px_3px_0_rgb(0_0_0/0.2)]">
          ...
          <div className="mt-3 rounded-lg bg-slate-50/60 p-2.5 dark:bg-[#0b1321]/50 sm:p-3">
            <div className="grid grid-cols-2 gap-x-2.5 gap-y-3 sm:grid-cols-3 lg:grid-cols-5 xl:grid-cols-[96px_130px_132px_118px_1fr]">
              ...
              >
                <label className="mb-1 block text-[10px] font-semibold uppercase tracking-wider text-text-subtle">Schedule</label>
              ...
              <div
                ...
                className="relative col-span-1 min-w-0"
              >
                <label className="mb-1 block text-[10px] font-semibold uppercase tracking-wider text-text-subtle">Timing</label>
                <button
                  type="button"
                  disabled={!canEdit}
                ...
                {isFoodTimingDropdownOpen && canEdit && (
                  <div
                    ...
                    className="fixed z-[9999] min-w-[160px] overflow-hidden rounded-xl border border-line bg-surface p-1.5 shadow-xl shadow-slate-200/70 dark:shadow-black/30"
                    onMouseDown={(event) => event.preventDefault()}
                  >
              ...
              {timingMode === "Custom" && (
                <div className="col-span-2 sm:col-span-1 min-w-0">
                  <label className="mb-1 block text-[10px] font-semibold uppercase tracking-wider text-text-subtle">Custom</label>
                  <input
                    type="text"
                    value={customFood}
                    disabled={!canEdit}
              ...
              <div className="col-span-1 min-w-0">
                <label className="mb-1 block text-[10px] font-semibold uppercase tracking-wider text-text-subtle">Frequency</label>
                <div className="grid h-9 grid-cols-2 overflow-hidden rounded-lg border border-slate-200/70 bg-surface dark:border-[#273244]/70">
                  <button
                    type="button"
                    disabled={!canEdit}
              ...
              <div className="col-span-1 min-w-0">
                <label className="mb-1 block text-[10px] font-semibold uppercase tracking-wider text-text-subtle">Duration</label>
                <div className="flex h-9 min-w-0 items-center rounded-lg border border-slate-200/70 bg-surface px-3 transition focus-within:border-emerald-400 focus-within:ring-2 focus-within:ring-emerald-100 focus-within:z-10 dark:border-[#273244]/70 dark:focus-within:border-[#46beae]/60 dark:focus-within:ring-[#46beae]/20">
                  <input
                    type="number"
                    min={1}
              ...
              <div className="col-span-2 sm:col-span-3 lg:col-span-2 xl:col-span-1 min-w-0">
                <label className="mb-1 block text-[10px] font-semibold uppercase tracking-wider text-text-subtle">Instruction</label>
                <input
                  type="text"
                  value={dosage}
                  disabled={!canEdit}
    ...
    return (
      <div className="rounded-xl border border-line bg-surface px-2 py-2 shadow-sm md:px-5 md:py-4">
        <div className="flex items-start justify-between gap-3">
          {medicineHeader}

          <div className="flex items-center gap-3">
            <span className="rounded-lg bg-success/10 px-3 py-2 text-sm font-semibold text-emerald-700 dark:bg-emerald-950/40 dark:text-emerald-400">
            ...
            {canRemove && (
              <button
                ...
                className={[
                  ...
                  canEdit
                    ? "border-danger/30 text-rose-600 hover:bg-danger/10 dark:border-rose-500/30 dark:bg-rose-500/10 dark:text-rose-200 dark:hover:bg-rose-500/20"
                    : "border-line text-slate-400 cursor-not-allowed dark:text-white",
                ].join(" ")}
                aria-label="Remove"
              >
        ...
        <div className="mt-4 grid grid-cols-1 gap-6 lg:grid-cols-2">
          <div>
            ...
            <div className="mt-4 text-xs font-semibold tracking-wide text-text-muted">
            ...
            <div className="mt-3 space-y-2">
              ...
              {canEdit && (
                <div className="w-full">
                  <input
                    type="text"
                    className="h-9 w-full rounded-full border border-line bg-surface px-4 text-xs focus:border-emerald-600 focus:outline-none dark:text-white dark:placeholder-slate-500 dark:focus:border-[#46beae]/60"
