...
import type { ParsedToken } from "../../helpers/prescriptionSyntax";
...
const CompletedPrescriptionEditModal: React.FC<{
  ...
  addMedicineDirectEdit: (m: any, quick?: QuickDoseArg) => void;
  editHighlight: number;
  parsedEditQuick?: QuickDoseArg;
  parsedEditTokens?: ParsedToken[];
  /** Medicine-name part of the edit query, with dose modifiers stripped. */
  editSearchTerm: string;
  setEditHighlight: React.Dispatch<React.SetStateAction<number>>;
    ...
    return (
      <Modal
        ...
        classNames={{
          ...
          footer: "p-0 shrink-0",
          closeButton:
            "top-3 right-3 sm:top-5 sm:right-5 z-20 text-text-muted hover:text-slate-700 hover:bg-slate-100 dark:hover:text-white dark:hover:bg-[#1e293b]",
        }}
      >
        <ModalContent>
          {(onClose) => (
            <>
              {/* ─── Header ─────────────────────────────────────────── */}
              <ModalHeader className="border-b border-line px-4 pt-4 pb-3 pr-12 sm:px-8 sm:pt-6 sm:pb-4 sm:pr-16">
              ...
              <ModalBody className="px-2 py-3 sm:px-6 sm:py-4 flex-1 min-h-0 !overflow-hidden">
                <div className="grid h-full min-h-0 grid-cols-1 gap-3 lg:grid-cols-12 lg:gap-4">
                  ...
                  <div className="col-span-1 flex min-h-0 flex-col overflow-hidden rounded-xl border border-line bg-surface shadow-sm lg:col-span-9 lg:rounded-2xl">
                    ...
                    <PrescriptionMedicineSidebar
                      ...
                      parsedQuick={parsedEditQuick}
                      parsedTokens={parsedEditTokens}
                      debouncedQuery={editSearchTerm}
                    ...
                    <div className="flex min-h-0 min-w-0 flex-1 flex-col overflow-hidden bg-surface">
                      ...
                      <div className="min-h-0 min-w-0 flex-1 overflow-y-auto overflow-x-hidden px-3 py-3 [scrollbar-color:#cbd5e1_transparent] [scrollbar-width:thin] [&::-webkit-scrollbar]:w-1.5 [&::-webkit-scrollbar-track]:bg-transparent [&::-webkit-scrollbar-thumb]:rounded-full [&::-webkit-scrollbar-thumb]:bg-slate-300/70 hover:[&::-webkit-scrollbar-thumb]:bg-slate-400 dark:[scrollbar-color:#334155_transparent] dark:[&::-webkit-scrollbar-thumb]:bg-[#334155] sm:px-5 sm:py-4">
                        {editSelectedMeds.length === 0 ? (
                          <div className="min-h-[280px] sm:min-h-[360px] overflow-hidden rounded-xl border border-dashed border-teal-100 bg-surface px-2 py-4 dark:border-[#46beae]/30 sm:rounded-2xl sm:px-4 sm:py-5 [&_img]:mx-auto [&_img]:h-auto [&_img]:max-w-full [&_svg]:max-w-full">
              ...
              {/* ─── Footer ─────────────────────────────────────────── */}
              <ModalFooter className="border-t border-line px-4 pb-4 pt-3 sm:px-8 sm:pb-6 sm:pt-4">
                <div className="flex w-full items-center justify-between gap-3">
                  {/* Medicine count badge */}
                  <div className="hidden text-[13px] font-medium text-text-muted sm:block">
                    {editSelectedMeds.length > 0 && (
