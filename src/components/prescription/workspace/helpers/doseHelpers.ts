...
/**
 * Turn a duration in days into the calendar window it actually covers.
 *
 * "5 Days" alone makes a patient count forward from a date they have to
 * remember; "31 Mar – 04 Apr" is the same fact already resolved. Returns an
 * empty string when there is no prescription date to anchor to, so callers can
 * fall back to the day count on its own.
 */
export const buildDurationDateRange = (
  dose: Dose,
...
/**
 * Apply the picker's quick dose selection on top of a form's default dose.
 *
 * Shared by the main workspace and the completed-prescription edit modal so
 * both honour the schedule/duration/frequency the doctor chose inline. The
 * edit modal previously dropped these values on the floor.
 */
export const applyQuickDose = (
  base: Dose,
  quick?: {
    pattern?: string;
    days?: number;
