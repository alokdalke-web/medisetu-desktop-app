...
export const highlightHtml = (text: string, q: string) => {
  ...
  // Alpha tint + explicit foreground: a raw `bg-emerald-100` is a near-white
  // patch in dark mode, and `<mark>`'s UA default color is black either way.
  return `${pre}<mark class="rounded bg-primary/20 px-1 text-inherit">${mid}</mark>${post}`;
};
...
export const extractAnyComposition = (x: any) =>
  String(
    x?.composition ?? x?.details?.composition ?? x?.medicine?.composition ?? "",
  ).trim();
...
export const getMedicineDetailsForForm = (
  ...
  const composition = extractAnyComposition(med);
