...
export const OnboardingStepSkeleton: React.FC<{
  ...
  return (
    <div
      className={["w-full font-outfit", className].join(" ")}
      aria-busy="true"
  ...
  <div className="w-full" aria-hidden="true">
    ...
    >
      {Array.from({ length: stepCount }).map((_, index) => {
        ...
        return (
          <div key={index} className="relative flex min-w-0 justify-center">
            {!isLast && (
              <div
            ...
            <div className="relative z-10 flex flex-col items-center">
              ...
              <Skel className="mt-1.5 h-3 w-20 rounded-lg" />
            </div>
          </div>
        );
      })}
...
export const OnboardingPageSkeleton: React.FC<{
  ...
  <div
    className="flex h-full min-h-0 w-full overflow-hidden font-outfit"
    aria-busy="true"
    aria-label="Loading onboarding"
  >
    <div className="flex min-h-0 w-full flex-col gap-3 overflow-hidden sm:gap-4 lg:gap-5">
      <div className="relative z-30 -mx-3 shrink-0 overflow-x-auto border-b border-slate-200 bg-white/95 px-3 py-1 shadow-[0_10px_22px_rgba(15,23,42,0.07)] backdrop-blur hide-scrollbar dark:border-slate-800 dark:bg-[#111726]/95 dark:shadow-[0_10px_22px_rgba(0,0,0,0.26)] sm:-mx-4 sm:px-4 lg:-mx-6 lg:px-6 2xl:-mx-8 2xl:px-8">
        <OnboardingStepperSkeleton stepCount={stepCount} />
      </div>

      <div className="relative min-h-0 flex-1 overflow-y-auto overflow-x-hidden overscroll-contain pr-1 [scrollbar-gutter:stable]">
        <div
          aria-hidden="true"
          className="pointer-events-none sticky top-0 z-20 -mb-4 h-4 bg-gradient-to-b from-white via-white/90 to-transparent dark:from-[#111726] dark:via-[#111726]/90"
        />
        {variant === "verification" ? (
          <OnboardingStepSkeleton variant="verification" showFooter={false} />
        ) : (
          <main className="w-full">
            <div className="flex flex-col overflow-visible rounded-2xl border border-slate-200 bg-white shadow-[0_14px_40px_rgba(15,23,42,0.05)] dark:border-slate-700 dark:bg-slate-900">
