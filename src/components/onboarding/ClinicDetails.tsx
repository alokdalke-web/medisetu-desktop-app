...
const CLINIC_TEXT_MAX_LENGTH = 50;

const ClinicDetails: React.FC<OnboardingClinicDetailsProps> = ({
  onNext,
  onBack,
  adminProfileData,
  onFormChange,
  submitLabel = "Continue",
  loadExistingClinic = true,
}) => {
  const { data: clinics, isLoading: isClinicsLoading } =
    useGetAllClinicsQuery(undefined, {
      skip: !loadExistingClinic,
  ...
  const { control, handleSubmit, reset, setValue, watch } =
    useForm<OnboardingClinicFormValues>({
      defaultValues: {
        clinicName: "",
        clinicPhone: "",
        tagline: "",
  ...
  const handleLocationChange = (
    ...
    if (addressDetails.address)
      setValue("clinicAddress", addressDetails.address, {
        shouldValidate: true,
        shouldDirty: true,
      });
    ...
    if (addressDetails.pincode)
      setValue("zipCode", addressDetails.pincode, {
        shouldValidate: true,
        shouldDirty: true,
      });
  };
  ...
  const handleCropSave = (croppedFile: File, croppedUrl: string) => {
    ...
    setValue("clinicLogo", croppedUrl, { shouldDirty: true, shouldValidate: true });
    setIsCropModalOpen(false);
    setPendingFile(null);
  };

  const onSubmit = async (data: OnboardingClinicFormValues) => {
    try {
      if (clinic?.id) {
        const payload: any = {
          clinicDetails: {
        ...
        if (selectedFile) {
          ...
          Object.keys(payload.clinicDetails).forEach((key) => {
            formData.append(
              `clinicDetails[${key}]`,
              String(payload.clinicDetails[key]),
            );
          });
          ...
          if (data.clinicLogo)
            payload.clinicDetails.clinicLogo = data.clinicLogo;
        ...
        if (data.latitude)
          formData.append("clinicDetails[latitude]", data.latitude);
        if (data.longitude)
          formData.append("clinicDetails[longitude]", data.longitude);
        ...
        if (adminProfileData) {
          ...
          formData.append(
            "adminProfile[mobile]",
            adminProfileData.mobile || "",
          );
          formData.append(
            "adminProfile[qualification]",
            adminProfileData.qualification || "",
          );
          formData.append(
            "adminProfile[licenseNumber]",
            adminProfileData.licenseNumber || "",
          );
          formData.append(
            "adminProfile[speciality]",
            adminProfileData.speciality || "",
          );
          formData.append(
            "adminProfile[yearsOfExperience]",
            adminProfileData.yearsOfExperience || "0",
          );
          formData.append(
            "adminProfile[isAdminDoctorAccess]",
            adminProfileData.isAdminDoctorAccess ? "true" : "false",
          );
        }
      ...
      >
        ...
        ════════════════════════════════════════ */}
            <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-3 lg:gap-4">
              <OnboardingInput
                ...
                isRequired
                maxLength={CLINIC_TEXT_MAX_LENGTH}
                parse={(val) => val.slice(0, CLINIC_TEXT_MAX_LENGTH)}
                rules={{
                  maxLength: {
                    value: CLINIC_TEXT_MAX_LENGTH,
                    message: "Clinic name must be 50 characters or fewer",
                  },
                }}
                icon={
              ...
              <OnboardingInput
                ...
                rules={{
                  maxLength: {
                    value: CLINIC_TEXT_MAX_LENGTH,
                    message: "Tagline must be 50 characters or fewer",
                  },
                }}
                icon={
                  <svg width="18" height="18" viewBox="0 0 24 24" fill="none">
        ...
        ════════════════════════════════════════ */}
            <div className="space-y-2">
              <div className="grid gap-3 lg:grid-cols-[minmax(0,520px)_minmax(240px,280px)] lg:items-end lg:gap-6">
                ...
                <div className="flex flex-col justify-end">
                  <div className="flex min-h-[84px] items-center rounded-xl border border-teal-100 bg-teal-50/70 p-3.5 dark:border-teal-900 dark:bg-teal-950/30 sm:p-4">
                    <div className="flex items-start gap-2.5 sm:gap-3">
                      <svg
                        ...
                        viewBox="0 0 24 24"
                        fill="none"
                        className="text-teal-600 dark:text-teal-400 flex-shrink-0 mt-0.5 sm:w-5 sm:h-5"
                      >
                        <path
                      ...
                      <div>
                        <h4 className="text-[12px] sm:text-[13px] font-semibold text-teal-900 dark:text-teal-100 mb-0.5 sm:mb-1">
                          Tip
                        </h4>
                        <p className="text-[11px] sm:text-[12px] text-teal-700 dark:text-teal-300 leading-relaxed">
                          A logo helps your patients recognize your clinic
                          easily.
                        </p>
                      </div>
                    </div>
        ...
        ════════════════════════════════════════ */}
            <div className="space-y-3">
              ...
              <div className="grid grid-cols-1 gap-5 lg:grid-cols-[minmax(0,520px)_1fr] lg:gap-7">
                {/* Left: Map */}
                <div className="overflow-hidden rounded-xl border border-slate-200 bg-white shadow-[0_1px_2px_rgba(15,23,42,0.03)] dark:border-slate-700 dark:bg-slate-800">
                  <div className="relative h-[170px] sm:h-[190px] lg:h-[clamp(160px,24dvh,224px)]">
                    <InteractiveMap
                      height="100%"
                      initialLocation={
                  ...
                  <div className="border-t border-slate-100 bg-slate-50/80 px-4 py-3 dark:border-slate-700 dark:bg-slate-800">
                    <div className="flex items-start gap-2">
                      ...
                      >
                        <path
                          d="M12 22s-8-4.5-8-11.8A8 8 0 0120 10.2c0 7.3-8 11.8-8 11.8z"
                          stroke="currentColor"
                          strokeWidth="2"
                          strokeLinecap="round"
                          strokeLinejoin="round"
                      ...
                      <p className="text-[11px] text-slate-600 dark:text-slate-400 leading-relaxed">
                        You can move the pin or drag on the map to set the exact
                        location.
                      </p>
                    </div>
                  </div>
          ...
          ════════════════════════════════════════ */}
          <div className="mt-4 flex flex-col items-stretch justify-between gap-3 border-t border-slate-200 bg-white pt-4 dark:border-slate-700 dark:bg-slate-900 sm:flex-row sm:items-center sm:gap-4">
