...
type DoctorAvailabilityStepProps = {
  ...
  onBack: () => void;
  submitLabel?: string;
  useSingleShiftOnFirstSetup?: boolean;
};
...
const DEFAULT_SELECTED_DAYS = DAYS.filter(
  (day) => day.value !== "Saturday",
).map((day) => day.value);
const DEFAULT_START_TIME = "09:00:00";
const DEFAULT_END_TIME = "18:00:00";
const DEFAULT_SLOT_MINUTES = "30";
const DEFAULT_SHIFTS = [{ startTime: DEFAULT_START_TIME, endTime: DEFAULT_END_TIME }];
const MAX_PATIENTS_DIGITS = 4;

const parseMaxPatientsInput = (value: string) =>
  value.replace(/\D/g, "").slice(0, MAX_PATIENTS_DIGITS);
...
function getAvailabilityBreaks(availability: any): any[] {
  if (Array.isArray(availability?.aivblityBreak)) return availability.aivblityBreak;
  if (Array.isArray(availability?.availabilityBreak)) return availability.availabilityBreak;
  if (Array.isArray(availability?.breaks)) return availability.breaks;
  return [];
}

function availabilityToShifts(startTime: string, endTime: string, breaks: any[] = []): Shift[] {
  ...
  const sortedBreaks = breaks
    .filter((breakItem) => breakItem?.status !== false)
    .map((breakItem) => ({
      start: parseTimeToMinutes(breakItem?.startTime),
      end: parseTimeToMinutes(breakItem?.endTime),
    }))
    .filter((breakItem): breakItem is { start: number; end: number } =>
      breakItem.start !== null &&
      breakItem.end !== null &&
      breakItem.end > breakItem.start &&
      breakItem.end > startMin &&
      breakItem.start < endMin
    )
    .sort((a, b) => a.start - b.start);

  if (!sortedBreaks.length) {
    return [{ startTime: formatMinutesTo24h(startMin), endTime: formatMinutesTo24h(endMin) }];
  }
  ...
  for (const breakItem of sortedBreaks) {
    const gapStart = Math.max(breakItem.start, startMin);
    const gapEnd = Math.min(breakItem.end, endMin);

    if (gapStart > currentStart) {
      shifts.push({
        startTime: formatMinutesTo24h(currentStart),
        endTime: formatMinutesTo24h(gapStart),
      });
    }
  ...
  if (endMin > currentStart) {
    shifts.push({
      startTime: formatMinutesTo24h(currentStart),
      endTime: formatMinutesTo24h(endMin),
    });
  }
...
const TimeSelectField = ({
  ...
  const triggerCls =
    "h-9 min-h-0  cursor-pointer rounded-lg border-slate-200 bg-white shadow-none data-[hover=true]:border-primary/50 data-[open=true]:border-primary dark:border-slate-700 dark:bg-slate-900";
    ...
    <div className="space-y-1 ">
      ...
      <div className="flex items-center gap-2 ">
        <Select aria-label={`${label} hour`} placeholder="HH" variant="bordered" size="sm" isDisabled={isDisabled}
          ...
          className="w-[82px] cursor-pointer" classNames={{ trigger: triggerCls, value: valueCls, selectorIcon: "text-slate-400 " }}>
...
const DoctorAvailabilityStep: React.FC<DoctorAvailabilityStepProps> = ({
  ...
  useSingleShiftOnFirstSetup = false,
}) => {
  const {
    data: doctorData,
  ...
  useEffect(() => {
    ...
    const activeAvailability = availability.filter((a: any) =>
      Boolean(a?.isAvailable),
    );
    const first = activeAvailability[0] ?? availability[0] ?? {};
    const activeDays = activeAvailability.map((a: any) => a?.dayOfWeek).filter(Boolean);
    const startTime = first?.startTime || DEFAULT_START_TIME;
    const endTime = first?.endTime || DEFAULT_END_TIME;
    const shifts = useSingleShiftOnFirstSetup
      ? [{ startTime, endTime }]
      : availabilityToShifts(startTime, endTime, getAvailabilityBreaks(first));
    ...
    const maxPatientsValue = parseMaxPatientsInput(maxPatientsFromBackend || maxPatientsFromNotes);
    reset({
      selectedDays: activeDays,
      consultMode: inferredMode,
      shifts: shifts.length > 0 ? shifts : [{ startTime, endTime }],
      applyToAll: true,
  ...
  }, [doctorData, reset, useSingleShiftOnFirstSetup]);
  ...
  const onSubmit = async (data: AvailabilityFormValues) => {
    try {
      ...
      if (data.consultMode === "token") {
        ...
        const n = Number(raw);
        if (!raw || !/^\d{1,4}$/.test(raw) || !Number.isFinite(n) || n <= 0) { addToast({ title: "Max patients required", description: "Please enter a valid number up to 4 digits.", color: "warning" }); return; }
      ...
      if (res?.success) { await refetchDoctor(); await onComplete?.(); await onNext(); }
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} className="flex flex-col font-outfit">
      <div className="px-4 pb-6 pt-3 sm:px-5 sm:pb-7 sm:pt-4 lg:px-6">
          ...
          {DAYS.map(({ label, value }) => {
            ...
            return (
              <button key={value} type="button"
                ...
                className={cn(
                  "flex h-10 items-center justify-center gap-2 rounded-lg border text-[13px] font-bold transition-all select-none cursor-pointer",
                  isSelected
      ...
      <section className="flex flex-col gap-3">
        ...
        <Controller name="consultMode" control={control} render={({ field }) => (
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            {([
              { value: "token" as const, icon: FiTag, label: "Token Based", sub: "Patients book using tokens. Each token represents a consultation.", tone: "text-primary bg-[#eaf5f4]" },
              ...
              <button key={opt.value} type="button" onClick={() => field.onChange(opt.value)}
                className={cn(
                  "flex min-h-[92px] items-center justify-between gap-4 rounded-xl border p-4 text-left transition-all cursor-pointer",
                ...
                <div className="flex min-w-0 items-center gap-4">
                  <div className={cn("flex h-14 w-14 shrink-0 items-center justify-center rounded-full", opt.tone)}>
                    <opt.icon className="h-7 w-7" />
                  </div>
                  <div className="min-w-0">
                    <p className="text-[15px] font-bold text-slate-900 dark:text-slate-100">{opt.label}</p>
                    <p className="mt-1 text-[13px] text-slate-600 dark:text-slate-400 leading-snug">{opt.sub}</p>
                  </div>
                </div>
                <span className={cn("flex h-6 w-6 shrink-0 items-center justify-center rounded-full border-2", field.value === opt.value ? "border-primary" : "border-slate-300 dark:border-slate-600")}>
                  <span className={cn("h-3 w-3 rounded-full", field.value === opt.value ? "bg-primary" : "")} />
                </span>
              </button>
            ))}
          </div>
      ...
      <section className="flex flex-col gap-3">
        <div className="flex items-center justify-between">
          ...
          <button type="button" onClick={addNewShift}
            className="flex items-center gap-1 text-xs sm:text-sm font-semibold text-primary hover:text-primary-hover transition-colors cursor-pointer">
          ...
          <div className="flex flex-col sm:flex-row items-start sm:items-end gap-3 pt-1">
            <div className="w-full sm:w-[240px]">
              <OnboardingInput
                ...
                type="tel"
                inputMode="numeric"
                maxLength={MAX_PATIENTS_DIGITS}
                parse={parseMaxPatientsInput}
                isRequired
                rules={{
                  required: "Max patients per day is required",
                  validate: (value: any) => {
                    if (!/^\d{1,4}$/.test(String(value || ""))) return "Enter up to 4 digits only";
              ...
              <div className="flex flex-wrap gap-3">
                {["15", "20", "30"].map((m) => (
                  <button key={m} type="button" onClick={() => field.onChange(m)}
                    className={cn("h-9 min-w-[92px] rounded-lg px-5 text-[13px] font-bold transition-all cursor-pointer",
        ...
        {/* ── Navigation ── */}
        <div className={cn("mt-4 flex items-center gap-4 border-t border-slate-100 bg-white pt-4 dark:border-slate-700 dark:bg-slate-900", isSaveMode ? "justify-end" : "justify-between")}>
        {!isSaveMode && (
          <Button
            type="button"
            variant="bordered"
