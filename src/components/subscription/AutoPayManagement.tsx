        ...
        <>
            <div className="rounded-2xl border border-default-200 dark:border-default-100 bg-background p-4 sm:p-5">
                <div className="flex items-center gap-2 mb-4">
                    <div className={`flex h-7 w-7 items-center justify-center rounded-lg ${autoRenew ? "bg-success-100 dark:bg-success-100/20" : pendingAuthorization ? "bg-warning-100 dark:bg-warning-100/20" : "bg-default-100 dark:bg-default-50/50"}`}>
                        <FiRefreshCw className={`text-xs ${autoRenew ? "text-success-600" : pendingAuthorization ? "text-warning-600" : "text-default-400"}`} />
                    </div>
                    ...
                    <Chip
                        size="sm"
                        color={autoRenew ? "success" : pendingAuthorization ? "warning" : "default"}
                        variant="flat"
                        startContent={autoRenew ? <FiCheck className="text-[9px]" /> : <FiX className="text-[9px]" />}
                    >
                        {autoRenew ? "Active" : pendingAuthorization ? "Pending" : "Inactive"}
                    </Chip>
                </div>
                ...
                <div className="rounded-xl bg-default-50 dark:bg-default-50/30 border border-default-100 dark:border-default-100/50 p-3 space-y-2.5">
                    ...
                    <div className="flex items-center gap-2.5">
                        ...
                        <div className="flex-1">
                            ...
                            <p className="text-[12px] font-medium text-default-700 dark:text-default-200">
                                {autoRenew
                                    ? "Mandate authorized — charges auto-debited"
                                    : pendingAuthorization
                                        ? "Awaiting authorization — complete it on Razorpay, then refresh"
                ...
                <div className="mt-4">
                    ...
                    {pendingAuthorization && (
                        <p className="text-[10px] text-warning-600 dark:text-warning-400 mt-2">
                            If you already completed authorization on Razorpay, click "Refresh Status" to sync. Otherwise, "Cancel & Retry" clears the pending mandate so you can start again.
                        </p>
                    )}
                    {!canEnable && reason && (
                        <p className="text-[10px] text-warning-600 dark:text-warning-400 mt-2">
                            ...
                            <div className="space-y-3">
                                ...
                                <div className="rounded-xl bg-primary-50 dark:bg-primary-100/5 border border-primary-100 dark:border-primary-200/20 px-4 py-3 space-y-1.5">
                                    ...
                                    <ul className="text-xs text-primary-600 dark:text-primary-400 space-y-1">
                                        ...
                                        <li className="flex items-start gap-2">
                                            <FiCheck className="shrink-0 mt-0.5 text-[10px]" />
                                            <span>Razorpay's authorization page will show your full plan amount — this is the exact amount that gets mandated for auto-renewal</span>
                                        </li>
                                        <li className="flex items-start gap-2">
                                            <FiCheck className="shrink-0 mt-0.5 text-[10px]" />
                                            <span>You won't be charged now — the mandate is only debited on your renewal date</span>
                                        </li>
                                        <li className="flex items-start gap-2">
                                            <FiCheck className="shrink-0 mt-0.5 text-[10px]" />
                                ...
                                <p className="text-[10px] text-default-400 italic">
                                    Note: after you authorize on Razorpay, it can take a moment to confirm — if AutoPay still shows "Pending" after completing authorization, refresh this page.
                                </p>
                            </div>
                        ) : (
                            <div className="space-y-3">
