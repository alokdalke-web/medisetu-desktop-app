...
export const StatCard = ({
  ...
  // Labels must be allowed to wrap. With `whitespace-nowrap` a long label
  // ("Total Appointments") gives every tile a ~200px minimum content width, so
  // a multi-column stat row overflows its cells and scrolls the whole PAGE
  // sideways on smaller screens. Wrapping is invisible when there's room.
  ...
  // Shown only when THIS tile is wide enough to hold the graphic beside the
  // value — measured on the tile itself (`@container/tile`), not the viewport.
  // The old `hidden md:flex lg:hidden 2xl:flex` rule inferred tile width from
  // window width, so in a 5-column row the 87px sparkline was drawn on top of
  // amounts like "₹2,000". A tile needs ~icon(36) + text(~120) + spark(87) +
  // padding(32).
  const sparklineClassName = compact
    ? "hidden @[250px]/tile:flex shrink-0 self-center items-center origin-right scale-[0.86]"
    : [
        "hidden @[290px]/tile:flex shrink-0 self-center items-center origin-right",
...
const SparklineNeutral = () => (
  ...
  return (
    // Outer element only establishes the `tile` container for the sparkline's
    // `@[…]/tile:` variant above — a container's variants can't resolve
    // against the element that declares the container itself.
    <div className="@container/tile flex min-w-0 flex-1">
    <div className={`flex min-w-0 flex-1 items-stretch justify-between border border-line bg-surface shadow-[0_1px_1px_rgba(0,0,0,0.05)] dark:shadow-none ${cardClassName}`}>
      <div className={`flex min-w-0 flex-1 items-start ${contentGapClassName}`}>
        <div
          className={`flex shrink-0 items-center justify-center rounded-lg ${iconClassName} ${bgColor}`}
        >
          {icon}
        </div>
        ...
        <div className="flex min-w-0 flex-1 flex-col">
          <div className="flex flex-col leading-[18px]">
            <span className={`${labelClassName} text-text-muted`}>
              {label}
            </span>
            {sublabel && (
              <span className={`${labelClassName} text-text-muted`}>
          ...
          <span className={`whitespace-nowrap text-text ${valueClassName}`}>
            {value}
          </span>
      ...
      <div className={sparklineClassName} aria-hidden="true">
