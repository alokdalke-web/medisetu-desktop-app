...
import PrescriptionDetails, { emptyPrescriptionDetails } from "../../components/prescription/PrescriptionDetails";
import PrescriptionRightPanel from "../../components/prescription/workspace/components/PrescriptionRightPanel";
import type {
  PreferenceState,
  ListSectionKey,
  ProfileOutletContext as OutletContextType,
} from "../../types/profile";
...
/**
 * Which option list a section owns, if any. Sections absent from this map
 * (Vitals, Advice, Follow-Up, …) are filled in at prescribing time and have
 * nothing for the doctor to configure here.
 */
const SECTION_LIST_KEYS: Partial<Record<string, ListSectionKey>> = {
  Habits: "habitList",
  Allergy: "allergyList",
  Diagnosis: "diagnosisList",
  "Surgery Suggested": "surgerySuggestedList",
  "Dietary Suggestions": "dietarySuggestionsList",
};
...
const PrescriptionPreference: React.FC = () => {
  ...
  /** Only one section's option editor is open at a time, to keep the list short. */
  ...
  const handleSave = async (closePreview?: boolean) => {
    if (!doctorId) return;
    ...
    try {
      ...
      // Remount the inline preview so it reflects what was just stored.
  ...
  const handleTogglePreview = () => {
    ...
    // Remount on open so the panel picks up the current preference draft.
    setPreviewRefreshKey((prev) => prev + 1);
  ...
  const hiddenSections = ALL_AVAILABLE_HEADERS.filter(
    (header) => !preferences.headerOrder.includes(header),
  );
  ...
  return (
    <>
      <Card className="overflow-hidden rounded-2xl shadow-none dark:bg-[#111726]">
          ...
          actions={
            <>
              ...
              <Button
                ...
                // The preview is always on screen from lg up, so the toggle
                // only exists at widths where it sits below the settings.
        ...
        <CardBody className="p-3 sm:p-6 [&_.text-default-900]:dark:text-white [&_.text-default-600]:dark:text-text-muted [&_.text-default-500]:dark:text-text-muted [&_.bg-default-50]:dark:bg-[#0f1728]">
          {" "}
          ...
          {!isInitialLoading && !doctorId && (
            <div className="rounded-2xl border border-line bg-surface p-5 text-sm text-text-muted shadow-sm">
              Doctor ID not available.
            </div>
          )}
          {!isInitialLoading && doctorId && (
            <div className="grid grid-cols-1 gap-4 lg:grid-cols-[minmax(0,1fr)_360px] lg:gap-6">
              {/* ── Configuration: one list, not three ───────────────────
                  Previously the same sections appeared as checkboxes, again
                  as an ordered list, and their options again as five cards
                  at the bottom of the page. Each section is now a single row
                  that carries its own on/off, position and options. */}
              <section className="min-w-0 space-y-4">
                <div className="rounded-2xl border border-line bg-surface p-3 sm:p-4">
                  <div className="mb-3">
                    <h3 className="flex items-center gap-2 text-sm font-semibold text-text">
                      <FiList className="text-primary" />
                      Sections on the prescription
                    </h3>
                    <p className="mt-1 text-xs text-text-muted">
                      Drag to reorder. Open a section to edit the options it
                      offers while prescribing.
                    </p>
                  </div>
                  ...
                  {preferences.headerOrder.length > 0 ? (
                    <ol className="space-y-2">
                      {preferences.headerOrder.map((item, index) => {
                        ...
                        const listKey = SECTION_LIST_KEYS[item];
                        const isExpanded = expandedSection === item;
                        ...
                        return (
                          <li
                            ...
                            className={`touch-none select-none rounded-xl border bg-surface transition-all ${isDragging
                              ? "border-primary opacity-70 shadow-md"
                              : "border-line"
                              } ${isDragOver ? "border-primary ring-2 ring-primary/40" : ""}`}
                          >
                            <div className="flex items-center gap-2 p-2.5 sm:gap-3 sm:p-3">
                              {/* Decorative: the whole row carries the drag and
                                  touch handlers. Hidden on the narrowest screens
                                  so the section name keeps usable width. */}
                              <span
                                aria-hidden="true"
                                className="hidden shrink-0 cursor-grab text-text-subtle active:cursor-grabbing sm:block"
                              ...
                              <div className="min-w-0 flex-1">
                                ...
                                <p className="mt-0.5 truncate text-[11px] text-text-muted">
                                  {listKey
                                    ? `${preferences[listKey].length} option${preferences[listKey].length === 1 ? "" : "s"}`
                                    : SECTION_TOOLTIPS[item]}
                                </p>
                              </div>
                              ...
                              {listKey && (
                                <button
                                  type="button"
                                  onClick={() =>
                                    setExpandedSection(isExpanded ? null : item)
                                  }
                                  aria-expanded={isExpanded}
                                  aria-label={`${isExpanded ? "Hide" : "Edit"} ${item} options`}
                                  className="flex h-10 w-10 shrink-0 items-center justify-center rounded-lg text-text-muted transition hover:bg-default-100 lg:h-8 lg:w-8"
                                >
                                  <FiChevronDown
                                    size={16}
                              ...
                              <button
                                type="button"
                                onClick={() => toggleHeaderItem(item, false)}
                                disabled={isButtonLoading}
                                aria-label={`Remove ${item} from the prescription`}
                                className="flex h-10 w-10 shrink-0 items-center justify-center rounded-lg text-text-muted transition hover:bg-danger/10 hover:text-danger disabled:opacity-50 lg:h-8 lg:w-8"
                              >
                                <FiX size={16} />
                              </button>
                            </div>
                            ...
                            {listKey && isExpanded && (
                              <div className="border-t border-line px-3 pb-3 pt-3">
                                <div className="grid grid-cols-[minmax(0,1fr)_auto] gap-2">
                                  <input
                                    type="text"
                                    value={newValues[listKey]}
                                    onChange={(e) =>
                                      updateNewValue(listKey, e.target.value)
                                    }
                                    onKeyDown={(e) =>
                                      handleInputKeyDown(e, listKey)
                                    }
                                    aria-label={`Add an option to ${item}`}
                                    placeholder={`Add ${item.toLowerCase()} option`}
                                    disabled={isButtonLoading}
                                    className="h-10 min-w-0 rounded-xl border border-line bg-surface px-3 text-sm text-text outline-none transition focus:border-primary disabled:cursor-not-allowed disabled:bg-surface-muted"
                                  />
                                  ...
                                  <button
                                    ...
                                    disabled={isButtonLoading}
                                    className="h-10 shrink-0 rounded-xl bg-primary px-4 text-sm font-semibold text-white transition disabled:cursor-not-allowed disabled:opacity-50"
                                  >
                                    Add
                                  </button>
                                </div>

                                {preferences[listKey].length > 0 ? (
                                  <div className="mt-3 flex flex-wrap gap-2">
                                    {preferences[listKey].map(
                                      (option, optionIndex) => {
                                        const isLast =
                                          preferences[listKey].length <= 1;

                                        return (
                                          <span
                                            key={`${option}-${optionIndex}`}
                                            className="inline-flex max-w-full items-center gap-1.5 rounded-full border border-line bg-surface-muted py-1 pl-3 pr-1 text-xs text-text"
                                          >
                                            ...
                                            <button
                                              ...
                                              className={`shrink-0 rounded-full p-1 text-text-subtle transition hover:bg-danger/10 hover:text-danger ${isLast ? "cursor-not-allowed opacity-50" : ""}`}
                                            >
                                              <FiTrash2 size={12} />
                                            </button>
                                          </span>
                    ...
                    <div className="rounded-xl border border-dashed border-line px-4 py-8 text-center text-sm text-text-muted">
                      No sections yet. Add one from the list below.
                    </div>
                  )}
                </div>
                ...
                {hiddenSections.length > 0 && (
                  <div className="rounded-2xl border border-line bg-surface p-3 sm:p-4">
                    ...
                    <p className="mt-1 text-xs text-text-muted">
                      Tap to add a section to the list above.
                    </p>

                    <div className="mt-3 flex flex-wrap gap-2">
                      {hiddenSections.map((header) => (
                        <Tooltip
                          key={header}
                          content={
                            <div className="max-w-[220px] text-[11px] leading-4 text-text">
                          ...
                          classNames={{
                            content:
                              "rounded-xl border border-line bg-surface px-3 py-2 shadow-lg",
                          }}
                        >
                          <button
                            type="button"
                            onClick={() => toggleHeaderItem(header, true)}
                            disabled={isButtonLoading}
                            className="inline-flex items-center gap-1.5 rounded-full border border-line bg-surface px-3 py-2 text-xs font-medium text-text transition hover:border-primary hover:text-primary disabled:opacity-50"
                          >
                            <FiPlus size={12} />
                            {header}
                          </button>
              ...
              {/* ── Live preview: the real appointment-flow panel ──────────
                  Sticky beside the settings on desktop so edits are visible
                  as they're made; collapsible below lg where there's no room.
                  `lg:block` is restated so the toggle can't hide it there. */}
              <aside
                aria-label="Live preview"
                className={`${isPreviewVisible ? "block" : "hidden"} min-w-0 lg:block`}
              >
                <div className="lg:sticky lg:top-4">
                  {/* Bounded height on lg so the panel's own `lg:h-full` +
                      internal scroll resolve against something; natural
                      height below that, where it stacks under the settings. */}
                  <div className="rounded-2xl border border-line bg-surface-muted p-3 lg:flex lg:h-[calc(100vh-9rem)] lg:flex-col">
                    <h3 className="text-sm font-semibold text-text">
                      Live preview
                    </h3>
                    <p className="mb-3 mt-0.5 text-xs text-text-muted">
                      Exactly how this panel appears while prescribing
                    </p>

                    <div className="lg:min-h-0 lg:flex-1">
                      <PrescriptionRightPanel
                        key={previewRefreshKey}
                        details={emptyPrescriptionDetails}
                        onChange={() => { }}
                        canEditPrescription
                        lockMessage=""
                        addedTests={[]}
                        ...
                        previewPreferences={{
                          ...
                          dietarySuggestionsList:
                            preferences.dietarySuggestionsList,
                        }}
                      />
                    </div>
                  </div>
