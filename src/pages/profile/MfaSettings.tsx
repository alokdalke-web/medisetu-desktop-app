...
import type { MfaStep } from "../../types/profile";
import { useGetUserQuery } from "../../redux/api/authApi";

export default function MfaSettings() {
  const { data: userRaw } = useGetUserQuery();
  const isPending = userRaw?.userStatus === "Pending";
  ...
  const totpInputClass =
    "h-[46px] w-full rounded-lg border border-[#E2E8F0] dark:border-[#273244] bg-surface px-4 text-center text-base font-mono tracking-[0.25em] text-[#1A1A2E] dark:text-white outline-none transition-colors focus:border-[#0A6C74] focus:ring-1 focus:ring-[#0A6C74] disabled:opacity-50";

  const passwordInputClass =
    "h-[46px] w-full rounded-lg border border-[#E2E8F0] dark:border-[#273244] bg-surface px-4 pr-10 text-sm text-[#1A1A2E] dark:text-white outline-none transition-colors focus:border-[#0A6C74] focus:ring-1 focus:ring-[#0A6C74] disabled:opacity-50";

  if (statusLoading) {
    return (
      <div className="rounded-2xl bg-surface border border-[#E8EEEE] dark:border-[#273244] p-8">
  ...
  return (
    <>
      <div className="rounded-2xl bg-surface border border-[#E8EEEE] dark:border-[#273244]">
      ...
      <Modal
        ...
        backdrop="blur"
        isDismissable={false}
        classNames={{ base: "mx-3 rounded-2xl dark:bg-[#111726]", closeButton: "text-text-muted hover:bg-slate-100 dark:hover:bg-[#172033]" }}
      >
        <ModalContent>
          ...
          <ModalBody className="px-6 py-4">
            ...
            <div className="flex flex-col items-center gap-3 py-2">
              <div className="rounded-xl border border-[#E8EEEE] dark:border-[#273244] bg-surface dark:bg-surface p-3">
                <img
            ...
            <div className="rounded-xl bg-[#F8FAFA] dark:bg-[#172033] border border-[#E8EEEE] dark:border-[#273244] p-4">
              <div className="grid grid-cols-2 gap-2">
                {recoveryCodes.map((code, i) => (
                  <div
                    key={i}
                    className="rounded-lg bg-surface px-3 py-2.5 text-center font-mono text-[13px] text-[#1A1A2E] dark:text-white border border-[#E8EEEE] dark:border-[#273244] select-all"
                  >
                    {code}
                  </div>
                ))}
