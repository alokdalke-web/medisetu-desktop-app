...
import type { MedicineRow } from "../../types/profile";
...
const MedicineEmptyState = ({
  ...
  return (
    <div className="mt-4 rounded-2xl border border-gray-200 bg-surface px-4 py-12 sm:py-14">
...
const Medicine: React.FC = () => {
  ...
  const [favoriteFilter, setFavoriteFilter] = useState<"all" | "favorite" | "nonFavorite">("all");
  const [favFilterOpen, setFavFilterOpen] = useState(false);
  const favFilterRef = React.useRef<HTMLDivElement | null>(null);
  ...
  useEffect(() => {
    const handleClick = (e: MouseEvent) => {
      ...
      if (favFilterRef.current && !favFilterRef.current.contains(e.target as Node)) {
  ...
  const queryArg = React.useMemo(() => {
    const args: any = {};
    if (statusFilter === "active") args.isActive = true;
    if (statusFilter === "disabled") args.isActive = false;

    if (favoriteFilter === "favorite") args.isFavorite = true;
    if (favoriteFilter === "nonFavorite") args.isFavorite = false;
  ...
  const handleToggleFavorite = async (r: MedicineRow) => {
    try {
      await toggleFavorite(r.id).unwrap();
    } catch (e: any) {
      const errorMsg = e?.error || e?.data?.message || "";
      addToast({
        title: "Failed",
        description: errorMsg.includes("Medicine not found")
          ? "You can only favorite medicines you have created"
          : (e?.data?.message ?? "Failed to toggle favorite status"),
        color: "danger",
      });
    }
  };
  ...
  return (
    <>
      <div className="rounded-2xl border border-slate-200 bg-surface overflow-hidden dark:border-[#273244] dark:bg-[#111726] [&_.border-gray-200]:dark:border-[#273244] [&_.border-gray-100]:dark:border-[#273244] [&_.bg-surface]:dark:bg-[#0f1728] [&_.bg-slate-50]:dark:bg-[#0f1728] [&_.bg-slate-50\\/70]:dark:bg-[#0f1728] [&_.text-text]:dark:text-text-subtle [&_.text-text]:dark:text-white [&_.text-text-muted]:dark:text-text-muted [&_.hover\\:bg-gray-50]:dark:hover:bg-[#1a2535]">
        <ProfilePageHeader
        ...
        <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between border-b border-slate-100 px-5 sm:px-6 py-3 dark:border-[#273244]">
          <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:gap-3">
            <Input
              ...
              startContent={<FiSearch className="text-text-muted" />}
              className="w-full sm:max-w-[280px]"
              classNames={{
                inputWrapper: "rounded-full border border-gray-200 bg-surface h-9 shadow-none",
                input: "text-sm",
              }}
            />
            ...
            <div className="relative" ref={filterRef}>
              <button
                ...
                className="flex h-9 items-center gap-2 whitespace-nowrap rounded-lg border border-gray-200 bg-surface px-4 pr-9 text-sm text-text outline-none cursor-pointer hover:border-slate-300 focus:border-teal-600 transition"
              >
                {statusFilter === "all" ? "All Status" : statusFilter === "active" ? "Active Only" : "Disabled Only"}
                <svg className="absolute right-3 h-3.5 w-3.5 text-text-muted" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 9l-7 7-7-7" />
                </svg>
              </button>
              {filterOpen && (
                <div className="absolute left-0 top-full z-50 mt-1 min-w-[150px] overflow-hidden rounded-xl border border-gray-200 bg-surface py-1 shadow-lg">
                  {([
            ...
            <div className="relative" ref={favFilterRef}>
              <button
                type="button"
                onClick={() => setFavFilterOpen((p) => !p)}
                className="flex h-9 items-center gap-2 whitespace-nowrap rounded-lg border border-gray-200 bg-surface px-4 pr-9 text-sm text-text outline-none cursor-pointer hover:border-slate-300 focus:border-teal-600 transition"
              >
                {favoriteFilter === "all" ? "All" : favoriteFilter === "favorite" ? "Favorite Only" : "Non-Favorite Only"}
                <svg className="absolute right-3 h-3.5 w-3.5 text-text-muted" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 9l-7 7-7-7" />
                </svg>
              </button>
              {favFilterOpen && (
                <div className="absolute left-0 top-full z-50 mt-1 min-w-[150px] overflow-hidden rounded-xl border border-gray-200 bg-surface py-1 shadow-lg">
                  {([
                    ...
                    { value: "nonFavorite", label: "Non-Favorite Only" },
                  ] as const).map((opt) => (
                    <button
                      key={opt.value}
                      type="button"
                      onClick={() => {
                        setFavoriteFilter(opt.value);
                        setFavFilterOpen(false);
                      }}
                      className={`flex w-full items-center gap-2 px-4 py-2 text-left text-sm transition ${favoriteFilter === opt.value
                        ? "bg-primary/10 text-primary font-medium"
                        : "text-text hover:bg-slate-50"
                        }`}
                    >
                      ...
                      {favoriteFilter !== opt.value && <span className="w-3.5" />}
                      {opt.label}
                    </button>
                  ))}
                </div>
        ...
        <div className="px-5 sm:px-6 py-4">
          ...
          {isLoading && (
            <div className="mt-4 rounded-2xl border border-gray-200 p-6 text-center text-text-muted">
              Loading medicines...
            </div>
          )}
                ...
                <>
                  ...
                  <div className="mt-4 grid gap-3 sm:hidden">
                    {paginated.map((r) => (
                      <div
                        key={r.id}
                        className={`rounded-2xl border border-gray-200 p-3 ${!r.isActive ? "bg-slate-50/50" : "bg-surface"}`}
                      >
                        <div className="flex items-start justify-between gap-3">
                          <div className="min-w-0 flex-1 flex items-center gap-2">
                            <button
                              ...
                              className={`shrink-0 focus:outline-none ${
                                r.isActive ? "cursor-pointer" : "cursor-not-allowed opacity-40"
                              }`}
                              title={!r.isActive ? "Activate medicine to favorite" : r.isFavorite ? "Remove from favorites" : "Add to favorites"}
                            >
                              <FiStar
                                className={`h-[18px] w-[18px] transition-colors ${r.isFavorite
                                    ? "text-yellow-500 fill-current"
                                    : "text-text-subtle hover:text-yellow-500"
                                  }`}
                              />
                            </button>
                            <div className={`min-w-0 ${!r.isActive ? "opacity-50" : ""}`}>
                          ...
                          <div className="flex items-center gap-2 shrink-0">
                            <button
                              className="grid h-8 w-8 place-items-center rounded-full border border-gray-200 hover:bg-gray-50 transition"
                              onClick={() => openEdit(r)}
                              title="Edit"
                              type="button"
                            >
                              <FiEdit2 className="text-text" />
                            </button>
                            <button
                              className="grid h-8 w-8 place-items-center rounded-full border border-gray-200 hover:bg-gray-50 transition"
                        ...
                        <div className={`mt-3 grid grid-cols-2 gap-x-3 gap-y-2 text-sm ${!r.isActive ? "opacity-50" : ""}`}>
                          <div className="min-w-0">
                            <div className="text-[11px] text-text-muted">Brand Name</div>
                            <div className="truncate text-text">{r.brandName || "—"}</div>
                          </div>
                          <div className="min-w-0">
                            <div className="text-[11px] text-text-muted">Manufacturer</div>
                            <div className="truncate text-text">
                              {r.manufacturer || "—"}
                            </div>
                          </div>
                          <div className="min-w-0">
                            <div className="text-[11px] text-text-muted">Form & Str.</div>
                            <div className="truncate text-text">
                              {(r.form || "—") + (r.strength ? ` ${r.strength}` : "")}
                            </div>
                          </div>
                          <div className="min-w-0">
                            <div className="text-[11px] text-text-muted">Composition</div>
                            <div className="truncate text-text">
                              {r.composition || "—"}
                            </div>
                          </div>
                    ...
                    <div className="overflow-x-auto">
                      <table className="w-full text-left text-sm">
                        <thead className="bg-slate-50/70">
                          <tr className="border-b border-gray-200 text-text-muted font-normal">
                        ...
                        <tbody>
                          {paginated.map((r) => (
                            <tr
                              key={r.id}
                              className={`border-b border-gray-200 last:border-b-0 ${!r.isActive ? "bg-slate-50/50" : ""}`}
                            >
                              <td className={`px-5 py-4 ${!r.isActive ? "opacity-50" : ""}`}>
                                <div className="min-w-0">
                                  <div className="flex items-center gap-3 truncate text-text" title={r.brandName}>
                                    <button
                                      type="button"
                                      disabled={!r.isActive}
                                      onClick={() => handleToggleFavorite(r)}
                                    ...
                                    <span className={`flex h-9 w-9 shrink-0 items-center justify-center rounded-full ${r.form?.toLowerCase() === "tablet" ? "bg-emerald-50 text-emerald-600"
                                      : r.form?.toLowerCase() === "capsule" ? "bg-blue-50 text-blue-600"
                                        : r.form?.toLowerCase() === "syrup" || r.form?.toLowerCase() === "suspension" || r.form?.toLowerCase() === "liquid" ? "bg-slate-100 text-text-muted"
                  ...
                  <div className="truncate text-text" title={r.strength}>
                  ...
                  >
                    {r.manufacturer || "—"}
                              ...
                              <td className={`px-5 py-4 ${!r.isActive ? "opacity-50" : ""}`}>
                                <div className="truncate text-text" title={r.form}>
                                  {r.form || "—"}
                                </div>
                              </td>
                              ...
                              <td className={`px-5 py-4 ${!r.isActive ? "opacity-50" : ""}`}>
                                <div
                                  className="max-w-[320px] truncate text-text"
                                  title={r.composition}
                                >
                                  {r.composition || "—"}
                                </div>
                      ...
                      {r.strength ? `${r.strength} mg` : "—"}
                              ...
                              <td className="px-5 py-4">
                                <div className="flex items-center justify-center gap-2">
                                  <button
                                    className="grid h-9 w-9 place-items-center rounded-full border border-gray-200 bg-surface hover:bg-gray-50 transition"
                                    onClick={() => openEdit(r)}
                                    title="Edit"
                                    type="button"
                                  >
                                    <FiEdit2 className="h-4 w-4 text-text-muted" />
                                  </button>

                                  <button
                                    className="grid h-9 w-9 place-items-center rounded-full border border-gray-200 bg-surface hover:bg-rose-50 transition"
                                    onClick={() => openDelete(r)}
                                    title="Delete"
                                    type="button"
                                  >
