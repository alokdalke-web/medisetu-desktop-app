...
import type { ClinicFormValues } from "../../types/clinicDetails";
...
import type { AddClinicProps } from "../../types/profile";
  ...
  const handleUseCurrentLocation = () => {
    ...
    navigator.geolocation.getCurrentPosition(
      async (position) => {
        ...
        try {
          const addressDetails = await reverseGeocodeCoordinates(
            latitude,
            longitude,
            { nominatimLanguage: "en-US,en;q=0.9" },
          );
          ...
          // `city` as well as `City`: the payload reads `City`, but the City &
          // State selector is bound to lowercase `city`.
            ...
            <div className="rounded-2xl border border-slate-200 bg-slate-50/50 p-4 sm:p-5 space-y-5">
              <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
                ...
                <button
                  ...
                  disabled={isLocating}
                  className="inline-flex items-center justify-center gap-2 rounded-lg border border-primary bg-surface px-4 py-2 text-[12px] font-semibold text-primary hover:bg-primary/10 disabled:cursor-not-allowed disabled:opacity-60"
                >
