...
import type {
  SingleMedicineValues,
  AddMedicineModalProps as Props,
  AutoBoxProps,
} from "../../types/profile";
...
const tabIdle = "border-transparent text-text-muted hover:text-text";
...
function AutoBox({
  ...
  return (
    <div
      ref={boxRef}
      className="absolute z-50 mt-2 w-full overflow-hidden rounded-2xl border border-gray-200 bg-surface shadow-sm"
    >
      <div className="max-h-56 overflow-auto py-1">
        {isFetching ? (
          <div className="px-4 py-2 text-sm text-text-muted">Searching...</div>
        ) : suggestions.length === 0 ? (
          <div className="px-4 py-2 text-sm text-text-muted">No match found</div>
        ) : (
          suggestions.map((s, i) => (
            <button
              key={s + i}
                ...
                <div className="pt-5">
                  <div className="grid gap-4 md:grid-cols-2">
                    ...
                    <Input
                      ...
                      endContent={
                        <span className="text-sm text-text-muted">mg</span>
                      }
                      classNames={{ inputWrapper: "rounded-full" }}
                    />
                  </div>
                ...
                <div className="pt-5">
                  ...
                  {showPreview && csvFile ? (
                    <div className="flex flex-col gap-3">
                      ...
                      <div className="flex items-center justify-between">
                        <div className="flex items-center gap-3">
                          ...
                          >
                            <IoArrowBack className="text-lg text-text-muted" />
                          </button>
                      ...
                      <div className="max-h-[320px] overflow-auto rounded-xl border border-gray-200">
                        <table className="w-full text-left text-xs">
                          <thead className="sticky top-0 z-10 bg-slate-50">
                            <tr>
                              <th className="whitespace-nowrap border-b border-gray-200 px-3 py-2 font-semibold text-text-muted">
                                #
                              </th>
                              {previewHeaders.map((h, i) => (
                                <th
                                  key={i}
                                  className="whitespace-nowrap border-b border-gray-200 px-3 py-2 font-semibold text-text-muted"
                                >
                                  {h}
                                </th>
                              ))}
                          ...
                          <tbody>
                            {previewRows.map((row, ri) => (
                              <tr
                                key={ri}
                                className={ri % 2 === 0 ? "bg-surface" : "bg-slate-50/50"}
                              >
                                <td className="whitespace-nowrap border-b border-gray-100 px-3 py-1.5 text-text-muted">
                                  {ri + 1}
                                </td>
                                {previewHeaders.map((_, ci) => (
                                  <td
                                    key={ci}
                                    className="max-w-[180px] truncate border-b border-gray-100 px-3 py-1.5 text-text"
                                    title={row[ci] ?? ""}
                                  >
                                    {row[ci] ?? ""}
                                  </td>
                    ...
                    <>
                      ...
                      <div
                        className="mt-4 flex min-h-[150px] flex-col items-center justify-center rounded-2xl border-2 border-dashed border-gray-200 bg-surface px-4 text-center"
                        onDrop={onDrop}
                        onDragOver={onDragOver}
                      >
                        <FiUploadCloud className="mb-2 text-2xl text-text-muted" />
