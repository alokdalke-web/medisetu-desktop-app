...
import type { SettingsForm, EventInfo } from "../../types/profile";
    ...
    <div className="space-y-0">
      ...
      <form onSubmit={handleSubmit(onSubmit)}>
        <div className="px-5 sm:px-6 py-5">
          ...
          {localPreferences ? (
            <div className="rounded-xl border border-slate-200 bg-surface dark:border-[#273244] dark:bg-[#111726] overflow-hidden">
              {/* Table Header */}
              <div className="grid grid-cols-[1fr_72px_72px] sm:grid-cols-[1fr_90px_90px] gap-2 px-4 py-3 bg-slate-50/80 dark:bg-[#0f1728] border-b border-line text-[10px] sm:text-[11px] font-semibold text-text-muted uppercase tracking-wider">
                        ...
                        <div className="flex items-center gap-2.5 min-w-0">
                          ...
                          <div className="min-w-0">
                            <div className="flex items-center gap-1.5">
                              <span className="text-[13px] font-medium text-text truncate">{mapping.name}</span>
                              {(!inAppConfigurable || !pushConfigurable) && (
                                <span className="inline-flex items-center gap-0.5 rounded bg-slate-100 dark:bg-[#273244] px-1 py-0.5 text-[8px] font-medium text-text-muted">
                                  <LuLock size={7} />
                                </span>
                              )}
                            </div>
                            {mapping.description && (
                              <p className="text-[10px] sm:text-[11px] text-text-muted leading-normal mt-0.5 line-clamp-1">{mapping.description}</p>
                            )}
                          </div>
                        </div>
            ...
            <div className="rounded-xl border border-line bg-slate-50 dark:bg-[#0f1728] p-10 text-center text-[13px] text-text-muted">
        ...
        {/* Save Actions */}
        <div className="flex justify-end gap-3 px-5 sm:px-6 py-4 border-t border-line">
          <AppButton
            text="Cancel"
            onPress={() => {
              if (prefData?.success && prefData.data) {
