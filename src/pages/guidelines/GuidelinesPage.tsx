...
import { IoClose, IoMenu, IoArrowUp, IoSearch, IoCopyOutline, IoCheckmark } from "react-icons/io5";
...
const guideSections = [
  ...
  },
  {
    id: "receptionist",
    title: "Receptionist",
    file: "docs/roles/receptionist.md",
  ...
  },
  {
    ...
    file: "docs/pharmacy/prescriptions.md",
  ...
  },
  {
    ...
    file: "docs/pharmacy/medicines.md",
  ...
  },
  {
    ...
    file: "docs/pharmacy/suppliers.md",
  ...
  },
  {
    ...
    file: "docs/pharmacy/subscriptions.md",
  ...
  {
    id: "pharmacy-guide-daily-checklist",
    title: "Pharmacy: Daily Checklist",
    file: "docs/pharmacy/daily-checklist.md",
    subtitle: "End-of-day tasks",
    category: "Feature Guides",
    icon: "✅",
    parent: "pharmacy-guide",
  },
...
function stripFrontmatter(md: string): string {
  // Tolerate both LF and CRLF line endings.
  const m = md.match(/^---\r?\n[\s\S]*?\r?\n---\r?\n/);
...
function parseDocument(md: string): ParsedDoc {
  ...
  // The hero blurb is ONLY the first real text paragraph after the H1 — never
  // images, videos, callouts, or headings. Those render once, in the body.
  const leadIdx = new Set<number>();
  const leadLines: string[] = [];
  if (h1Idx >= 0) {
    const skippable = (t: string) =>
      t === "" || /^!\[/.test(t) || /^https?:\/\/\S+$/.test(t);
    const stops = (t: string) =>
      /^#{1,6}\s/.test(t) || /^---\s*$/.test(t) || /^>/.test(t);
    let i = h1Idx + 1;
    while (i < lines.length && skippable(lines[i].trim())) i++;
    if (i < lines.length && !stops(lines[i].trim())) {
      while (i < lines.length) {
        const t = lines[i].trim();
        if (t === "" || stops(t) || /^!\[/.test(t)) break;
  ...
  // Body = everything except the H1 line and the extracted lead paragraph,
  // so the lead is not duplicated and callouts/images render properly.
  const body = lines
    .filter((_, idx) => idx !== h1Idx && !leadIdx.has(idx))
    .join("\n")
    .trim();

  return { h1, intro, body };
}
...
function makeComponents(dark?: boolean): Components {
  ...
  return {
    ...
    a: ({ href, children, ...p }) => {
      // A YouTube link on its own renders as an embedded, responsive player.
      const ytId = typeof href === "string" ? getYouTubeId(href) : null;
      const linkText = Array.isArray(children) ? children.join("") : String(children ?? "");
      if (ytId && (linkText === href || linkText.trim() === "" || /^(video|watch|play)\b/i.test(linkText))) {
        return <YouTubeEmbed id={ytId} title={linkText === href ? undefined : linkText} />;
      }
      // Internal cross-links between guides (e.g. /docs/pharmacy/sales) switch
      // the active section in-app instead of navigating to a dead URL.
      const isInternalDocLink =
        typeof href === "string" && /^\/?docs\//.test(href.replace(/^https?:\/\/[^/]+/, ""));
      return (
      <a
        href={href}
        onClick={
          isInternalDocLink
            ? (e) => {
                ...
                window.dispatchEvent(
                  new CustomEvent("doc-nav-section", { detail: { href } }),
                );
              }
            : undefined
        }
    ...
    code: ({ inline, children, ...p }: any) =>
      inline ? (
        ...
        <CopyableCodeBlock codeProps={p}>{children}</CopyableCodeBlock>
      ),
    blockquote: ({ children }) => {
      const firstText = (() => {
    ...
    img: ({ src, alt, title }: any) => {
      ...
      // Resolve doc-relative image paths against the Vite base ("/app/"),
      // so authors can write `/docs/images/foo.png` regardless of deploy base.
      ...
      return (
        <figure style={{ margin: "0 0 28px" }}>
          <button
            ...
            onClick={() =>
              window.dispatchEvent(
                new CustomEvent("doc-img-zoom", { detail: { src: resolvedSrc, alt } }),
              )
            }
            aria-label={caption ? `Zoom: ${caption}` : "Zoom image"}
            className="doc-figure-btn"
            style={{
              display: "block",
              width: "100%",
              padding: 0,
...
function CopyableCodeBlock({
  ...
  return (
    <div className="code-block" style={{ position: "relative", margin: "0 0 24px" }}>
      <button
        type="button"
        onClick={handleCopy}
        ...
        style={{
          ...
          fontWeight: 600,
          fontFamily: "inherit",
          color: copied ? "#4fbfa8" : "#cbd5e1",
          background: "rgba(15,23,42,0.6)",
          border: `1px solid ${copied ? `${ACCENT_LIGHT}55` : "#334155"}`,
          borderRadius: 8,
          cursor: "pointer",
      ...
      >
        {copied ? <IoCheckmark size={13} /> : <IoCopyOutline size={13} />}
        {copied ? "Copied" : "Copy"}
      </button>
      <pre
        style={{
...
function Lightbox() {
  ...
  useEffect(() => {
    const onZoom = (e: Event) => {
      setImg((e as CustomEvent).detail || null);
    };
    const onKey = (e: KeyboardEvent) => {
      if (e.key === "Escape") setImg(null);
    };
    window.addEventListener("doc-img-zoom", onZoom);
    window.addEventListener("keydown", onKey);
    return () => {
      window.removeEventListener("doc-img-zoom", onZoom);
...
function FeedbackWidget({ isDark }: { isDark: boolean }) {
  const [vote, setVote] = useState<"yes" | "no" | null>(null);

  const cardBg = isDark ? "#111726" : "#fff";
  const cardBorder = isDark ? "#273244" : "#e8ecf0";
  const textHeading = isDark ? "#f1f5f9" : "#0f172a";
  ...
  const btn = (active: boolean): React.CSSProperties => ({
    ...
    borderRadius: 10,
    cursor: "pointer",
    background: active ? ACCENT : isDark ? "#1e293b" : "#fff",
  ...
  return (
    ...
    >
      {vote ? (
        <div style={{ fontSize: 14.5, color: textHeading, fontWeight: 500 }}>
          {vote === "yes"
            ? "🎉 Thanks for your feedback!"
            : "🙏 Thanks — we'll use this to improve this guide."}
          {vote === "no" && (
            <span style={{ display: "block", marginTop: 4, fontSize: 13, color: textMuted, fontWeight: 400 }}>
              Need more help? Email{" "}
              <a href="mailto:support@infinitymedisetu.com" style={{ color: ACCENT, fontWeight: 600 }}>
        ...
        <>
          <div style={{ fontSize: 15, fontWeight: 600, color: textHeading }}>
            Was this page helpful?
          </div>
          <div style={{ display: "flex", gap: 10 }}>
            <button type="button" style={btn(false)} onClick={() => setVote("yes")}>
              👍 Yes
            </button>
            <button type="button" style={btn(false)} onClick={() => setVote("no")}>
              👎 No
            </button>
          </div>
        </>
...
const GLOBAL_STYLES = `
  ...
  /* Stepped ordered lists — numbered badges (Razorpay-style workflows) */
  .doc-ol { list-style: none; counter-reset: doc-step; }
  .doc-ol > li { counter-increment: doc-step; position: relative; padding-left: 44px; min-height: 28px; }
  .doc-ol > li::before {
    ...
    box-shadow: 0 2px 6px ${ACCENT}40;
  }
  /* Nested ordered lists keep their own counter, slightly smaller */
  .doc-ol .doc-ol > li::before { width: 24px; height: 24px; font-size: 11px; }

  /* Table rows highlight on hover for scannability */
  .doc-content tbody tr { transition: background 0.12s; }
  .doc-content tbody tr:hover { background: ${ACCENT}06; }

  /* Copy-code button: subtle until block hover */
  .code-copy-btn { opacity: 0; }
  .code-block:hover .code-copy-btn { opacity: 1; }
  .code-copy-btn:hover { color: #fff !important; border-color: ${ACCENT_LIGHT} !important; }
  @media (hover: none) { .code-copy-btn { opacity: 1; } }

  /* Figure zoom affordance */
  .doc-figure-btn img { transition: transform 0.2s ease, box-shadow 0.2s ease; }
  .doc-figure-btn:hover img { transform: translateY(-2px); box-shadow: 0 10px 28px rgba(15,23,42,0.14); }
...
export const GuidelinesPage = () => {
  ...
  // ── Global predictive search across all guides ──────────────────────────────
  type SearchEntry = {
    id: string;
    title: string;
    subtitle: string;
  ...
  const searchIndexRef = useRef<SearchEntry[] | null>(null);
  const pendingScrollRef = useRef<string | null>(null);
  const [indexReady, setIndexReady] = useState(false);
  const [gQuery, setGQuery] = useState("");
  const [gOpen, setGOpen] = useState(false);
  const [gActiveIdx, setGActiveIdx] = useState(0);

  const buildSearchIndex = useCallback(async () => {
    if (searchIndexRef.current) return;
    const base = (import.meta.env.BASE_URL || "/").replace(/\/$/, "") + "/";
    const entries = await Promise.all(
      guideSections.map(async (s): Promise<SearchEntry> => {
        const meta = { id: s.id, title: s.title, subtitle: s.subtitle, category: s.category };
        try {
          const res = await fetch(`${base}${s.file.replace(/^\//, "")}`);
          ...
          const plain = txt.replace(/[#>*`|_[\]()!-]/g, " ").replace(/\s+/g, " ");
          return { ...meta, headings, haystack: `${s.title} ${s.subtitle} ${plain}`.toLowerCase() };
        } catch {
          return { ...meta, headings: [], haystack: `${s.title} ${s.subtitle}`.toLowerCase() };
        }
      }),
    );
  ...
  const gResults = useMemo(() => {
    // Normalize the query the SAME way the index is built (strip markdown
    // punctuation, collapse spaces) so hyphenated terms like "cold-chain" match.
    const norm = (s: string) =>
      s.toLowerCase().replace(/[#>*`|_[\]()!-]/g, " ").replace(/\s+/g, " ").trim();
    const q = norm(gQuery);
    if (!q || !searchIndexRef.current) return [];
    const out: { id: string; title: string; category: string; heading: { text: string; slug: string } | null; score: number }[] = [];
    for (const e of searchIndexRef.current) {
      const titleMatch = norm(e.title).includes(q) || norm(e.subtitle).includes(q);
      const headingMatch = e.headings.find((h) => norm(h.text).includes(q)) || null;
      const bodyMatch = e.haystack.includes(q);
      if (titleMatch || headingMatch || bodyMatch) {
        out.push({ id: e.id, title: e.title, category: e.category, heading: headingMatch, score: titleMatch ? 0 : headingMatch ? 1 : 2 });
      }
    }
    out.sort((a, b) => a.score - b.score);
    return out.slice(0, 8);
    // indexReady is a dep so results recompute once the index finishes loading.
  ...
  // Internal doc cross-links (/docs/pharmacy/sales) switch the active section.
  useEffect(() => {
    const onDocNav = (e: Event) => {
      const href = (e as CustomEvent).detail?.href as string | undefined;
      if (!href) return;
      // Normalize "/docs/pharmacy/sales" (or with .md) → "pharmacy/sales".
      const target = href
        .replace(/^https?:\/\/[^/]+/, "")
        .replace(/^\/?docs\//, "")
        .replace(/\.md$/, "")
      ...
      const match = guideSections.find(
        (s) => s.file.replace(/^docs\//, "").replace(/\.md$/, "") === target,
      );
      if (match) setActiveId(match.id);
    };
    window.addEventListener("doc-nav-section", onDocNav);
    return () => window.removeEventListener("doc-nav-section", onDocNav);
  }, []);
  ...
  useEffect(() => {
    (async () => {
      ...
      try {
        ...
        // A missing .md falls through to the SPA's index.html rather than a
        // network error, so check explicitly or we render the shell as prose.
        if (!res.ok || res.headers.get("content-type")?.includes("text/html")) {
          throw new Error(`Not found: ${file}`);
        }
        // Normalize CRLF → LF so frontmatter stripping and heading/TOC
        // parsing behave identically regardless of the file's line endings.
        setRawContent((await res.text()).replace(/\r\n/g, "\n"));
      } catch {
        setRawContent(
  ...
  // Scroll to hash / searched heading / mapped section id when content changes
  useEffect(() => {
    if (!loading && rawContent) {
      const target = pendingScrollRef.current || window.location.hash.replace("#", "");
      pendingScrollRef.current = null;
      if (target) {
        const timer = setTimeout(() => scrollTo(target), 160);
        return () => clearTimeout(timer);
      }
    }
  }, [loading, rawContent, activeId]);

  const goToResult = (r: { id: string; heading: { slug: string } | null }) => {
    if (r.heading) pendingScrollRef.current = r.heading.slug;
    setGOpen(false);
    setGQuery("");
    setGActiveIdx(0);
    if (r.id === activeId && r.heading) scrollTo(r.heading.slug);
  ...
  return (
    ...
    >
      ...
      >
        ...
        {!isMobile && (
          <div style={{ position: "relative", flex: "0 1 440px", maxWidth: 440 }}>
            <IoSearch
              style={{
                position: "absolute", left: 12, top: "50%", transform: "translateY(-50%)",
                width: 15, height: 15, color: colors.textMuted, pointerEvents: "none",
              }}
            />
            <input
              type="text"
              ...
              onFocus={() => { void buildSearchIndex(); setGOpen(true); }}
              onBlur={() => setTimeout(() => setGOpen(false), 150)}
              onChange={(e) => { setGQuery(e.target.value); setGOpen(true); setGActiveIdx(0); }}
              onKeyDown={(e) => {
                if (e.key === "ArrowDown") { e.preventDefault(); setGActiveIdx((i) => Math.min(i + 1, gResults.length - 1)); }
                else if (e.key === "ArrowUp") { e.preventDefault(); setGActiveIdx((i) => Math.max(i - 1, 0)); }
                else if (e.key === "Enter") { const r = gResults[gActiveIdx]; if (r) goToResult(r); }
                else if (e.key === "Escape") { setGOpen(false); (e.target as HTMLInputElement).blur(); }
              }}
              style={{
                width: "100%", padding: "9px 12px 9px 36px", fontSize: 13.5,
                border: `1px solid ${colors.border}`, borderRadius: 10, outline: "none",
                background: colors.searchBg, color: colors.text, fontFamily: "inherit",
            ...
            {gOpen && gQuery.trim() && (
              <div
                ...
                style={{
                  ...
                  maxHeight: 380, overflowY: "auto", background: colors.cardBg,
                  border: `1px solid ${colors.border}`, borderRadius: 12,
                  boxShadow: "0 16px 44px rgba(15,23,42,0.18)", zIndex: 60, padding: 6,
                }}
              >
                {gResults.length ? (
                  gResults.map((r, i) => (
                    <button
                      key={`${r.id}-${r.heading?.slug || "top"}`}
                      onMouseDown={(e) => e.preventDefault()}
                      onClick={() => goToResult(r)}
                      onMouseEnter={() => setGActiveIdx(i)}
                      style={{
                        width: "100%", textAlign: "left", display: "block",
                        padding: "9px 12px", borderRadius: 8, border: "none", cursor: "pointer",
                        background: i === gActiveIdx ? colors.activeBg : "transparent",
                        fontFamily: "inherit",
                      }}
                    >
                      <div style={{ fontSize: 13.5, fontWeight: 600, color: colors.textHeading }}>
                        {r.title}
                      </div>
                      <div style={{ fontSize: 11.5, color: colors.textMuted, marginTop: 2 }}>
                        {r.category}{r.heading ? ` › ${r.heading.text}` : ""}
                      </div>
                    </button>
                  ))
                ) : (
                  <div style={{ padding: "14px 12px", fontSize: 13, color: colors.textMuted, textAlign: "center" }}>
                    {indexReady ? "No matches" : "Searching…"}
                  </div>
                )}
              </div>
            )}
          ...
          >
            {loading ? (
              ...
              <div className="doc-content">
                ...
                {/* WAS THIS HELPFUL */}
                <FeedbackWidget key={activeId} isDark={isDark} />
