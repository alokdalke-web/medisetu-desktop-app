...
import { FiInfo, FiMapPin, FiPhone, FiArrowLeft } from "react-icons/fi";
...
import { useCreatePharmacyMutation, useGetPharmaciesQuery } from "../../redux/api/pharmacyApi";
...
const PharmacyConfiguration: React.FC = () => {
  ...
  const { data: clinicRes, isLoading: isClinicLoading } = useGetAllClinicsQuery(undefined);
  const clinicProfile = (clinicRes as any)?.profile;
  const isPendingApproval =
    normalizeStatus(clinicProfile?.userStatus ?? user?.userStatus) === "pending";
  ...
  useEffect(() => {
    if (!isUserLoading && !isClinicLoading && !pharmacyLoading && primaryPharmacyId) {
      navigate(
        isPendingApproval
          ? "/users?type=Pharmacist&role=Pharmacist"
          : `/configuration/pharmacy/${primaryPharmacyId}`,
        { replace: true },
      );
    }
  }, [isClinicLoading, isPendingApproval, isUserLoading, pharmacyLoading, primaryPharmacyId, navigate]);
  ...
  const validate = () => {
    ...
    else if (!/^[6-9]\d{9}$/.test(contactNumber.trim())) e.contactNumber = "Must be Valid 10 digits";
    setErrors(e);
  ...
  if (isUserLoading || isClinicLoading || pharmacyLoading) {
