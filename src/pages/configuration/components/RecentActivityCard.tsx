...
const RecentActivityCard: React.FC<RecentActivityCardProps> = ({
  ...
  <div className="rounded-2xl border border-slate-200 bg-white p-5 dark:border-default-100 dark:bg-background sm:p-6">
    <div className="mb-4 flex items-center justify-between">
      <h3 className="text-[13px] font-semibold text-slate-700 dark:text-default-200">
        Recent Activity
      </h3>
      {onViewAll && (
        <button
    ...
    {activities.length === 0 ? (
      <div className="flex flex-col items-center justify-center py-8 text-center">
        <div className="mb-2 grid h-10 w-10 place-items-center rounded-full bg-slate-100 dark:bg-default-50">
          <FiActivity className="text-slate-400" />
        </div>
        <p className="text-[12px] font-medium text-slate-500">No recent activity</p>
          ...
          >
            ...
            <div className="min-w-0 flex-1">
              <div className="flex items-center justify-between gap-2">
                <span className="text-[12px] font-medium text-slate-800 dark:text-default-200 line-clamp-1">
                  {activity.title}
                </span>
                <span
              ...
              <span className="mt-1 block text-[10px] text-slate-300 dark:text-default-500">
                {activity.timestamp}
              </span>
            </div>
          </div>
