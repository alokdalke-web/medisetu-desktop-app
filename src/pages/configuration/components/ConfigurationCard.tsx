...
const ConfigurationCard: React.FC<ConfigurationCardProps> = ({ section }) => {
  ...
  return (
    <div className="flex flex-col rounded-2xl border border-slate-200 bg-white p-5 transition-colors hover:border-slate-300 dark:border-default-100 dark:bg-background dark:hover:border-default-200 sm:p-6">
      {/* Header row */}
        ...
        <div className="flex items-start gap-3.5">
          ...
          <div className="min-w-0 pt-0.5">
            <div className="flex flex-wrap items-center gap-2">
              <h3 className="text-[15px] font-bold text-slate-950 dark:text-white sm:text-[16px]">
                {section.title}
              </h3>
              <StatusBadge status={section.status} />
            </div>
            <p className="mt-1 text-[12px] leading-relaxed text-slate-500 dark:text-default-400 sm:text-[13px]">
              {section.description}
            </p>
          </div>
        </div>
      ...
      {/* Stats row */}
      <div className={`mt-5 grid grid-cols-2 gap-3 ${section.stats.length === 3 ? "sm:grid-cols-3" : "sm:grid-cols-4"}`}>
        {section.stats.map((stat) => (
          <div
            key={stat.label}
            className="flex items-center gap-2.5 rounded-lg bg-slate-50 px-3 py-2.5 dark:bg-default-50/30"
          >
