...
import { FiInfo, FiMail, FiMapPin, FiPhone, FiArrowLeft } from "react-icons/fi";
...
import { useCreateLabMutation, useGetLabsByClinicIdQuery } from "../../redux/api/labApi";
...
const LabConfiguration: React.FC = () => {
  ...
  useEffect(() => {
    if (!isUserLoading && !clinicLoading && !labsLoading && primaryLabId) {
      navigate(
        isPendingApproval
          ? "/users?type=Lab_Assistant&role=Lab_Assistant"
          : `/configuration/labs/${primaryLabId}`,
        { replace: true },
      );
    }
  }, [clinicLoading, isPendingApproval, isUserLoading, labsLoading, primaryLabId, navigate]);
  ...
  const validate = () => {
    ...
    else if (!/^[6-9]\d{9}$/.test(contactNo.trim())) e.contactNo = "Must be Valid 10 digits";
  ...
  if (isUserLoading || clinicLoading || labsLoading) {
