...
import { FiArrowLeft, FiFileText, FiPlus, FiSave, FiTrash2, FiUpload } from "react-icons/fi";
import {
...
import InvoiceScannerModal from "./component/InvoiceUploadModal";
...
const PharmacistAddStock: React.FC = () => {
  const navigate = useNavigate();
  const tomorrow = today(getLocalTimeZone()).add({ days: 1 });
  const [formData, setFormData] = useState<FormData>({
    supplierId: "",
    purchaseDate: today(getLocalTimeZone()),
    paymentStatus: "paid",
    paymentNotes: "",
    invoice: null,
    medicines: [
  ...
  const [supplierInputValue, setSupplierInputValue] =
    useState("");
  const [isSupplierDropdownOpen, setIsSupplierDropdownOpen] =
    useState(false);
  ...
  const [isMedicineDropdownOpen, setIsMedicineDropdownOpen] =
    useState<{
      [key: string]: boolean;
    }>({});
  ...
  const activeMedicineSearch = useMemo(() => {
    const openedRowId = Object.entries(
        isMedicineDropdownOpen
    ).find(([_, isOpen]) => isOpen)?.[0];
  ...
  const [addStock, { isLoading: isAddingStock }] =
    useAddStockMutation();
  ...
  const handlePaymentStatusChange = (
    ...
    const selectedKey = Array.from(
        keys as Iterable<React.Key>
    )[0];

    setFormData((prev) => ({
        ...prev,
        paymentStatus:
        selectedKey as StockPaymentStatus,
    }));
  ...
  const validateForm = (): boolean => {
    ...
    formData.medicines.forEach((medicine) => {
        ...
        medicineErrors.pharmacyMedicineId =
        ...
        if (
        Object.keys(medicineErrors).length > 0
        ) {
        newErrors.medicines[medicine.id] =
            medicineErrors;
        }
    });
    ...
    try {
      ...
      const stockPayload = {
        ...
        pharmacyStockPaymentStatus:
          formData.paymentStatus as StockPaymentStatus,
        paymentNotes:
          formData.paymentNotes || undefined,
        medicines: formData.medicines.map((med) => ({
          pharmacyMedicineId:
            med.pharmacyMedicineId,
          // batch: med.batch,
          ...(med.batch.trim() && {
            batch: med.batch.trim(),
          }),
          expiry: formatDateToYYYYMMDD(
            med.expiry
            ),
      ...
      const stockId = stockResult?.data?.stock?.id;
    ...
    const handleKeyDown = (e: KeyboardEvent) => {
      ...
      if (e.key.toLowerCase() === "s") {
        setSPressCount((prev) => {
          ...
          if (next >= 3) {
            if (isAddingStock || isUploadingInvoice) {
              return 0;
            }
            setSPressCount(0);
            handleOpenSaveModal();
  ...
  }, [formData, isAddingStock, isUploadingInvoice]);

  /* ======================== COMPUTED ======================== */

  const grandTotal = useMemo(() => {
    return formData.medicines.reduce(
      (total, med) => {
        return (
          total +
          calculateTotalCost(
  ...
  const isLoading =
    isAddingStock || isUploadingInvoice;
  ...
  const handleTabNavigation = (
    ...
    const selectors = [
        `[data-row="${rowIndex}"][data-field="batch"] input`,
        `[data-row="${rowIndex}"][data-field="expiry"] button`,
        `[data-row="${rowIndex}"][data-field="quantity"] input`,
        `[data-row="${rowIndex}"][data-field="mrp"] input`,
        `[data-row="${rowIndex}"][data-field="cost"] input`,
    ];

    if (
        !e.shiftKey &&
        fieldOrder < selectors.length - 1
    ) {
        e.preventDefault();

        setTimeout(() => {
        const nextElement = document.querySelector(
            selectors[fieldOrder + 1]
        ) as HTMLElement | null;
  ...
  return (
    <div className="w-full space-y-6">
      <PageBackNav
        backTo="/pharmacy/stock"
        crumbs={[{ label: "Stock", to: "/pharmacy/stock" }, { label: "Add Stock" }]}
        className="mb-5"
      />
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            <div className="space-y-2">
              ...
              <I18nProvider locale="en-IN">
                <DatePicker
                  ...
                  classNames={{
                    inputWrapper:
                      "h-10 min-h-10 rounded-lg border border-slate-200 bg-white shadow-sm dark:border-[#273244] dark:bg-[#111726]",
                  }}
                />
              </I18nProvider>
            </div>
            ...
            <div className="space-y-2">
                ...
                <div className="relative flex-1">
                  ...
                  >
                    ...
                    <span className="text-sm truncate">
                      {invoiceFileName || "Upload Invoice"}
                    </span>
                  </button>
                </div>
              </div>
              ...
              <p className="text-xs text-slate-500">
                Click to upload invoice (PDF, JPG, PNG - Max 5MB)
              </p>
            </div>

            {/* Show invoice preview if uploaded */}
            {formData.invoice && (
              <div className="mt-2 p-3 bg-slate-50 rounded-lg border border-slate-200 col-span-3">
                    ...
                    return (
                      ...
                      >
                        ...
                        <TableCell className="min-w-[220px]">
                          <Autocomplete
                            placeholder="Search medicine by sku or name"
                            maxLength={50}
                            inputProps={{
                              classNames: {
                        ...
                        >
                          <Input
                            ...
                            isInvalid={
                                !!errors.medicines[medicine.id]?.batch
                            }
                            errorMessage={
                                errors.medicines[medicine.id]?.batch
                            }
                            ...
                            classNames={{
                              inputWrapper:
                                "h-9 min-h-9 rounded-lg border border-slate-200 bg-white shadow-sm dark:border-[#273244] dark:bg-[#111726]",
                              input: "text-[13px] text-slate-700 dark:text-white",
                            }}
                          />
                        </TableCell>
                        ...
                        >
                          <I18nProvider locale="en-IN">
                            <DatePicker
                              ...
                              isInvalid={
                                  !!errors.medicines[medicine.id]?.expiry
                              }
                              errorMessage={
                                  errors.medicines[medicine.id]?.expiry
                              }
                        ...
                        >
                          <Input
                            ...
                            isInvalid={
                                !!errors.medicines[medicine.id]?.quantity
                            }
                            errorMessage={
                                errors.medicines[medicine.id]?.quantity
                            }
                        ...
                        >
                          <Input
                            ...
                            isInvalid={
                                !!errors.medicines[medicine.id]?.mrp
                            }
                            errorMessage={
                                errors.medicines[medicine.id]?.mrp
                            }
                        ...
                        >
                          <Input
                            ...
                            isInvalid={
                                !!errors.medicines[medicine.id]?.cost
                            }
                            errorMessage={
                                errors.medicines[medicine.id]?.cost
                            }
...
export default PharmacistAddStock;
