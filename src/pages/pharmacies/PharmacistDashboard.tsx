...
import { DateTab } from "../dashboard/DateFilterTabs";
import {
...
import { addToast, Select, SelectItem, useDisclosure } from "@heroui/react";
import MedicineFormModal from "./component/MedicineFormModal";
import PharmacistPendingDashboard from "./PharmacistPendingDashboard";
          ...
          <div className="flex rounded-lg overflow-hidden border border-slate-200 dark:border-slate-600">
            {tabs.map((tab) => (
              <button
                ...
                className={`cursor-pointer px-3 py-1 text-[11px] font-medium transition-all ${
                  activeTab === tab
...
const Sk = ({ className = "" }: { className?: string }) => (
  <div className={`animate-pulse rounded-lg bg-slate-200/80 dark:bg-[#172033] ${className}`} />
);

const PharmacyDashboardSkeleton: React.FC = () => {
  const statWidths = ["w-28", "w-36", "w-44", "w-32"];
  const statValueWidths = ["w-24", "w-12", "w-28", "w-24"];
  ...
  return (
    <div
      ...
      aria-busy="true"
      aria-label="Loading pharmacy dashboard"
    >
      <div className="flex flex-col gap-3 sm:gap-4 xl:flex-row xl:items-start xl:justify-between">
        <div className="flex shrink-0 flex-col gap-2">
          <Sk className="h-7 w-64 max-w-full sm:h-8 sm:w-80" />
      ...
      <div className="grid gap-2 sm:gap-3 md:grid-cols-2 lg:grid-cols-4 lg:gap-4">
        {statWidths.map((width, index) => (
          <div
            key={width}
            className="flex min-h-[104px] min-w-0 flex-1 items-stretch justify-between gap-2 rounded-[6px] border border-[rgba(229,231,234,0.6)] bg-white px-3 py-3 shadow-[0_1px_1px_rgba(0,0,0,0.05)] dark:border-[#273244] dark:bg-[#111726] dark:shadow-none sm:gap-3 sm:px-4"
          >
            <div className="flex min-w-0 flex-1 items-start gap-2.5 sm:gap-3">
              <Sk className="h-9 w-9 shrink-0" />
              <div className="flex min-w-0 flex-1 flex-col">
                <Sk className={`h-4 ${width}`} />
                <Sk className={`mt-3 h-7 ${statValueWidths[index]}`} />
                <Sk className="mt-3 h-3 w-32" />
              </div>
            </div>
            <Sk className="hidden h-8 w-20 shrink-0 md:flex lg:hidden 2xl:flex" />
          </div>
        ))}
      </div>

      <div className="grid grid-cols-1 gap-3 sm:gap-4 lg:grid-cols-12">
        <div className="rounded-2xl border border-[rgba(229,231,234,0.6)] bg-white px-5 py-4 shadow-[0_1px_1px_rgba(0,0,0,0.05)] dark:border-[#273244] dark:bg-[#111726] dark:shadow-none lg:col-span-5">
          ...
          <Sk className="mt-4 h-7 w-36" />
          <Sk className="mt-4 h-[150px] w-full rounded-xl" />
          <div className="mt-4 flex gap-4 border-t border-[#e5e7ea] pt-3 dark:border-[#273244]">
        ...
        <div className="rounded-2xl border border-[rgba(229,231,234,0.6)] bg-white p-5 shadow-[0_1px_1px_rgba(0,0,0,0.05)] dark:border-[#273244] dark:bg-[#111726] dark:shadow-none lg:col-span-4">
          <Sk className="h-5 w-36" />
          <div className="mt-5 flex items-center gap-4">
            <Sk className="h-[130px] w-[130px] shrink-0 rounded-full" />
            <div className="flex-1 space-y-3">
              {[0, 1, 2].map((item) => (
                <Sk key={item} className="h-4 w-full" />
              ))}
            </div>
          </div>
          <div className="mt-4 grid grid-cols-3 gap-2">
            {[0, 1, 2].map((item) => (
              <Sk key={item} className="h-16 rounded-xl" />
            ))}
          </div>
        </div>

        <div className="rounded-2xl border border-[rgba(229,231,234,0.6)] bg-white px-4 py-4 shadow-[0_1px_1px_rgba(0,0,0,0.05)] dark:border-[#273244] dark:bg-[#111726] dark:shadow-none md:col-span-2 lg:col-span-3">
          <div className="flex items-center justify-between">
          ...
          <div className="mt-4 space-y-2">
            {[0, 1, 2, 3].map((item) => (
              <div key={item} className="flex items-center gap-2.5 rounded-xl border border-[rgba(229,231,234,0.6)] p-3 dark:border-[#273244]">
      ...
      <div className="grid grid-cols-1 gap-3 sm:gap-4 md:grid-cols-2 xl:grid-cols-3">
        {[0, 1, 2].map((card) => (
          <div
            key={card}
            className="rounded-2xl border border-[rgba(229,231,234,0.6)] bg-white px-5 py-4 shadow-[0_1px_1px_rgba(0,0,0,0.05)] dark:border-[#273244] dark:bg-[#111726] dark:shadow-none"
          >
            <div className="flex items-center justify-between">
              <Sk className="h-5 w-36" />
              <Sk className="h-8 w-28" />
        ...
        {[0, 1, 2].map((card) => (
          ...
          >
            ...
            <div className="mt-4 space-y-3">
              ...
              <Sk className="h-10 w-10/12 rounded-xl" />
              <Sk className="h-10 w-full rounded-xl" />
            </div>
          </div>
        ))}
...
const PharmacistDashboard: React.FC = () => {
  ...
  if (isPending) {
    return <PharmacistPendingDashboard />;
  }

  const fullGreeting = getFullGreeting(user);
  const {
    isOpen: isMedicineModalOpen,
    onOpen: onOpenMedicineModal,
  ...
  // ─── Date Filter State ───
  const [startDate, _setStartDate] = useState(() => toYMD(new Date()));
  const [endDate, _setEndDate] = useState(() => toYMD(new Date()));
  const [_activeTab, _setActiveTab] = useState<DateTab>("today");
  ...
  const handleAddMedicine = async (
    ...
    try {
      await addMedicine(data as AddMedicineRequest).unwrap();
      addToast({
        title: "Success",
      ...
      <div className="flex flex-col gap-3 sm:gap-4 xl:flex-row xl:items-start xl:justify-between">
        ...
        <div className="flex w-full flex-col gap-3 sm:flex-row sm:items-center sm:justify-end xl:w-auto">
          <button
            ...
            className="cursor-pointer flex w-full items-center justify-center gap-2 rounded-xl bg-[#0a6c74] px-4 py-2.5 text-[14px] font-medium text-white transition hover:bg-[#085a61] sm:w-auto"
          >
            <FiPlus className="h-4 w-4" /> Add Medicine
          </button>
          <button
            type="button"
            onClick={() => navigate("/pharmacy/sales/new")}
            className="cursor-pointer flex w-full items-center justify-center gap-2 rounded-xl border border-[rgba(207,207,207,0.5)] bg-white px-4 py-2.5 text-[14px] font-medium text-[#100e1c] transition hover:bg-slate-50 dark:border-[#273244] dark:bg-[#111726] dark:text-white dark:hover:bg-[#151e31] sm:w-auto"
          >
            <FiShoppingCart className="h-4 w-4" /> New Sale
          </button>
        </div>
      </div>
