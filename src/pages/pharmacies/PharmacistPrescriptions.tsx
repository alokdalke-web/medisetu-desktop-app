...
const PharmacistPrescriptions: React.FC = () => {
  ...
  const rowStatusMenuRef = useRef<HTMLDivElement | null>(null);
  const [openRowStatusMenu, setOpenRowStatusMenu] = useState<{
    id: string;
    left: number;
    top: number;
    width: number;
  ...
  useEffect(() => {
    const handleClickOutside = (event: MouseEvent) => {
      if (
        rowStatusMenuRef.current &&
        !rowStatusMenuRef.current.contains(event.target as Node)
      ) {
        setOpenRowStatusMenu(null);
      }
    };

    const handleClose = () => setOpenRowStatusMenu(null);
      ...
      <div className="grid grid-cols-2 gap-2 sm:grid-cols-2 md:gap-3 xl:grid-cols-4">
        {stats.map((stat, index) => (
          <PharmacyStatCard
            ...
            detail={`${stat.percentageOfTotal}% of total`}
            icon={stat.icon}
            iconBg={stat.iconBg}
                      ...
                      <td className="px-5 py-4">
                        {item.status === "COMPLETED" || item.status === "REJECTED" ? (
                          ...
                          <div className="relative">
                            <button
                              type="button"
                              onClick={(event) => handleRowStatusToggle(event, item.id)}
                              className={[
                            ...
                            {openRowStatusMenu?.id === item.id && (
                              <div
                                ref={rowStatusMenuRef}
                                className="fixed z-[9999] max-h-72 overflow-y-auto rounded-xl border border-slate-200 bg-white p-1.5 shadow-xl shadow-slate-200/70 dark:border-[#273244] dark:bg-[#111726] dark:shadow-black/30"
                                style={{
                                  left: openRowStatusMenu.left,
                                  top: openRowStatusMenu.top,
                                  minWidth: openRowStatusMenu.width,
                                }}
                              >
                                {[
