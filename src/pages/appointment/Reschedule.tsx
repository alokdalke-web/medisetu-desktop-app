...
import CustomSlotModal from "./new-appointment/components/CustomSlotModal";
import {
  formatDurationLabel,
  addMinutesToTime,
  formatIsoForUi,
...
import { formatTimeTo12Hour } from "./new-appointment/helpers/appointmentSummaryHelpers";
import type {
  Slot as SharedSlot,
  TimeSlot as SharedTimeSlot,
  TokenSlot as SharedTokenSlot,
  DayRange as SharedDayRange,
} from "./new-appointment/types";

// Reschedule-specific helpers/types — deliberately NOT merged into the
// New Appointment ones above; see src/types/appointment/reschedule.ts
import {
  fmtTime,
  buildAppointmentDateTimeIso,
  normalizeSlotsFromApi,
  groupSlotsIntoMultipleShifts,
  COMMON_RESCHEDULE_REASONS,
  limitReasonText,
  isoFromYMD,
  startOfLocalDay,
  addDays,
  ...
  normalizeSymptoms,
  isRescheduleState,
} from "./helpers/rescheduleHelpers";
import type {
  RescheduleFormValues as FormValues,
  RescheduleDayRange as DayRange,
  Slot,
  TimeSlot,
  TokenSlot,
...
const Reschedule: React.FC = () => {
  ...
  const [customSlot, setCustomSlot] = useState<Slot | null>(null);
  ...
  // Feeds the custom-slot modal's out-of-hours warning: one window per shift.
  const workingWindows = useMemo(
    () =>
      groupSlotsIntoMultipleShifts(slots)
        .shifts.map((shift) => ({
          start: shift[0]?.startTime ?? "",
          end: shift[shift.length - 1]?.endTime ?? "",
        }))
  ...
  const tokenSlots = useMemo(
    () => slots.filter((s): s is TokenSlot => s.kind === "token"),
    [slots],
  );

  const defaultCustomDurationMinutes = useMemo(() => {
  ...
  // Manually-entered time/token (same pattern as NewAppointment.tsx's handleCustomTimeSlot/
  // handleCustomToken). `customSlot` only holds a pick with no match in `slots` — if it
  // matches an existing slot/token, that real one is selected instead of duplicating it.
    ...
    if (existing) {
      if (existing.status !== "available") {
        addToast({
          ...
          description: `The ${formatTimeTo12Hour(time24)} slot is already ${existing.status}. Please choose another time.`,
          color: "warning",
          variant: "flat",
        });
        return;
    ...
    if (existing) {
      if (existing.status !== "available") {
        addToast({
          ...
          description: `Token ${pad2(tokenNo)} is already ${existing.status}. Please choose another token.`,
          color: "warning",
          variant: "flat",
        });
        return;
  ...
  return (
    <div className="p-0">
      {/* Page title + back nav */}
      <div className="mb-6">
        <h1 className="text-[22px] sm:text-[26px] font-semibold text-text">
          Reschedule Appointment
        </h1>

        <PageBackNav
          backTo={id ? `/appointment/${id}` : "/appointment"}
          crumbs={[
            { label: "Appointment", to: "/appointment" },
            id
              ? { label: "Appointment Details", to: `/appointment/${id}` }
      ...
      {/* Loading / Error */}
      {isFetching && (
        <div className="mb-3 text-sm text-text-muted">Loading appointment…</div>
      )}
      {isError && (
      ...
      <div className="grid gap-5 lg:grid-cols-[minmax(0,1fr)_280px] xl:grid-cols-[minmax(0,1fr)_320px]">
        ...
        <form
          onSubmit={onSubmit}
          className="rounded-2xl border border-line bg-surface p-4 shadow-sm dark:border-[#273244] dark:bg-[#111726] dark:shadow-none"
        >
        {/* Top cards */}
        <div className="grid items-stretch gap-3 lg:grid-cols-2 xl:grid-cols-12 mb-4">
          ...
          <section
            className={[
              "flex h-full flex-col rounded-xl border border-line bg-surface px-4 py-3 dark:border-[#273244] dark:bg-[#0f1728]",
              hasDoctorDetails
            ...
            <div className="flex items-center justify-between gap-2">
              <div className="flex items-center gap-2.5">
                ...
                <div className="min-w-0">
                  <p className="truncate text-[14px] font-bold text-text">{patientName || "—"}</p>
                </div>
              </div>
              {appointmentStatus ? (
            ...
            <div className="mt-2.5 grid grid-cols-3 gap-x-3 gap-y-1.5 text-[11px]">
              {currentAppointmentRows.map((row) => (
                <div key={row.label} className="min-w-0">
                  ...
                  <p className="mt-0.5 break-words text-[11px] font-semibold text-text">{row.value}</p>
                </div>
              ))}
            </div>
          ...
          {hasDoctorDetails && (
            <section className="rounded-xl border border-line bg-surface px-4 py-3 lg:col-span-1 xl:col-span-6 dark:border-[#273244] dark:bg-[#0f1728]">
              <div className="flex min-w-0 items-center gap-2.5">
                ...
                <div className="min-w-0">
                  {doctorName && <p className="truncate text-[13px] font-bold text-text">Dr. {doctorName}</p>}
                  ...
                  {doctorInfoRows.slice(0, 6).map((row) => (
                    <div key={row.label} className="min-w-0 flex items-center gap-1.5 rounded-md border border-slate-100 bg-slate-50/50 px-2 py-1.5 dark:border-[#273244] dark:bg-[#111726]">
                      ...
                      <div className="min-w-0">
                        ...
                        <p className="truncate text-[11px] font-semibold text-text">{row.value}</p>
                      </div>
                    </div>
                  ))}
                </div>
          ...
          onOpenCustomSlot={() => {
            if (!dateParam || !doctorId) {
              addToast({
                ...
                description: "Please choose a date before adding a custom slot.",
                color: "warning",
                variant: "flat",
              });
              return;
        ...
        >
          <AppointmentSlotSection
            ...
            customSlot={customSlot as SharedSlot | null}
        ...
        {/* Reason */}
        <section className="mt-3 rounded-xl border border-line bg-surface px-3 py-2.5 sm:px-4 sm:py-3 dark:border-[#273244] dark:bg-[#0f1728]">
          {/* Header row with label + clear */}
          <div className="flex items-center justify-between mb-2">
            <p className="text-[13px] font-semibold text-text">
              Reason <span className="text-[11px] font-normal text-text-muted">(optional)</span>
            </p>
            {reasonValue && (
              <button
                type="button"
                onClick={() => setValue("reason", "", { shouldDirty: true, shouldValidate: false })}
                className="text-[11px] font-semibold text-slate-500 hover:text-text dark:hover:text-slate-200"
              >
                Clear
              </button>
            )}
        ...
        <div className="fixed bottom-0 left-0 right-0 z-50 border-t border-line bg-surface px-4 py-3 shadow-[0_-2px_10px_rgba(0,0,0,0.06)] lg:hidden dark:border-[#273244] dark:bg-[#111726] dark:shadow-[0_-2px_10px_rgba(0,0,0,0.3)]">
          <button
            type="button"
            onClick={onSubmit}
        ...
        <aside className="hidden lg:flex flex-col rounded-2xl border border-line bg-surface shadow-sm dark:border-[#273244] dark:bg-[#111726] dark:shadow-none sticky top-6 self-start">
          {/* Header */}
          ...
          <div className="divide-y divide-line">
            ...
            <div className="px-4 py-3 flex items-center gap-3">
              ...
              <div className="min-w-0 flex-1">
                <p className="truncate text-[13px] font-bold text-text">{patientName || "—"}</p>
            ...
            <div className="px-4 py-3 grid grid-cols-2 gap-x-3 gap-y-2">
              <div className="min-w-0">
                <p className="text-[10px] font-semibold uppercase tracking-wide text-text-muted">Doctor</p>
                <p className="mt-0.5 truncate text-[12px] font-semibold text-text">{doctorName ? `Dr. ${doctorName}` : "—"}</p>
              </div>
              <div className="min-w-0">
              ...
              {patientMobile && (
                <div className="min-w-0">
                  <p className="text-[10px] font-semibold uppercase tracking-wide text-text-muted">Contact</p>
                  <p className="mt-0.5 truncate text-[12px] font-semibold text-text">{patientMobile}</p>
                </div>
              )}
              {doctorSpeciality && (
                <div className="min-w-0">
                  <p className="text-[10px] font-semibold uppercase tracking-wide text-text-muted">Speciality</p>
                  <p className="mt-0.5 truncate text-[12px] font-semibold text-text">{doctorSpeciality}</p>
                </div>
              )}
            </div>
            ...
            {hasSymptoms && (
              <div className="px-4 py-2.5">
                <p className="text-[10px] font-semibold uppercase tracking-wide text-text-muted">Symptoms</p>
            ...
            <div className="px-4 py-3 space-y-2">
              ...
              <div className="flex items-start gap-2">
                ...
                <div className="min-w-0">
                  <p className="text-[10px] font-semibold uppercase tracking-wide text-text-muted">Current</p>
                  <p className="text-[13px] font-semibold text-text-muted line-through decoration-slate-300 dark:decoration-slate-500">
                  ...
                  {dateParam ? (
                    <p className="text-[13px] font-bold text-text">
                      ...
                      {selectedSlot ? (
                        ...
                        <span className="ml-1 text-text-muted">· Select slot</span>
                      )}
                    </p>
                  ) : (
                    <p className="text-[13px] text-text-muted">Select date & slot</p>
                  )}
                </div>
              </div>
            </div>
            ...
            {reasonValue && (
              <div className="px-4 py-2.5">
                <p className="text-[10px] font-semibold uppercase tracking-wide text-text-muted">Reason</p>
                <p className="mt-0.5 text-[12px] text-text-muted line-clamp-2">{reasonValue}</p>
              </div>
            )}
          </div>
