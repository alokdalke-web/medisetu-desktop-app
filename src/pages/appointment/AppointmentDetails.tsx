...
import FutureDateActionModal from "./components/FutureDateActionModal";
...
import AppointmentSummaryCard, { getServiceName } from "./components/AppointmentSummaryCard";
import AppointmentVitalsSection from "./components/AppointmentVitalsSection";
import ConsultationPatientBar from "./components/ConsultationPatientBar";
...
import {
  ...
  clearAppointmentPrescriptionDraft,
  getAppointmentPrescriptionDraft,
  hasPrescriptionDetailsDraftData,
  saveAppointmentPrescriptionDraft,
...
import { getServiceExpiryIso } from "./new-appointment/helpers/dateTimeHelpers";
...
const AppointmentDetails: React.FC = () => {
  ...
  /**
   * Consultation mode: while the doctor is on the prescription tab the three
   * summary cards are replaced by a one-line patient strip. "View Full Details"
   * opts back into the full cards, and leaving the tab resets that — so the
   * full summary is always what a doctor sees when they arrive.
   */
  ...
  useEffect(() => {
    if (activeTab !== "prescription") setShowFullPatientDetails(false);
  }, [activeTab]);
  ...
  const draftLoadedAppointmentIdRef = useRef<string | null>(null);
  const localPrescriptionDraftAppliedRef = useRef(false);
  const [draftHydratedAppointmentId, setDraftHydratedAppointmentId] = useState<
    string | null
  >(null);
  const [canUseDraftStorage, setCanUseDraftStorage] = useState(() =>
    canUseAppointmentPrescriptionDrafts(),
  );
  ...
  const handlePrescriptionCompletionStateChange = ({
    ...
    setActiveTab((prev) => (prev === "prescription" ? prev : "prescription"));
    ...
    if (!isProcessing && isSuccess) {
      clearAppointmentPrescriptionDraft(id);
      refetchAppointment();
      refetchReports();
    }
  };
  ...
  // Only used to prefill the prescription form's test list, so defer until
  // the prescription tab is opened.
  const { data: appointmentTestsRaw } =
    useGetAppointmentTestsByAppointmentIdQuery(id, {
      skip: !id || activeTab !== "prescription",
    });
  ...
  const isAlreadyPaidPrimaryForTotal =
    String((a as any)?.paymentStatus ?? "").trim().toLowerCase() === "already paid" ||
    String((a as any)?.paymentMode ?? "").trim().toLowerCase() === "already paid";

  const appointmentPriceRaw =
    (a as any)?.primaryServicePrice ??
    apiResult?.appointment?.primaryServicePrice ??
    (a as any)?.price ??
    apiResult?.appointment?.price ??
    null;

  const priceValue = Number(appointmentPriceRaw ?? clinicService?.price ?? 0);

  const primaryServicePriceValue = isAlreadyPaidPrimaryForTotal ? 0 : priceValue;
  const primaryServicePriceText =
    !isAlreadyPaidPrimaryForTotal && Number.isFinite(priceValue) && priceValue > 0
  ...
  // Total shown to the user must be primary service (if unpaid/not already paid) + all add-on services
  const combinedTotalValue =
    (isAlreadyPaidPrimaryForTotal ? 0 : (Number.isFinite(priceValue) ? priceValue : 0)) +
    (Number.isFinite(additionalServicesTotal) ? additionalServicesTotal : 0);
  const combinedTotalText =
  ...
  const handleOpenInvoice = () => {
    ...
    const isAlreadyPaidPrimary =
      String(a?.paymentStatus ?? "").trim().toLowerCase() === "already paid" ||
      String(a?.paymentMode ?? "").trim().toLowerCase() === "already paid" ||
      String(appointment?.paymentMode ?? "").trim().toLowerCase() === "already paid";

    if (
      !isAlreadyPaidPrimary &&
      (paymentMode === null ||
  ...
  const isPaid = isAlreadyPaidPrimary
    ? additionalServices.length > 0
    : Boolean(appointment.paymentMode) || additionalServices.length > 0;
  ...
  // ── Next Patient ──────────────────────────────────────────────────────────
  // "Next" is defined relative to this doctor's queue for this appointment's
  // day: same chronological ordering (time, then tokenNo) the clinic list and
  // appointment-engine already use, walking forward from the current
  // appointment to the first still-active (non-terminal) one.
  const appointmentDateYMD = useMemo(() => {
    const raw = a?.appointmentDate as string | undefined;
    if (!raw) return "";
  ...
  /** Whole days from today to the appointment day; 0 for today, negative for past. */
  const daysUntilAppointment = useMemo(() => {
    if (!appointmentDateYMD) return 0;
    const [y, m, d] = appointmentDateYMD.split("-").map(Number);
    if (!y || !m || !d) return 0;
    const now = new Date();
  ...
  /** True when this appointment is booked for a day after today (clinic-local). */
  const isFutureDayAppointment = daysUntilAppointment > 0;

  /** Appointment day in display form, used by the future-date confirmation popup. */
  const formattedAppointmentDate = useMemo(() => {
    const raw = a?.appointmentDate as string | undefined;
    if (!raw) return "a later date";
    const d = new Date(raw);
    if (Number.isNaN(d.getTime())) return "a later date";
  ...
  /** Which future-day action is awaiting the extra confirmation popup. */
  const [pendingFutureAction, setPendingFutureAction] = useState<
    "confirm" | "arrived" | null
  >(null);

  const { data: doctorDayAppointments } = useGetClinicAppointmentsQuery(
    {
      doctorId: appointmentDoctorId,
      startDate: appointmentDateYMD,
      endDate: appointmentDateYMD,
      pageSize: 200,
    },
    { skip: !appointmentDoctorId || !appointmentDateYMD },
  );

  const nextPatient = useMemo(() => {
    const list = (doctorDayAppointments as any)?.result?.patients ?? [];
    ...
    const normalized = list
      .map((p: any) => ({
        ...
        patientName: p?.name,
        time: p?.appointment?.appointmentTime ?? null,
        tokenNo: p?.appointment?.tokenNo ?? null,
        status: p?.appointment?.appointmentStatus,
      }))
      .filter((x: any) => x.appointmentId);

    normalized.sort((x: any, y: any) => {
      const diff = toMinutes(x.time) - toMinutes(y.time);
      if (diff !== 0) return diff;
      return (x.tokenNo ?? 0) - (y.tokenNo ?? 0);
    });
    ...
    for (let i = currentIndex + 1; i < normalized.length; i++) {
      if (ACTIVE_STATUSES.has(normalized[i].status)) {
        return normalized[i];
      }
    }
    return null;
  }, [doctorDayAppointments, id]);

  const handleGoToNextPatient = React.useCallback(() => {
    if (!nextPatient?.appointmentId) return;
    navigate(`/appointment/${nextPatient.appointmentId}`);
  }, [nextPatient, navigate]);
  ...
  // Only consumed to seed the prescription form's follow-up-days default, so
  // there's no reason to fetch it until the prescription tab is actually open.
  const { data: doctorPreferencesData } = useGetDoctorPreferencesQuery(
    appointmentDoctorId,
    {
      skip: !appointmentDoctorId || activeTab !== "prescription",
  ...
  useEffect(() => {
    const syncDraftStorageConsent = () => {
      setCanUseDraftStorage(canUseAppointmentPrescriptionDrafts());
    };

    window.addEventListener(
      "cookie-consent-updated",
      syncDraftStorageConsent,
    );
  ...
  useEffect(() => {
    if (!appointment.id || !data || apptLoading) return;

    if (!canUseDraftStorage) {
      clearAppointmentPrescriptionDraft(String(appointment.id));
      return;
    }

    if (isCompletedStatus || isCancelledStatus || isNoShowStatus) {
      clearAppointmentPrescriptionDraft(String(appointment.id));
      return;
    }

    const appointmentId = String(appointment.id);
    if (draftLoadedAppointmentIdRef.current === appointmentId) return;
    draftLoadedAppointmentIdRef.current = appointmentId;
    setDraftHydratedAppointmentId(null);
    localPrescriptionDraftAppliedRef.current = false;
    setReportApplied(false);

    const draft = getAppointmentPrescriptionDraft(appointmentId);
    if (!draft) {
      setRxMeds([]);
      setRxDetails(emptyPrescriptionDetails);
      setRxTests([]);
      setDraftHydratedAppointmentId(appointmentId);
      return;
    }

    setRxMeds(draft.meds);
    setRxDetails(draft.details);
    setRxTests(draft.tests);

    if (
      draft.meds.length > 0 ||
      hasPrescriptionDetailsDraftData(draft.details, { includeVitals: false })
    ) {
      localPrescriptionDraftAppliedRef.current = true;
  ...
  useEffect(() => {
    ...
    if (draftHydratedAppointmentId !== appointmentId) return;
  ...
  useEffect(() => {
    if (!reportsData || reportApplied || localPrescriptionDraftAppliedRef.current) {
      return;
    }

    const res: any = (reportsData as any).result ?? reportsData;
    const rc = res.reportCard ?? null;
    const list: any[] = Array.isArray(res.prescriptions)
      ? res.prescriptions
      : [];
  ...
  const handlePickerDone = (
    ...
    if (
      appointment.id &&
      draftHydratedAppointmentId === String(appointment.id) &&
      canUseDraftStorage &&
      !isCompletedStatus &&
      !isCancelledStatus &&
      !isNoShowStatus
  ...
  const [saveMedicalCertificate, { isLoading: isSavingCertificate }] =
    useSaveMedicalCertificateMutation();
  // Certificate data is only needed once the user opens the certificate
  // modal — fetching it on every appointment page load was wasted work for
  // the common case where nobody ever opens that modal. Once opened, the
  // hasOpenedCertificateModal flag keeps it fetched even after the modal is
  // closed again, so re-opening reuses the cached result instead of skip
  // toggling back and forth.
  const [hasOpenedCertificateModal, setHasOpenedCertificateModal] =
    useState(false);
  useEffect(() => {
    if (isMedicalCertificateModalOpen) setHasOpenedCertificateModal(true);
  ...
  } = useGetMedicalCertificateQuery(appointment.id, {
    skip: !appointment.id || !hasOpenedCertificateModal,
  });
  ...
  const handleClearPrescription = () => {
    ...
    setRxDetails(emptyPrescriptionDetails);
    setReportApplied(false);
    if (appointment.id) clearAppointmentPrescriptionDraft(String(appointment.id));
  };
  ...
  // Confirming or checking in an appointment booked for a later day is almost
  // always a misclick on the front desk / doctor queue, so it goes through an
  // extra confirmation step. Same-day (and past-day) appointments act
  // immediately, as before.
  const handleConfirm = () => {
    if (isFutureDayAppointment) {
      setPendingFutureAction("confirm");
      return;
    }
    void runConfirm();
  };
  ...
  const handlePatientArrived = () => {
    if (isFutureDayAppointment) {
      setPendingFutureAction("arrived");
      return;
    }
    void runPatientArrived();
  };

  const handleFutureActionConfirmed = () => {
    const action = pendingFutureAction;
    setPendingFutureAction(null);
    if (action === "confirm") void runConfirm();
    else if (action === "arrived") void runPatientArrived();
  };
  ...
  // A failed fetch (403 access-denied or otherwise) previously fell through to
  // render the entire page below with empty/placeholder data next to a thin
  // red text line — confusing rather than informative. Block the page here
  // instead, once, rather than every card individually rendering blank.
  if (isError && !apptLoading) {
    const status = (error as any)?.status;
    const backendMessage =
      (error as any)?.data?.message || (error as any)?.error;
    // Prefer the real status code, but some older endpoints still throw a
    // plain Error for permission checks (defaulting to 500 instead of 403)
    // rather than HttpError — fall back to the message text so this still
    // shows the right UI even against those, instead of a generic failure.
    const isAccessDenied =
      status === 403 ||
      /permission|access.?denied|not authorized|not assigned to this clinic/i.test(
        String(backendMessage || "")
      );

    const isNotFound = !isAccessDenied && status === 404;
    // RTK Query reports a dead network/offline request with no HTTP status.
    const isOffline =
      !isAccessDenied && (status === "FETCH_ERROR" || status === undefined);

    /* The backend's message is written for developers — a sanitized 500 arrives
       as literally "Database error", which a receptionist can neither act on
       nor report usefully. Show an explanation of what to DO instead, and keep
       the raw text below as a support reference rather than as the headline.
       403/404 messages are user-facing by design, so those still pass through. */
    ...
    const message = isAccessDenied
      ? backendMessage ||
        "You don't have permission to view this appointment. It may belong to another clinic."
      : isNotFound
        ? backendMessage ||
          "This appointment may have been deleted, or the link is no longer valid."
        : isOffline
          ? "Check your internet connection, then try again."
          : "Something went wrong on our end while loading this appointment. Your data is safe — please try again in a moment.";

    /* Only surfaced for genuine server faults, where the headline above is
       intentionally vague and support needs something concrete to go on. */
    const technicalDetail =
      !isAccessDenied && !isNotFound && !isOffline && backendMessage
        ? `${backendMessage}${status ? ` (${status})` : ""}`
        : "";
        ...
        <div className="mx-auto w-full max-w-[1800px]">
          ...
          <div className="flex min-h-[400px] items-center justify-center px-4">
            <AccessDeniedState
              title={title}
              message={message}
              /* The shield icon reads as "blocked/forbidden", which misdescribes
                 a server fault or a dropped connection — match the icon to the
                 actual failure so the state is legible before reading a word. */
              icon={
                isAccessDenied ? undefined : isOffline ? (
                  <FiWifiOff className="h-7 w-7 text-danger" />
                ) : (
              ...
              action={
                <div className="flex flex-col items-center gap-3">
                  {/* Retry leads, since for a transient 500 it's the action that
                      most often works; stacks on narrow screens. */}
                  <div className="flex w-full flex-col-reverse items-stretch justify-center gap-3 sm:w-auto sm:flex-row sm:items-center">
                    <AppButton
                      text="Go Back"
                  ...
                  {technicalDetail && (
                    <p className="max-w-sm break-words text-[11px] leading-relaxed text-text-muted">
                      If this keeps happening, quote this to support:{" "}
  ...
  return (
    <div className="min-h-[calc(100vh-64px)] bg-background">
      <div className="mx-auto w-full max-w-[1800px]">
        ...
        <div className="space-y-3">
          {apptLoading ? (
            ...
            !showFullPatientDetails &&
            /* Only once the appointment is confirmed. Before that the doctor is
               still verifying who this is, what was booked and whether it's
               paid — exactly what the three cards carry — so collapsing them
               early would hide the information the screen exists for. */
            /confirm|arrived|complete/i.test(String(appointment.status ?? "")) ? (
            /* Prescribing: the three cards (~180px) collapse to one line, and
               the reclaimed height goes to the medicine table. Nothing is lost
               — "View Full Details" brings the cards straight back. */
            <ConsultationPatientBar
              ...
              /* Token only — falling back to the date put "31 July 2026" under
                 a "Visit ID" label, which reads as wrong data. */
              visitId={appointment.tokenNo}
              /* Read defensively off the raw appointment: these fields aren't
                 on the mapped shape, and their names vary between payloads. */
              /* Reuses the summary card's resolver rather than re-deriving it:
                 the service name lives under ~18 different keys depending on
                 the payload, and a short guess here left the field blank. */
              /* `clinicService` is the required second argument — it lives at
                 `data.result.clinicService`, outside the `appointment` memo, so
                 passing only `a` made the resolver fall through to its "—". */
              service={getServiceName(a, clinicService)}
              amount={priceValue}
              /* Coverage from an earlier paid appointment expires on the
                 service record, not this appointment — same resolver the
                 booking flow uses to decide whether a visit is covered. */
              coveredUntil={getServiceExpiryIso(clinicService)}
              doctorName={doctor?.name}
              paymentStatus={
                (a as any)?.paymentStatus ??
                (a as any)?.payment_status ??
                ((a as any)?.isPaid ? "Paid" : undefined)
              }
              paymentMode={
                (a as any)?.paymentMode ?? (a as any)?.payment_mode
              }
              onViewFullDetails={() => setShowFullPatientDetails(true)}
              showMarkAsCompleted={canShowMarkAsCompleted}
              onMarkAsCompleted={handleMarkAsCompleted}
              isMarkAsCompletedLoading={actionLoading === "confirm"}
              isMarkAsCompletedDisabled={actionsDisabled}
            />
          ) : (
            <>
              ...
              totalAmountText={combinedTotalText}
              ...
              nextPatientInfo={nextPatient}
              onGoToNextPatient={nextPatient ? handleGoToNextPatient : undefined}
              onCollapseDetails={
                isPrescriptionTab &&
                /confirm|arrived|complete/i.test(
      ...
      {/* Guard rail for confirming / checking in an appointment scheduled for
          a later day — see handleConfirm / handlePatientArrived. */}
      <FutureDateActionModal
        action={pendingFutureAction}
        onClose={() => setPendingFutureAction(null)}
        onConfirm={handleFutureActionConfirmed}
        appointmentDate={formattedAppointmentDate}
        appointmentTime={to12h((a as any)?.appointmentTime)}
        patientName={patient?.name}
        doctorName={doctor?.name}
        daysAway={daysUntilAppointment}
