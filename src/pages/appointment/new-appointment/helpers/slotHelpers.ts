...
/**
 * The doctor's working windows for the fetched day, derived from the slot grid itself: one
 * `{ start, end }` per shift (breaks excluded, since a shift boundary is exactly where the break
 * slots sit). Empty for token-mode days, which have no clock times to compare against.
 */
export const getWorkingWindowsFromSlots = (
  slots: Slot[],
