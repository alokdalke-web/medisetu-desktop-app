...
const UPIQRCodeModal: React.FC<UPIQRCodeModalProps> = ({
  ...
  return (
    <Modal
      aria-label="UPI QR Code Payment"
      isOpen={isOpen}
      ...
      classNames={{
        base: "rounded-2xl overflow-hidden bg-surface",
        body: "p-6",
      }}
    >
      <ModalContent>
        {(onClose) => (
          <ModalBody className="flex flex-col items-center text-center">
            ...
            <div className="relative flex items-center justify-between w-full mt-6 px-2">
              <div>
                {upiIds.length > 1 && (
                  <button
                    type="button"
                    onClick={handlePrev}
                    className="w-10 h-10 rounded-full border border-line bg-surface flex items-center justify-center text-text-muted hover:bg-slate-50 dark:hover:bg-[#1a2535] active:scale-95 transition-all shadow-sm flex-shrink-0"
              ...
              {/* QR Image Box */}
              <div className="w-[240px] h-[240px] border border-line bg-surface rounded-2xl p-2.5 flex items-center justify-center shadow-sm relative overflow-hidden">
                {qrCodeUrl ? (
                  <img
                    src={qrCodeUrl}
            ...
            {upiIds.length > 1 && (
              <div className="mt-4 flex flex-col items-center">
                ...
                <span className="text-sm font-semibold text-text mt-2 select-all select-none cursor-pointer">
                  {activeUpiId}
                </span>
              </div>
            )}
            ...
            <div className="w-full mt-6 flex gap-2">
              <Button
                color="default"
                variant="flat"
                className="flex-1 rounded-xl text-text-muted font-semibold"
                onClick={onClose}
              >
                Close
              </Button>
