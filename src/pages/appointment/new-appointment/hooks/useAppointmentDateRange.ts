...
const useAppointmentDateRange = ({
  ...
  const calendarMonthSections = React.useMemo(() => {
    ...
    return monthKeys.map((monthKey) => {
      ...
      // Keep every week so the "Pick a date" popover shows a complete month
      // calendar. Days outside the booking window stay disabled (isAllowed:
      // false); the horizontal day strip still lists only the allowed days.
