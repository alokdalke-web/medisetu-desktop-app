...
const useAppointmentSymptoms = ({
  ...
  const toggleSymptomByName = (name: string) => {
    const trimmed = name.trim();
    if (!trimmed) return;

    const existing = symptoms.find(
      (s) => normalizeSymptomText(s.name) === normalizeSymptomText(trimmed)
    );
    if (existing) {
      if (existing.id === symptoms[0]?.id) {
        updateSymptomName(existing.id, "");
      } else {
        removeSymptom(existing.id);
      }
    } else {
      if (symptomLimitReached) {
        showSymptomLimitToast();
        return;
      }
      setSymptoms((prev) => {
        const first = prev[0];
        const newSymptom = mkSymptom(trimmed);
        return [first, newSymptom, ...prev.slice(1)];
      });
    }
  };
