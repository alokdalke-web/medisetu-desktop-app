...
export const mapReportPrescriptionToSelectedMed = (
  ...
  const name = safe(p.medicineName || p.name, "Medicine").toUpperCase();
  const medicineId = safe(p.medicineId || p.medicine?.id || p.id || String(idx));
  ...
  const imageUrl = safe(p.imageUrl);
  const medicineCount = p.medicineCount ?? "";
...
export const mergeReportCardToDetails = (
  ...
  if (rc.followUpDate && !next.followUpDate) {
    next.followUpDate = String(rc.followUpDate).slice(0, 10);
  }

  if (rc.additionalInformation && typeof rc.additionalInformation === "object") {
    next.additionalInformation = rc.additionalInformation;
  }
