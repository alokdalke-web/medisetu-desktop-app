 ...
 * - Week days array
 *
 * The current-time-line position (which needs a periodic clock tick) is
 * NOT computed here — it's computed inside `AppointmentCalendarView` itself,
 * so that tick only re-renders the calendar subtree instead of the whole
 * Appointment page every 60 seconds. See UI_REMEDIATION_LOG.md.
 */
...
export const useAppointmentCalendar = (): UseAppointmentCalendarReturn => {
