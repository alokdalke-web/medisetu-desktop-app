...
const AppointmentCalendarView: React.FC<AppointmentCalendarViewProps> = ({
  ...
  // Owns its own clock tick so the 60s re-render it drives stays scoped to
  // this component's subtree instead of re-rendering the whole Appointment
  // page (toolbar, stat cards, list/card rows) every minute — see
  // UI_REMEDIATION_LOG.md.
  ...
  const localCurrentTimeLine = useMemo(() => {
    ...
    const dateKey = toYmd(nowTick);
    const dayIdx = weekDays.findIndex((d) => toYmd(d) === dateKey);

    if (dayIdx < 0) return null;
    return { top, dayIdx };
  }, [nowTick, weekDays, minHour, hours.length, slotHeight]);

  const effectiveTimeLine = currentTimeLine || localCurrentTimeLine;
  ...
  const IconSquareBtn: React.FC<{
    ...
    <button
      ...
      onClick={onClick}
      className={[
        "h-10 w-10 lg:h-8 lg:w-8 rounded-md border border-line bg-surface",
        "grid place-items-center text-text cursor-pointer",
  ...
  const currentTimeLineForDay = useMemo(() => {
    if (!effectiveTimeLine) return null;
    const actualDay = weekDays[effectiveTimeLine.dayIdx]
      ? toYmd(weekDays[effectiveTimeLine.dayIdx])
      : "";
    if (actualDay !== selectedYmd) return null;
    return { top: effectiveTimeLine.top, dayIdx: 0 };
  }, [effectiveTimeLine, weekDays, selectedYmd]);
    ...
    return (
      <Tooltip
        ...
        content={
          <div
            onClick={onClick}
            className="w-[min(320px,calc(100vw-2rem))] overflow-hidden rounded-2xl border border-line bg-surface shadow-xl cursor-pointer"
          >
            <div className="flex items-start justify-between gap-3 px-4 pb-3 pt-4">
              <div className="flex items-start gap-3">
                <div className="h-10 w-10 shrink-0 overflow-hidden rounded-full bg-surface-muted ring-1 ring-line grid place-items-center">
                  {ev.avatarUrl ? (
                    <img
                      src={ev.avatarUrl}
                      alt={ev.title}
            ...
            <div className="bg-surface-muted px-4 py-3">
              <div className="flex items-center gap-3">
                <div className="h-10 w-10 shrink-0 rounded-full bg-emerald-500/10 ring-1 ring-emerald-500/20 grid place-items-center text-emerald-700 dark:text-emerald-400">
            ...
            <div className="flex items-center justify-between gap-3 px-4 py-3">
              <div className="flex items-center gap-3">
                <div className="grid h-9 w-9 place-items-center rounded-full bg-emerald-500/10 text-emerald-700 dark:text-emerald-400">
            ...
            <div className="border-t border-line px-4 py-2 text-[11px] font-semibold text-primary">
      ...
      >
        <div
          role="button"
          tabIndex={0}
          aria-label={`Appointment: ${ev.title}, ${formatDateLong(ev.start)}, ${
            ev.tokenNo != null
              ? `token ${ev.tokenNo}`
              : `${formatTime12(ev.start)} to ${formatTime12(ev.end)}`
          }, ${cap(ev.status)}`}
          className={[
            "absolute left-1.5 right-1.5 z-20 overflow-hidden",
        ...
        >
          <div className="flex items-start gap-1.5 h-full">
            <div
              className=" h-8 w-8 rounded-full overflow-hidden bg-surface ring-1 ring-[var(--st-fg)] grid place-items-center shrink-0"
              style={stVars}
            >
              {ev.avatarUrl ? (
                <img
  ...
  const TimeColumn = () => {
    return (
      <div className="border-r border-line bg-surface sticky left-0 z-20">
        {Array.from({ length: slotCount }).map((_, i) => {
          ...
          // Only the top-of-hour is labelled; a label on every 30-min row
          // doubles the text density without adding information.
          const isHourStart = m === 0;

          return (
            <div
              key={i}
              className={[getRowBorder(i), "bg-surface"].join(" ")}
              style={{ height: `${slotHeight}px` }}
            >
              <div className="flex h-full items-start justify-end pr-2 pt-1">
                {isHourStart && (
  ...
  const DayColumn = ({
    ...
    return (
      <div
        className={[
          "border-r border-line last:border-r-0 relative",
          highlight ? "bg-primary/5" : "bg-surface",
        ].join(" ")}
      >
        <div className="relative" style={{ height: `${gridHeight}px` }}>
          ...
          {blocked.map((block, i) => (
            <div
              ...
              className={[
                ...
                "bg-surface-muted/60 border-y border-line/50 text-text-subtle",
                stripedOverlay,
              ].join(" ")}
              style={{
                top: `${block.top}px`,
            ...
            >
              {/* Label only when the block is tall enough to hold it — short
                  breaks read as a plain tint rather than clipped text. */}
              {block.height >= 26 && (
                <div className="px-2 py-1 text-[10px] font-semibold uppercase leading-tight tracking-wide">
          ...
          {Array.from({ length: slotCount }).map((_, i) => {
            ...
            return (
              <div
                ...
                className={[
                  getRowBorder(i),
                  "group/slot relative transition-colors",
                  isPast
                    ? "bg-surface-muted/40"
                    : "bg-surface hover:bg-primary/5",
                  !isPast && !isUnavailable && !isBusy ? "cursor-pointer" : "",
                ].join(" ")}
                style={{ height: `${slotHeight}px` }}
                onClick={() => {
              ...
              >
                {/* Free slots stay visually empty — the affordance only appears
                    on hover, so a week of open slots reads as whitespace
                    instead of a wall of repeated "No Appointment" labels. */}
                {!isUnavailable && !isBusy && !isPast && (
                  <span className="pointer-events-none absolute inset-0 flex select-none items-center justify-center text-[11px] font-semibold text-primary opacity-0 transition-opacity duration-150 group-hover/slot:opacity-100">
                    + Book
                  </span>
                )}
              </div>
          ...
          {typeof timelineTop === "number" && (
            ...
            >
              <div className="-ml-1 h-2.5 w-2.5 rounded-full bg-danger ring-2 ring-surface" />
              <div className="h-[1.5px] flex-1 bg-danger" />
            </div>
          )}
  ...
  const WeekHeader = ({ bp }: { bp: "md" | "sm" }) => {
    ...
    return (
      <div
        className="grid border-b border-line sticky top-0 z-30 bg-surface shadow-sm pt-1"
        ...
        {weekDays.map((d) => {
          ...
          return (
            <div
              key={d.toISOString()}
              className={[
                "border-r border-line last:border-r-0 px-2 py-1.5 text-center",
                isToday ? "bg-primary/10" : "bg-surface-muted",
              ].join(" ")}
            >
              <p
                className={[
      ...
      <>
        ...
        <div className="grid relative -mt-1" style={{ gridTemplateColumns: tpl, contain: 'layout' }}>
          ...
          {eventsByDay.map((col, dayIdx) => {
            ...
            const tl =
              effectiveTimeLine && effectiveTimeLine.dayIdx === dayIdx
                ? effectiveTimeLine.top
                : null;
          ...
          {doctors.map((doc) => {
            ...
            return (
              <div
                key={doc.id}
                className={[
                  "border-r border-line last:border-r-0 px-2 py-2 text-center",
                  isToday ? "bg-primary/10" : "bg-surface-muted",
                ].join(" ")}
              >
  ...
  return (
    <div
      className={[
        "rounded-xl bg-surface shadow-sm overflow-hidden border border-line",
        isFullscreen ? "fixed inset-0 z-[9999] rounded-none" : "",
      ].join(" ")}
    >
      {/* Header (Desktop) */}
      <div className="hidden md:flex items-center justify-between border-b border-line bg-surface px-3 py-2">
        <div className="flex items-center gap-2">
          ...
          {onToggleMode && (
            <div className="ml-2 flex items-center rounded-md border border-line p-0.5">
              <button
                type="button"
                onClick={() => onToggleMode("week")}
                aria-pressed={activeMode === "week"}
                className={`h-10 lg:h-7 px-3 lg:px-2 rounded text-[11px] font-semibold cursor-pointer focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary ${activeMode === "week"
              ...
              <button
                ...
                className={`h-10 lg:h-7 px-3 lg:px-2 rounded text-[11px] font-semibold cursor-pointer focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary ${activeMode === "day"
                  ? "bg-primary text-white"
                  : "text-text-muted hover:bg-surface-muted"
                  }`}
              >
          ...
          {isAdminOrReception && doctors.length > 0 && activeMode === "week" && (
            <div className="w-[170px]">
              <Select
                ...
                classNames={{
                  trigger:
                    "bg-surface border border-line shadow-none min-h-[32px] h-8 rounded-md",
                  value: "text-xs text-text",
                }}
              >
                {doctors.map((doc) => (
      ...
      {/* Header (Mobile) */}
      <div className="md:hidden border-b border-line bg-surface px-3 py-2">
        ...
        {isAdminOrReception && doctors.length > 0 && activeMode === "week" && (
          <div className="mt-2">
            <Select
              ...
              classNames={{
                trigger:
                  "bg-surface border border-line shadow-none min-h-[36px] h-9 rounded-lg",
      ...
      <div
        className={[
          "overflow-x-auto overflow-y-auto bg-surface relative",
