...
const normalizeFromAPI = (u: any): Row => {
  ...
  const isAdminDoctorAccess = u?.isAdminDoctorAccess === true;
...
const StatusPill = ({ status, isDeactivatedByPlan }: { status: string; isDeactivatedByPlan?: boolean }) => {
  ...
  if (key === "rejected") {
    return (
      <span className="inline-flex items-center gap-2 rounded-full border px-3 py-1 text-[12px] font-semibold leading-none border-red-300 bg-red-50 text-red-700">
...
const UsersRoleTable: React.FC<Props> = ({
  ...
  const currentUser = currentUserData ? ((currentUserData as any).user ?? currentUserData) : null;
  const isCurrentUserPending =
    String(currentUser?.userStatus ?? "").trim().toLowerCase() === "pending";
  ...
  const [statusChangeModal, setStatusChangeModal] = useState<{
    isOpen: boolean;
    row: Row | null;
    selectedStatus: "Inactive" | "Rejected" | null;
    isLoading: boolean;
  }>({
    isOpen: false,
    row: null,
  ...
  const filteredRawUsers = useMemo(() => {
    ...
    return rawUsers.filter((u) => {
      ...
      if (apiType === t) return true;

      if (t === "doctor" && apiType === "admin") {
        return true;
      }
  ...
  const rejectedCount = summaryRows.filter(
    (row) => normalizeStatusKey(row.status) === "rejected",
  ).length;
  const activePercent =
  ...
  // Handle status toggle - opens modal for Active -> Inactive/Rejected
  const handleStatusToggle = (row: Row, isSelected: boolean) => {
    if (!row.rawId) return;
    if (updatingUserIds[row.rawId]) return;

    // If toggling to Active, directly update
    if (isSelected) {
      directStatusChange(row, "Active");
      return;
    }

    // If toggling from Active to Inactive/Rejected, open modal
    ...
    // For other cases, direct update
    directStatusChange(row, isSelected ? "Active" : "Inactive");
  };

  // Direct status change without modal (for activation)
  const directStatusChange = (row: Row, nextStatus: UserStatus) => {
    if (!row.rawId) return;
    if (updatingUserIds[row.rawId]) return;
    ...
    setStatusOverrides((prev) => ({ ...prev, [row.rawId]: nextStatus }));

    setUpdatingUserIds((prev) => ({ ...prev, [row.rawId]: true }));
    ...
    updateAddUser({
      ...
      .then(() => {
        addToast({
          ...
          description: `${row.name} is now ${nextStatus}.`,
          color: "success",
        });
        void refetch();
      })
      .catch((e: any) => {
        setStatusOverrides((prev) => ({ ...prev, [row.rawId]: currentStatus }));
        addToast({
          title: "Update failed",
          description: e?.data?.message || e?.message || "Unable to update user status.",
          color: "danger",
        });
      })
      .finally(() => {
  ...
  const handleStatusChangeConfirm = async () => {
    const { row, selectedStatus } = statusChangeModal;
    ...
    try {
      ...
      // If rejecting, add additional flags for billing/addon removal
      if (selectedStatus === "Rejected") {
        updatePayload.isRejected = true;
        updatePayload.rejectedAt = new Date().toISOString();
        updatePayload.removeAddons = true; // Flag to remove addon plans
        updatePayload.adjustBilling = true; // Flag to adjust billing
      } else if (selectedStatus === "Inactive") {
        updatePayload.deactivatedAt = new Date().toISOString();
      }
      ...
      setStatusOverrides(prev => ({
        ...prev,
        [row.rawId]: selectedStatus
      }));

      addToast({
        title: selectedStatus === "Rejected" ? "Profile Rejected" : "Profile Deactivated",
        description: selectedStatus === "Rejected" 
          ? `${row.name}'s profile has been rejected. Addon plans will be removed and billing adjusted.`
          : `${row.name}'s profile has been deactivated.`,
        color: selectedStatus === "Rejected" ? "warning" : "success",
      });
  ...
  const StatusToggle = ({ row }: { row: Row }) => {
    const currentStatus = getToggleableUserStatus(row.status);
    const canToggle = Boolean(currentStatus) && row.role !== "Admin";
    const isActive = currentStatus === "Active";
    const isUpdatingRow = Boolean(updatingUserIds[row.rawId]);
    const label = currentStatus ?? formatStatusLabel(row.status);

    // Don't allow toggling for Rejected users
    if (row.status === "Rejected") {
      return (
        <div className="inline-flex items-center gap-2">
          <span className="text-[12px] font-semibold text-red-600">Rejected</span>
    ...
    return (
      ...
      >
        <Switch
          ...
          isSelected={isActive}
          size="sm"
          onValueChange={(selected) => handleStatusToggle(row, selected)}
        >
          <span
            className={[
          ...
          >
            {row.status === "Rejected" ? "Rejected" : label}
          </span>
        </Switch>
      </div>
    );
  ...
  return (
    <div className="w-full space-y-4">
      ...
      >
        <ModalContent>
          {(onClose) => {
            const { row, selectedStatus, isLoading } = statusChangeModal;
            ...
            return (
              <>
                ...
                <ModalBody className="space-y-4 py-5">
                  <div className="rounded-lg bg-amber-50 border border-amber-200 p-4">
                    <p className="text-sm text-amber-800 flex items-start gap-2">
                      ...
                      <span>
                        {row.name} is currently <strong>Active</strong>. Changing their status will affect their access to the system.
                      </span>
                    </p>
                  </div>
                  ...
                  <RadioGroup
                    ...
                    onValueChange={(value) =>
                      setStatusChangeModal((prev) => ({
                        ...prev,
                        selectedStatus: value as "Inactive" | "Rejected",
                      }))
                    }
                    className="mt-2"
                  >
                    <div className="flex gap-20">
                      <Radio value="Inactive" className="py-2">
                        <div>
                          <span className="font-semibold text-slate-700">Inactive</span>
                        </div>
                      </Radio>
                      <Radio value="Rejected" className="py-2">
                        <div>
                  ...
                  {selectedStatus === "Inactive" && (
                    <div className="rounded-lg bg-green-50 border border-green-200 p-4">
                      <div className="flex items-start gap-2">
                        <FiXCircle className="mt-0.5 text-green-500 shrink-0" />
                        <div className="text-sm text-green-800">
                          <p className="font-semibold">This action is can be revert.</p>
                          <ul className="mt-2 list-disc list-inside space-y-1 text-xs">
                            <li>User can be reactivate</li>
                  ...
                  {selectedStatus === "Rejected" && (
                    <div className="rounded-lg bg-red-50 border border-red-200 p-4">
                      <div className="flex items-start gap-2">
                        <FiXCircle className="mt-0.5 text-red-500 shrink-0" />
                        <div className="text-sm text-red-800">
                          <p className="font-semibold">This action is permanent and cannot be undone.</p>
                          <ul className="mt-2 list-disc list-inside space-y-1 text-xs">
                            <li>User cannot be reactivate and will lose all access to the system</li>
                            <li>Remove Addon plans to avoid additional billing</li>
                          </ul>
                        </div>
                      </div>
                    </div>
                ...
                <ModalFooter className="gap-3 border-t border-slate-100 py-4">
                  ...
                  />
                  <AppButton
                    text={isLoading ? "Updating..." : `Set as ${selectedStatus || "Status"}`}
                    type="button"
                    ...
                    isDisabled={!selectedStatus || isLoading}
                    isLoading={isLoading}
                  />
                </ModalFooter>
              </>
