...
const LabDetails: React.FC = () => {
  ...
  useEffect(() => {
    if (!isUserLoading && isPendingApproval) {
      navigate("/users?type=Lab_Assistant&role=Lab_Assistant", { replace: true });
    }
  ...
  if (isUserLoading || isPendingApproval || isLabFetching) {
    return (
    ...
    <div className="mx-auto w-full">
      <div className="space-y-5">
        ...
        <section className="mt-5 overflow-hidden rounded-lg border border-slate-200 bg-white shadow-sm">
          <div className="flex flex-col gap-3 border-b border-slate-200 px-4 py-2 sm:flex-row sm:items-center sm:justify-between sm:px-5">
            <div>
            ...
            {isLabTestsFetching ? (
              ...
              <div className="flex flex-col items-center justify-center px-5 py-10 text-center">
                ...
                <div className="mt-1 max-w-sm text-[13px] text-slate-500">
                  Select departments and tests to add them to this lab catalog.
                </div>
              </div>
            ) : (
              <>
                <div className="max-h-[640px] space-y-3 overflow-y-auto overscroll-contain p-3 pr-2 [scrollbar-color:#cbd5e1_transparent] [scrollbar-gutter:stable] [scrollbar-width:thin] [&::-webkit-scrollbar]:w-1.5 [&::-webkit-scrollbar-track]:bg-transparent [&::-webkit-scrollbar-thumb]:rounded-full [&::-webkit-scrollbar-thumb]:bg-slate-300/70 hover:[&::-webkit-scrollbar-thumb]:bg-slate-400 lg:hidden">
                  {catalogRows.map((row) => (
                    <div
                      key={row.id}
                ...
                <div className="hidden max-h-[272px] divide-y divide-slate-100 overflow-y-auto overscroll-contain [scrollbar-color:#cbd5e1_transparent] [scrollbar-gutter:stable] [scrollbar-width:thin] [&::-webkit-scrollbar]:w-1.5 [&::-webkit-scrollbar-track]:bg-transparent [&::-webkit-scrollbar-thumb]:rounded-full [&::-webkit-scrollbar-thumb]:bg-slate-300/70 hover:[&::-webkit-scrollbar-thumb]:bg-slate-400 lg:block">
                  {catalogRows.map((row) => (
                    <div
                      key={row.id}
        ...
        <section className="mt-5 overflow-hidden rounded-lg border border-slate-200 bg-white shadow-sm">
          <div className="flex flex-col gap-3 border-b border-slate-200 px-4 py-2 sm:flex-row sm:items-center sm:justify-between sm:px-5">
            ...
            <AppButton
              text="Lab Users"
              onPress={() => navigate(`/users?role=Lab_Assistant&type=Lab_Assistant`)}
              buttonVariant="primary"
              className="h-10 w-full rounded-lg px-4 font-semibold sm:w-auto"
              // startContent={<FiPlus className="text-[14px]" />}
              isDisabled={status === "Inactive"}
