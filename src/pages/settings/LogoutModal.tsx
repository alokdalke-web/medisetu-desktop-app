...
import { clearAllCalls } from "../../redux/slices/callSlice";
...
const LogoutModal: React.FC<LogoutModalProps> = ({
  ...
  const clearLogoutOnlyStorage = () => {
    [
      "authToken",
      "authUser",
      "activeRole",
      "medisetu_notifications",
      "medisetu_notifications_dismissed",
      "medisetu_call_state",
    ].forEach((key) => {
  ...
  const handleLogout = () => {
    // Best-effort server-side revoke of the refresh token; local logout
    // proceeds regardless of whether this succeeds.
    logoutUser().catch(() => {
      /* non-critical */
    });
    dispatch(clearNotifications());
    dispatch(clearAllCalls());
    dispatch(clearActiveRole());
    dispatch(logout());
    disconnectSocket();
    clearLogoutOnlyStorage();
    onOpenChange(false);
    onConfirm?.();
  };
