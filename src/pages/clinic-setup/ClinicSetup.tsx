...
const ClinicSetup = () => {
  ...
  const userSetupState = user as SetupUserState | undefined;
  const shouldLoadClinicSetupState = userSetupState?.userType === "Admin";
  const { data: clinics } = useGetAllClinicsQuery(undefined, {
    skip: !shouldLoadClinicSetupState,
  });
  ...
  const approvalRequestSent =
    clinicProfile?.approvalRequestSent ??
    userSetupState?.approvalRequestSent ??
    false;
  ...
  // ✅ Check if user has already seen the waiting screen in this session or previous session
  ...
  const handleDashboardNavigation = React.useCallback(() => {
    // Mark as seen in both sessionStorage and localStorage
    if (user?.id) {
      const sessionKey = `onboarding_submitted_${user.id}`;
      try {
