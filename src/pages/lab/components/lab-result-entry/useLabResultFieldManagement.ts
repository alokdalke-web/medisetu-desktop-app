...
type UseLabResultFieldManagementArgs = {
  isOpen: boolean;
  template: LabResultTemplate | null;
  appointmentTestId?: string;
  report: LabResultReport | null;
  savedResultId: string | null;
  reloadTemplate: (options?: RefreshOptions) => Promise<void>;
  refreshOpenResultReport: (options?: RefreshOptions) => Promise<void>;
};

export function useLabResultFieldManagement({
  isOpen,
  template,
  appointmentTestId,
  report,
...
}: UseLabResultFieldManagementArgs) {
  ...
  useEffect(() => {
    ...
    resetFieldControls();
  }, [isOpen, resetFieldControls, resetFieldState]);

  const reloadManagedParameters = useCallback(
    async (templateId?: string, options: RefreshOptions = {}) => {
  ...
  const refreshFieldData = async (
    options: RefreshOptions = { silent: true },
  ) => {
    await Promise.all([
