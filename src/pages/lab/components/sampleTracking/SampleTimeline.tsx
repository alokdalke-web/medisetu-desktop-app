...
export function SampleTimeline({
  ...
  return (
    <motion.section
      ...
      className="rounded-xl border border-slate-200/80 bg-white px-3 py-3 shadow-[0_10px_28px_rgba(15,23,42,0.045)]"
    >
            ...
            return (
              ...
              >
                <div className="relative">
                  ...
                  {((completed && step.timestamp) || (current && expectedReportReadyAt)) && (
                    <div className="absolute bottom-full left-1/2 z-20 mb-2 flex -translate-x-1/2 flex-col items-center gap-0.5">
                      {completed && step.timestamp && (
                        <span className="whitespace-nowrap rounded-xl bg-white px-1.5 py-0.5 text-[10px] font-semibold leading-none text-slate-600 shadow-xs">
                          {formatTimestamp(step.timestamp)}
                        </span>
                      )}
                      {current && expectedReportReadyAt && (
                        <span className="inline-flex items-center gap-1 whitespace-nowrap rounded-xl border border-amber-200/60 bg-amber-50 px-1.5 py-0.5 text-[10px] font-semibold leading-none text-amber-700 shadow-xs">
                          <span>ETA</span>
                  ...
                  <div
                    className={[
                      "relative z-10 grid h-9 w-9 place-items-center rounded-full border-2 text-sm font-semibold shadow-sm transition-all duration-300",
                      completed
                ...
                <div className="mt-2 flex w-full min-w-0 flex-col items-center px-0.5">
                  <h3
                    title={step.title}
                    className={[
                      "max-w-full text-center text-[13px] font-semibold leading-[1.2] transition-colors duration-200",
                      completed
                  ...
                  <div className="mt-2.5 flex justify-center opacity-95 transition-opacity duration-200 group-hover:opacity-100">
                    <TrackingStatusBadge
                      label={statusLabel}
