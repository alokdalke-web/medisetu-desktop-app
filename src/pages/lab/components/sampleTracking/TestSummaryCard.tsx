...
export function TestSummaryCard({
  ...
  return (
    <motion.section
      ...
      className="overflow-hidden rounded-xl border border-slate-200/80 bg-white shadow-[0_12px_32px_rgba(15,23,42,0.06)]"
    >
      <div className="px-4 py-2.5 sm:px-5">
        <div className="flex flex-col gap-2.5 xl:flex-row xl:items-center xl:justify-between">
            ...
            <div className="min-w-0">
              <div className="flex flex-wrap items-center gap-2">
                <h2
                  ...
                  className="truncate text-lg font-semibold leading-tight text-slate-950"
                >
                ...
                <span
                  ...
                  className="rounded-full border border-primary/15 bg-primary/10 px-2.5 py-1 text-[11px] font-semibold text-primary"
                >
                  {appointmentTest.category ?? "-"}
                </span>
                <LabScreenInfoTooltip
                  title="Sample Tracking"
                  description="Use this screen to track one accepted test from payment to completion. Update sample steps, print or download the barcode, enter or edit results, preview the generated report, upload a PDF report when needed, and review the activity log."
                  items={[
                    "Complete payment and sample workflow steps before result entry or report upload.",
                    "Use the activity tracker to confirm when each lab action was recorded.",
                  ]}
                  placement="bottom"
                  guideSection="lab"
                  linkLabel="Read sample tracking guide"
                />
              </div>
            </div>
          </div>
          ...
          {barcodeValue ? (
            <div className="flex w-full items-center justify-between gap-3 rounded-xl border border-slate-200 bg-white px-3 py-1.5 shadow-xs xl:ml-auto xl:max-w-[450px]">
              <div className="flex min-w-0 flex-1 items-center gap-3">
                ...
                <div className="flex min-w-0 flex-1 flex-col items-center gap-0.5">
                  ...
                  <span className="font-mono text-[10.5px] font-semibold leading-none text-slate-500">
                    {barcodeValue}
                  </span>
                </div>
              </div>
            ...
            <div className="flex w-full items-center gap-3 rounded-xl border border-dashed border-slate-200 bg-slate-50/70 px-4 py-2 xl:ml-auto xl:max-w-[420px]">
        ...
        <div className="mt-2.5 grid gap-3 md:grid-cols-2 xl:grid-cols-4">
          {detailItems.map((item) => (
            <div
              key={item.label}
              className="rounded-xl border border-slate-200/80 bg-white px-4 py-3 shadow-[0_10px_24px_rgba(15,23,42,0.035)]"
            >
        ...
        <div
          className={[
            "mt-3 grid overflow-hidden rounded-xl border border-slate-200 bg-white shadow-xs sm:grid-cols-2 lg:grid-cols-4",
        ...
        >
          ...
          <div className="flex min-h-12 items-center gap-3 border-b border-slate-100 px-4 py-2.5 sm:border-b lg:border-b-0 lg:border-r">
            ...
            <span
              title={uniqueTestId}
              className="shrink-0 rounded-full border border-primary/15 bg-primary/10 px-2.5 py-1 font-mono text-[10px] font-semibold text-primary"
            >
              {uniqueTestId}
            </span>
          </div>
