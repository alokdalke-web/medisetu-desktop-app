...
export function EventsCard({
  ...
  return (
    <motion.section
      ...
      className="flex h-full min-h-[520px] flex-col rounded-xl border border-slate-200/80 bg-white p-5 shadow-[0_12px_32px_rgba(15,23,42,0.06)] xl:min-h-0"
    >
      <div className="flex items-start justify-between gap-3">
        <div className="flex items-center gap-3">
          <div className="grid h-10 w-10 place-items-center rounded-xl bg-primary/10 text-primary ring-1 ring-primary/10">
        ...
        <span className="rounded-full bg-slate-50 border border-slate-100 px-2.5 py-1 text-[11px] font-semibold text-slate-600">
          {events.length} Logs
        </span>
      </div>
            ...
            {events.map((event) => {
              ...
              return (
                ...
                >
                  <div
                    className={[
                      "relative z-10 grid h-9 w-9 shrink-0 place-items-center rounded-xl border text-sm shadow-xs transition-all duration-300 group-hover:scale-105",
                      visuals.colorClass,
                    ].join(" ")}
                  >
                    {visuals.icon}
                  ...
                  <div className="min-w-0 flex-1 rounded-xl border border-slate-100 bg-white p-3.5 shadow-[0_8px_20px_rgba(15,23,42,0.035)] transition-all duration-200 hover:border-slate-200 hover:shadow-[0_10px_24px_rgba(15,23,42,0.06)]">
                    <div className="flex flex-col gap-1.5 sm:flex-row sm:items-start sm:justify-between sm:gap-2">
                      <div className="min-w-0">
                        <p className="text-[13px] font-semibold leading-4 text-slate-900 transition-colors duration-150 group-hover:text-primary">
                          {event.title}
                        </p>
                        ...
                        {event.actorUserName && (
                          <div className="flex items-center gap-1.5 mt-1.5">
                            <span
                              className={[
                                "flex h-5 w-5 shrink-0 items-center justify-center rounded-full text-[9px] font-semibold border",
                      ...
                      <span className="mt-0.5 shrink-0 text-[11px] font-semibold text-slate-500">
                        {formatTimestamp(event.createdAt)}
                      </span>
                    </div>

                    {event.description && (
                      <p className="mt-2 border-t border-slate-100/70 pt-1.5 text-xs leading-4 text-slate-500">
                        {event.description}
                      </p>
                    )}
                  </div>
