...
const LOCKED_FROM_PENDING_STATUSES: BoardStatus[] = [
  "Approved",
  "Reviewing",
  "Rejected",
];

const isLockedStatusToPendingChange = (
  fromStatus?: BoardStatus,
  toStatus?: BoardStatus,
) =>
  toStatus === "Pending" &&
  Boolean(fromStatus && LOCKED_FROM_PENDING_STATUSES.includes(fromStatus));

const showPendingMoveBlockedToast = () => {
  addToast({
    title: "Status change not allowed",
    description:
      "Approved, Reviewing, or Rejected requests cannot be moved back to Pending.",
    color: "warning",
  });
};
...
export const useRequestPageHandlers = ({
  ...
  const handleDrop = useCallback(
    (
      ...
      const card = requestCards.find((card) => card.doctorId === payload.doctorId);
      const fromStatus = payload.currentStatus ?? card?.status ?? "Pending";

      if (isLockedStatusToPendingChange(fromStatus, targetStatus)) {
        showPendingMoveBlockedToast();
        return;
      }
  ...
  const handleMobileStatusChange = useCallback(
    (
      ...
      if (isLockedStatusToPendingChange(card.status, selectedStatus)) {
        showPendingMoveBlockedToast();
        return;
      }
