...
import { LabScreenInfoTooltip } from "../lab/components/LabScreenInfoTooltip";
import { formatCurrency, getGreetingName } from "../lab/labData";
import CustomDateRangePicker from "./CustomDateRangePicker";
import DateFilterTabs, { type DateTab } from "./DateFilterTabs";
import LabAssistantPendingDashboard from "./LabAssistantPendingDashboard";
...
function StatusBreakdown({
  ...
  return (
    <div className="grid h-full min-h-[220px] w-full grid-cols-1 items-center gap-4 sm:grid-cols-[170px_minmax(0,1fr)]">
      ...
      <div className="grid w-full min-w-0 grid-cols-1 gap-2">
        {items.map((item) => (
          <div
            key={item.key}
            className="grid min-h-[42px] min-w-0 grid-cols-[auto_minmax(0,1fr)_auto] items-center gap-2 rounded-lg bg-slate-50/80 px-3 py-2 text-xs transition-colors duration-150 hover:bg-slate-100 dark:bg-slate-800/30 dark:hover:bg-slate-800/60"
          >
            <div className="flex min-w-0 items-center gap-2">
              <span
              ...
              />
              <span className="min-w-0 truncate text-[11px] font-extrabold leading-snug text-slate-600 dark:text-slate-350">
                {item.name}
              </span>
            </div>
            <span className="block whitespace-nowrap text-right text-[12px] font-extrabold leading-none text-slate-850 dark:text-white">
              {item.value}
              <span className="ml-0.5 text-[10px] font-semibold text-slate-450 dark:text-slate-500">
                ({item.percentage}%)
              </span>
            </span>
          </div>
...
const LabDashContent = ({ user }: { user?: any }) => {
    ...
    <div id="tour-lab-dashboard" className="w-full min-w-0 px-3 pb-4 pt-0 antialiased dark:bg-[#0b1321] sm:px-0 sm:pb-6 lg:h-full lg:flex lg:flex-col lg:overflow-hidden">
      <div className="lg:min-h-0 lg:flex-1 lg:overflow-y-auto no-scrollbar">
        <div className="max-w-full space-y-4 sm:space-y-5">
          <div className="flex flex-col gap-3 sm:gap-4 xl:flex-row xl:flex-wrap xl:items-start xl:justify-between">
            <div className="flex shrink-0 flex-col gap-1">
              <div className="flex flex-wrap items-center gap-2">
                ...
                <LabScreenInfoTooltip
                  title="Lab Dashboard"
                  description="Use this dashboard to monitor today's lab workload, revenue, test trends, pending reports, and upcoming sample collection from one place."
                  items={[
                    "Switch the date range to compare daily, weekly, monthly, or custom lab performance.",
                    "Use quick actions and recent activity to jump into pending requests, active tests, reports, or catalog work.",
                  ]}
                  placement="bottom"
                  guideSection="dashboard-lab"
                  linkLabel="Read lab dashboard guide"
                />
            ...
            <div className="flex w-full flex-col gap-3 sm:flex-row sm:flex-wrap sm:items-center sm:justify-end xl:w-auto ">
              <div
                className="relative flex min-w-0 items-center gap-3 overflow-x-auto no-scrollbar sm:justify-end "
          ...
          <div className="grid grid-cols-1 gap-4 sm:gap-5 lg:grid-cols-[minmax(0,1fr)_310px] xl:grid-cols-[minmax(0,1fr)_330px]">
            <div className="flex min-w-0 flex-col gap-4 sm:gap-5">
              ...
              <div id="tour-lab-trends" className="grid grid-cols-1 items-start gap-4 sm:gap-5 xl:grid-cols-[1.45fr_1fr]">
                <TrendChartPanel
...
const LabDash = () => {
  ...
  const isPending = normalizeStatus(user?.userStatus) === "pending";

  if (isPending) {
    return <LabAssistantPendingDashboard />;
  }

  return <LabDashContent user={user} />;
};
