import { FiGlobe as WebIcon } from "react-icons/fi";
import {
  FaApple,
  FaAndroid,
  FaGooglePlay,
...
const SectionHeading = ({
  ...
  <div className="flex flex-col gap-1.5">
    <h4 className="flex items-center gap-2 text-[15px] font-semibold leading-tight text-text">
...
const AppTile = ({
  ...
  <div className="flex flex-1 flex-col items-center justify-center gap-2 rounded-xl border border-line bg-surface py-3.5 shadow-[0_1px_3px_rgba(16,24,40,0.04)] transition hover:border-primary/30 hover:shadow-[0_2px_8px_rgba(16,24,40,0.08)] dark:shadow-none">
    <span className="flex h-6 items-center justify-center">{icon}</span>
    <span className="text-center text-[11px] font-medium leading-none text-text-muted">
      {label}
    </span>
  </div>
);
...
const StoreBadge = ({
  ...
  <a
    ...
    className="flex flex-1 items-center justify-center gap-2 rounded-xl bg-[#100e1c] px-3 py-2 text-white shadow-sm transition hover:-translate-y-0.5 hover:shadow-md dark:bg-[#0a0a0a]"
  >
...
const CheckIcon = () => (
  <span className="flex h-[18px] w-[18px] shrink-0 items-center justify-center rounded-full bg-emerald-500/10">
    <svg className="h-[11px] w-[11px] text-emerald-600 dark:text-emerald-400" viewBox="0 0 20 20" fill="none">
      <path
...
const DashboardFooter = () => (
  <footer className="rounded-[16px] border border-line bg-gradient-to-br from-surface to-background-secondary px-5 py-6 sm:px-7 sm:py-7">
    <div className="grid grid-cols-1 gap-x-7 gap-y-6 sm:grid-cols-2 sm:gap-y-8 xl:grid-cols-[1.05fr_auto_0.8fr_auto_1.1fr_auto_1.1fr] xl:gap-x-0">
      ...
      {/* divider */}
      <div className="hidden xl:block w-px self-stretch bg-line" />
      ...
      <div className="flex flex-col gap-4 px-2 2xl:px-7">
        ...
        <div className="flex flex-col gap-2.5">
          {SOCIALS.map((s) => (
            <div
              key={s.name}
              className="flex items-center gap-2.5 rounded-lg px-1.5 py-1 -mx-1.5 transition hover:bg-surface-muted"
            >
              ...
              <div className="flex min-w-0 flex-1 items-baseline gap-2">
                ...
                <span className="hidden truncate text-[11px] text-text-muted sm:inline">
                  {s.meta}
                </span>
              </div>
              <a
                ...
                target="_blank"
                rel="noopener noreferrer"
                className="shrink-0 rounded-full border border-primary/30 px-3 py-1 text-[11px] font-semibold text-primary transition hover:bg-primary/10"
              >
                {s.cta}
              </a>
            </div>
      ...
      <div className="flex flex-col gap-4 px-2 2xl:px-7">
        ...
        <ul className="flex flex-col gap-2.5">
          {FEATURES.map((feature) => (
            <li
              key={feature}
              className="flex items-center gap-2.5 text-[12px] font-medium text-text-muted"
            >
              <CheckIcon />
              {feature}
            </li>
    ...
    {/* Bottom bar */}
    <div className="mt-6 flex flex-col items-center justify-between gap-2 border-t border-line pt-4 sm:flex-row">
      <p className="text-[12px] text-text-muted">
        © {new Date().getFullYear()} Infinity Medisetu. All rights reserved.
      </p>
      <p className="text-[12px] text-text-muted">
        Made with <span aria-hidden="true">💚</span> for healthcare professionals.
      </p>
    </div>
  </footer>
);
