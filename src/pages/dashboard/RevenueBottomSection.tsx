...
export default function RevenueBottomSection() {
  const [topClinicsPeriod, setTopClinicsPeriod] = useState<"thisMonth" | "thisYear">("thisMonth");
  const [planPeriod, setPlanPeriod] = useState<"thisMonth" | "thisYear">("thisMonth");
  const [analyticsPeriod, setAnalyticsPeriod] = useState<"monthly" | "weekly">("monthly");

  const { data: topClinicsApiResponse } = useGetSuperAdminTopPerformingClinicsQuery(
    { period: topClinicsPeriod === "thisMonth" ? "month" : "year" },
    { refetchOnMountOrArgChange: true }
  );

  const { data: planApiResponse } = useGetSuperAdminRevenueByPlanQuery(
    { period: planPeriod === "thisMonth" ? "month" : "year" },
    { refetchOnMountOrArgChange: true }
  );

  const { data: analyticsApiResponse } = useGetSuperAdminRevenueAnalyticsQuery(
    { period: analyticsPeriod === "weekly" ? "week" : "month" },
    { refetchOnMountOrArgChange: true }
  );

  // Dynamic clinics list from query
  const clinics = topClinicsApiResponse?.result ?? [];

  const planResult = planApiResponse?.result;
  const planData = planResult?.byPlan ?? [];
  const planTotalRevenue = planResult?.totalRevenue ?? 0;

  const analyticsResult = analyticsApiResponse?.result;
  const analyticsData = analyticsResult ? {
    thisMonth: {
      amount: analyticsResult.currentPeriod?.amount ?? 0,
      growthPercent: analyticsResult.currentPeriod?.growthPercent ?? 0,
    },
    lastMonth: {
      amount: analyticsResult.previousPeriod?.amount ?? 0,
      growthPercent: analyticsResult.previousPeriod?.growthPercent ?? 0,
    },
    allTime: {
      amount: analyticsResult.allTime?.amount ?? 0,
      growthPercent: analyticsResult.allTime?.growthPercent ?? 0,
    },
  } : {};

  const card1Label = analyticsPeriod === "weekly" ? "This Week" : "This Month";
  const card2Label = analyticsPeriod === "weekly" ? "Last Week" : "Last Month";
            ...
            <div className="col-span-12 md:col-span-6 lg:col-span-4">
              <div className="rounded-2xl border border-slate-200/80 bg-white p-4 shadow-sm">
                <div className="mb-4 flex items-center justify-between">
                  ...
                  <select
                    value={topClinicsPeriod}
                    onChange={(e) => setTopClinicsPeriod(e.target.value as any)}
                    className="rounded-lg border border-slate-200 px-2 py-1 text-xs font-medium text-slate-600 bg-white hover:bg-slate-50 outline-none cursor-pointer"
                  >
            ...
            <div className="col-span-12 xl:col-span-4">
              <div className="rounded-2xl border border-slate-200/80 bg-white p-4 shadow-sm">
                <div className="mb-3 flex items-center justify-between">
                  ...
                  <button
                    onClick={() => setPlanPeriod(prev => prev === "thisMonth" ? "thisYear" : "thisMonth")}
                  ...
                  >
                    {planPeriod === "thisMonth" ? "This Month" : "This Year"}
                ...
                <div className="flex items-center gap-4">
                  ...
                  <div className="relative flex h-28 w-28 shrink-0 items-center justify-center">
                    ...
                    <div className="absolute text-center">
                      ...
                      <p className="text-sm font-bold text-slate-900">{formatINR(planTotalRevenue)}</p>
                    </div>
                  </div>
                  ...
                  <div className="space-y-2 flex-1 min-w-0">
                    {planData.map((plan, idx) => {
                      const legendColors = ["bg-sky-500", "bg-teal-500", "bg-indigo-500", "bg-violet-500", "bg-fuchsia-500", "bg-rose-500"];
                      return (
                        <div key={plan.planName}>
                          <div className="flex items-center gap-2">
                            <div className={`h-2.5 w-2.5 rounded-full shrink-0 ${legendColors[idx % legendColors.length]}`} />
              ...
              <div className="rounded-2xl border border-slate-200/80 bg-white p-4 shadow-sm">
                ...
                <div className="grid grid-cols-3 gap-3">
                  ...
                  <div className="rounded-xl border border-slate-100 p-3">
                    <p className="text-[11px] text-slate-400">{card1Label}</p>
                  ...
                  {/* Last Month */}
                  <div className="rounded-xl border border-slate-100 p-3">
                    <p className="text-[11px] text-slate-400">{card2Label}</p>
                    <p className="mt-1 text-base font-bold text-slate-900">
