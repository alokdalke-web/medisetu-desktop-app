...
import { FiClock, FiInfo, FiLock, FiShield, FiUserCheck } from "react-icons/fi";
...
const securityHighlights = [
  ...
  { icon: FiShield, label: "Private" },
  { icon: FiUserCheck, label: "You're in control" },
];
...
const SignupEmail: React.FC = () => {
  ...
  const { control, handleSubmit, setValue } = useForm<SignupEmailDto>({
    resolver: zodResolver(signupEmailSchema),
    defaultValues: { email: locationState.email || "" },
  });
  ...
  const sendRegistrationLink = async (normalizedEmail: string) => {
    try {
      ...
      addToast({
        ...
        description: "Please check your mail for the registration link.",
        color: "success",
      });
    } catch (err: any) {
      addToast({
  ...
  const onSubmit = async (values: SignupEmailDto) => {
    ...
    await sendRegistrationLink(normalizedEmail);
  };
  ...
  const handleResend = () => {
    if (!submittedEmail || !isResendReady || isLoading) return;
    void sendRegistrationLink(submittedEmail);
  };
    ...
    <main className="relative flex min-h-dvh w-full items-center justify-center overflow-x-hidden bg-[#fcfefd] font-outfit lg:items-start lg:justify-start lg:overflow-hidden">
      ...
      >
        ...
        <img
          ...
          alt=""
          aria-hidden="true"
          className="pointer-events-none absolute right-[395px] bottom-[-300px] z-10 hidden h-[1200px] w-[286px] select-none object-contain min-[1650px]:block"
        />
        ...
        <div className="relative z-20 mx-auto w-full max-w-[493px] lg:absolute lg:right-[70px] lg:top-[50px] lg:mx-0 lg:w-[340px] lg:max-w-none lg:max-[1199px]:right-[45px] lg:max-[1199px]:top-[42px] lg:max-[1199px]:w-[320px]">
          <div className="mx-auto flex h-auto w-full flex-col items-center overflow-hidden rounded-[24px] border border-[#dfe9e8] bg-white/95 px-8 py-8 shadow-[0_18px_50px_rgba(10,108,116,0.14)] backdrop-blur-[4px] sm:px-[56px] sm:py-10 lg:h-[486px] lg:max-w-none lg:rounded-[16px] lg:px-[42px] lg:py-[28px] lg:max-[1199px]:px-[34px]">
            <div className="w-full text-center">
              <h2 className="text-[26px] font-semibold capitalize leading-[32px] text-primary sm:text-[28px] sm:leading-[35px] lg:text-[19px] lg:leading-[22px]">
                {submittedEmail ? "Check Your Email" : "Create Your Account"}
              </h2>
            ...
            {submittedEmail && (
              <div className="mt-4 flex justify-center lg:mt-[12px]">
                <div className="flex h-[50px] w-[50px] items-center justify-center rounded-full bg-[#EBF7F5]">
                  <svg
                    width="24"
                    height="24"
                    viewBox="0 0 24 24"
            ...
            <form
              ...
              className={[
                "flex w-full max-w-[381px] flex-col sm:gap-[20px] lg:w-[300px] lg:flex-1",
                submittedEmail
                  ? "mt-4 gap-[10px] sm:mt-6 lg:mt-[12px] lg:gap-[6px]"
                  : "mt-6 gap-[18px] sm:mt-7 lg:mt-[42px] lg:gap-[14px]",
              ].join(" ")}
              noValidate
            >
              {submittedEmail ? (
                <div className="flex w-full flex-col items-center text-center">
                  <div className="flex flex-col gap-1 tracking-normal">
                    <p className="text-[14px] font-normal leading-[18px] text-[#677294] sm:text-[15px] sm:leading-[20px] lg:text-[11.5px] lg:leading-[15px]">
                      We&apos;ve sent a registration link to
                    </p>
                    <p className="text-[15px] font-bold leading-[20px] text-[#100E1C] sm:text-[16px] lg:text-[12px] lg:leading-[16px]">
                      {submittedEmail}
                    </p>
                    <p className="mt-2 text-[12px] font-normal leading-[17px] text-[#677294] sm:text-[13px] sm:leading-[18px] lg:mt-[6px] lg:text-[10px] lg:leading-[14px]">
                  ...
                  <AppButton
                    ...
                    isLoading={isLoading}
                    isDisabled={Boolean(
                      isLoading || !isResendReady || countdown > 0,
                    )}
                    className="mt-6 h-[40px] w-full !rounded-xl text-[14px] font-semibold sm:h-[42px] sm:text-[15px] lg:mt-[12px] lg:h-[34px] lg:!rounded-lg lg:text-[11px]"
                  />

                  <button
                    type="button"
                    onClick={handleChangeEmail}
                    className="mt-4 cursor-pointer text-[13px] font-medium text-primary underline hover:opacity-90 sm:text-[14px] lg:mt-[8px] lg:text-[10.5px]"
                  >
                    Use a different email
                  </button>

                  <div className="mt-5 w-full rounded-[14px] border border-[#dfe9e8] bg-[#f6fbfb] px-4 py-3 shadow-[0_8px_24px_rgba(10,108,116,0.06)] lg:mt-[12px] lg:rounded-[10px] lg:px-[12px] lg:py-[9px]">
                    <div className="flex items-start gap-3 text-left lg:gap-[8px]">
                      <span className="grid h-8 w-8 shrink-0 place-items-center rounded-full bg-white text-primary shadow-[0_4px_12px_rgba(10,108,116,0.08)] lg:h-[23px] lg:w-[23px]">
                        <FiInfo className="h-4 w-4 lg:h-[12px] lg:w-[12px]" />
                      </span>
                      <div className="flex min-w-0 flex-col gap-1 tracking-normal lg:gap-[2px]">
                        <span className="text-[13px] font-semibold leading-[16px] text-[#100E1C] sm:text-[14px] lg:text-[10px] lg:leading-[12px]">
                          Registration link sent
                        </span>
                        <span className="text-[12px] font-normal leading-[17px] text-[#677294] sm:text-[13px] lg:text-[9px] lg:leading-[12px]">
                          Open the email on the same device and complete signup before the private link expires.
                        </span>
                      </div>
                    </div>

                    <div className="mt-3 border-t border-[#dfe9e8] pt-3 lg:mt-[8px] lg:pt-[8px]">
                      <div className="mb-3 flex items-center justify-center gap-2 text-primary lg:mb-[8px] lg:gap-[6px]">
                        <FiClock className="h-4 w-4 lg:h-[12px] lg:w-[12px]" />
                        <span className="text-[13px] font-semibold leading-[16px] sm:text-[14px] lg:text-[10px] lg:leading-[12px]">
                      ...
                      <div className="grid grid-cols-3">
                        {securityHighlights.map((item, index) => {
                          ...
                          return (
                            <div
                              key={item.label}
                              className={`flex min-w-0 flex-col items-center gap-2 px-2 text-center lg:gap-[5px] lg:px-[6px] ${
                                index > 0 ? "border-l border-[#dfe9e8]" : ""
                              }`}
                            >
                              <Icon className="h-5 w-5 shrink-0 text-primary lg:h-[14px] lg:w-[14px]" />
                              <span className="text-[11px] font-semibold leading-[13px] text-[#677294] sm:text-[12px] lg:text-[8.5px] lg:leading-[10px]">
                                {item.label}
                              </span>
                            </div>
                          );
        ...
        "
                  ...
                  <div className="rounded-[14px] border border-[#dfe9e8] bg-[#f6fbfb] px-4 py-3 shadow-[0_8px_24px_rgba(10,108,116,0.06)] lg:rounded-[10px] lg:px-[12px] lg:py-[9px]">
                    <div className="flex items-start gap-3 lg:gap-[8px]">
                      <span className="grid h-8 w-8 shrink-0 place-items-center rounded-full bg-white text-primary shadow-[0_4px_12px_rgba(10,108,116,0.08)] lg:h-[23px] lg:w-[23px]">
                        <FiInfo className="h-4 w-4 lg:h-[12px] lg:w-[12px]" />
                      </span>
                      <div className="flex min-w-0 flex-col gap-1 tracking-normal lg:gap-[2px]">
                        ...
                        <span className="text-[12px] font-normal leading-[17px] text-[#677294] sm:text-[13px] lg:text-[9px] lg:leading-[12px]">
                          We&apos;ll email a private signup link that expires in 15 minutes for your protection.
                        </span>
                      </div>
                    </div>
              ...
              <div className="mt-6 lg:mt-auto flex flex-col gap-2 w-full text-center">
