...
import { useLazyGetAllClinicsQuery } from "../../redux/api/clinicApi";
...
const Login: React.FC = () => {
  ...
  const [triggerGetAllClinics] = useLazyGetAllClinicsQuery();
  ...
  const handleGoogleSuccess = async (cr: CredentialResponse) => {
    ...
    try {
      ...
      // Fetch clinic and user profile info if the logged-in user is an Admin or Doctor
      const u = data.user;
      if (u?.userType === "Admin" || u?.userType === "Doctor") {
        try {
          await triggerGetAllClinics().unwrap();
        } catch (err) {
          console.error("Failed to pre-fetch clinic/user info after social login:", err);
        }
      }
    ...
    try {
      ...
      if (u?.userType === "Admin" || u?.userType === "Doctor") {
        try {
          ...
          console.error("Failed to pre-fetch clinic/user info after login:", err);
        }
      }
