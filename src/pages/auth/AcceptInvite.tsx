...
const INVITE_DESIGN_WIDTH = 1024;
const INVITE_DESIGN_HEIGHT = 635;
...
const AcceptInvite: React.FC = () => {
  ...
  const [inviteCanvas, setInviteCanvas] = useState<{ scale: number; width: number | null }>({
    scale: 1,
    width: null,
  });
  ...
  useEffect(() => {
    const updateScale = () => {
      if (window.innerWidth < 1024) {
        setInviteCanvas({ scale: 1, width: null });
        return;
      }

      const nextScale = Math.min(
        window.innerWidth / INVITE_DESIGN_WIDTH,
        window.innerHeight / INVITE_DESIGN_HEIGHT,
      );
  ...
  const onSubmit = handleSubmit(async (values: FormValues) => {
    ...
    try {
      ...
      addToast({
        color: "success",
        title: data?.message || "Invitation accepted successfully",
      });
  ...
  return (
    <main className="relative flex min-h-dvh w-full items-center justify-center overflow-x-hidden bg-[#fcfefd] font-outfit lg:items-start lg:justify-start lg:overflow-hidden">
      <div
        ...
        style={
          inviteCanvas.width
            ? { width: `${inviteCanvas.width}px`, transform: `scale(${inviteCanvas.scale})` }
            : undefined
        }
      >
        <img
        ...
        <section className="absolute left-[100px] top-[99px] z-10 hidden w-[430px] lg:block">
          <div className="tracking-normal">
            <h1 className="text-[23px] font-semibold capitalize leading-[29px] text-[#100E1C]">
              <span className="text-primary">You&apos;ve Been Invited </span>
              <span>To Join The Team</span>
            </h1>
            <p className="mt-[8px] text-[10.5px] font-normal capitalize leading-[19px] text-[#677294]">
              Accept your invitation and set a password to start managing appointments, patients, billing, and AI-powered workflows.
            </p>
          </div>
        ...
        <img
          ...
          alt=""
          aria-hidden="true"
          className="pointer-events-none absolute right-[420px] bottom-[-300px] z-10 hidden h-[1200px] w-[286px] select-none object-contain min-[1650px]:block"
        />
