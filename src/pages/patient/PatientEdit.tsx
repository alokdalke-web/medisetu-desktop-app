...
import type {
  PatientEditFormValues as FormValues,
  GenderOpt,
} from "../../types/patient";
      ...
      <div className="mx-auto w-full ">
        ...
        <div className="mb-5">
          ...
          <PageBackNav
            ...
            crumbs={[
              ...
              { label: patient?.name || "Details", to: `/patient/${id}` },
              { label: "Edit" },
            ]}
            className="mt-1"
          />
        ...
        <div className="flex gap-5 items-start">
          ...
          <div className="hidden lg:flex flex-col gap-4 w-[280px] shrink-0 sticky top-4 self-start">
            ...
            <div className="rounded-2xl border border-line bg-surface p-5 dark:bg-[#111726] dark:border-[#273244]">
              <div className="flex flex-col items-center text-center">
                <img
                  ...
                  className="h-[72px] w-[72px] rounded-full object-cover border-2 border-line"
                />
              ...
              <div className="mt-5 space-y-3">
                <div className="flex items-center justify-between text-sm">
                  <span className="text-[#677294] dark:text-text-muted">Patient ID</span>
                  <span className="font-medium text-text">
                    {patient?.patientId ?? patient?.patientCode ?? `PT-${String(id).slice(0, 4)}`}
                  </span>
                </div>
                <div className="flex items-center justify-between text-sm">
                  <span className="text-[#677294] dark:text-text-muted">Age</span>
                  <span className="font-medium text-text">
                    {watchedAge ? `${watchedAge} Years` : "—"}
                  </span>
                </div>
                <div className="flex items-center justify-between text-sm">
                  <span className="text-[#677294] dark:text-text-muted">Gender</span>
                  <span className="font-medium text-text">
                    {watchedGender || "—"}
                  </span>
                </div>
                <div className="flex items-center justify-between text-sm">
                  <span className="text-[#677294] dark:text-text-muted">Phone</span>
                  <span className="font-medium text-text">
                    {watchedMobile || "—"}
                  </span>
                </div>
                <div className="flex items-center justify-between text-sm">
                  <span className="text-[#677294] dark:text-text-muted">Address</span>
                  <span className="font-medium text-text text-right max-w-[140px] truncate">
                    {[watchedCity, watchedState].filter(Boolean).join(", ") || "—"}
                  </span>
                </div>
                <div className="flex items-center justify-between text-sm">
                  <span className="text-[#677294] dark:text-text-muted">Blood Group</span>
                  <span className="font-medium text-text">
                    {watchedBloodGroup || "—"}
                  </span>
                </div>
                <div className="flex items-center justify-between text-sm">
                  <span className="text-[#677294] dark:text-text-muted">Height / Weight</span>
                  <span className="font-medium text-text">
                ...
                <div className="flex items-center justify-between text-sm">
                  <span className="text-[#677294] dark:text-text-muted">Registered On</span>
                  <span className="font-medium text-text">
                    {patient?.createdAt
            ...
            <div className="rounded-2xl border border-line bg-surface p-5 dark:bg-[#111726] dark:border-[#273244]">
              <div className="flex items-center gap-2 mb-2">
                ...
                <h4 className="text-sm font-semibold text-text">Quick Tips</h4>
              </div>
