import { addToast, Select, SelectItem, Modal, ModalContent, ModalHeader, ModalBody, ModalFooter, Button } from "@heroui/react";
...
const InlineAddPatientForm: React.FC<Props> = ({ queryText, onCreated, onCancel }) => {
  ...
  return (
    <div ref={formContainerRef} className="mt-3 overflow-hidden rounded-xl border border-teal-100 bg-white shadow-sm dark:border-[#273244] dark:bg-[#0f1728]">
      ...
      {/* ── Form body ── */}
      {/* Every field in the two grids is a different HeroUI component (Input,
          Select, Autocomplete) and each brings its own label metrics, so the
          left and right columns started their controls a few pixels apart on
          each row. One label contract for all of them — 12px, 16px line, 4px
          gap — puts every control in a row on the same line. */}
      <div className="px-4 pb-4 pt-4 [&_[data-slot='input-wrapper']]:!rounded-lg [&_[data-slot='trigger']]:!rounded-lg [&_[data-slot='label']]:!mb-1 [&_[data-slot='label']]:!p-0 [&_[data-slot='label']]:!text-[12px] [&_[data-slot='label']]:!leading-4">
            ...
            <Controller
              ...
              render={({ field, fieldState }) => (
                <div className="flex flex-col">
                  {/* Matches InputLabel's metrics (12px / 16px line / 4px gap) so the control
                      lines up with Age on the same row. Deliberately NOT a `<label htmlFor>`
                      paired to an `id` on the Select: that bound the text to the hidden native
                      <select> React Aria renders, giving the field a second click target that
                      swallowed the click instead of opening the option list. `aria-label` on
                      the Select keeps it named for assistive tech. */}
                  <span className="mb-1 block font-outfit text-[12px] font-medium leading-4 text-text-muted">
