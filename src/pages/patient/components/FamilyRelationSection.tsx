...
import type {
  DuplicateFamilyCandidate,
  DuplicateFamilyResult,
  FamilyRelationSectionProps,
  FamilyRelationSectionRef,
} from "../../../types/patient";
...
// Kept co-located with RELATIONSHIP_OPTIONS (its source of truth) rather than
// moved to types/patient/ — see the note in that file.
export type FamilyRelationship = (typeof RELATIONSHIP_OPTIONS)[number]["value"];

export type { DuplicateFamilyCandidate, DuplicateFamilyResult, FamilyRelationSectionRef };
...
const detailsMatch = (
  ...
  return Boolean(candidateName) && candidateName === normalizeText(existing.name);
};

const findDuplicateFamilyMember = (
  foundPatient: PatientSearchItem | null,
  candidate: DuplicateFamilyCandidate,
): DuplicateFamilyResult | null => {
  if (!foundPatient) return null;

  if (detailsMatch(candidate, foundPatient)) {
    return {
      name: foundPatient.name,
      message: `${foundPatient.name} is already registered with this mobile number.`,
    };
  }

  const duplicate = (foundPatient.familyMembers ?? []).find((member) =>
    detailsMatch(candidate, member),
  );
  ...
  return {
    ...
    message: `${duplicate.name} is already added as ${duplicate.relationship}.`,
  };
};
...
>(
  ...
  useEffect(() => {
    onFamilyContextChange?.();
  }, [foundPatient, onFamilyContextChange]);
  ...
  useImperativeHandle(ref, () => ({
    ...
    getDuplicateFamilyMember: (candidate) =>
      findDuplicateFamilyMember(foundPatient, candidate),
  }));
  ...
  if (isFetching) {
    return (
      <div className="flex items-center gap-2 rounded-xl border border-slate-100 bg-slate-50/50 px-4 py-3">
        ...
        <span className="text-[13px] text-text-muted">Checking mobile…</span>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-3">
      {/* Patient info card */}
      <div className="flex items-center gap-3 rounded-lg border border-slate-200 bg-surface px-3 py-2.5">
        ...
        <div className="min-w-0 flex-1">
          ...
          <p className="mt-0.5 flex flex-wrap items-center gap-x-2 text-[11px] font-medium text-text-muted">
      ...
      {familyMembers.length > 0 && (
        <div className="flex flex-wrap items-center gap-2">
          <span className="text-[11px] font-semibold text-text-muted">Family:</span>
