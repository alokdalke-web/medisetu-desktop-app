...
import type { PatientFormSidebarProps } from "../../../types/patient";
...
const PatientFormSidebar: React.FC<PatientFormSidebarProps> = ({
  ...
  return (
    <aside className="hidden lg:flex flex-col gap-4 w-[280px] xl:w-[320px] shrink-0 sticky top-4 self-start">
      {/* ─── Live Preview Card ─── */}
      <div className="rounded-2xl border border-slate-200 bg-surface p-5 shadow-sm dark:bg-[#111726] dark:border-[#273244]">
        <div className="flex flex-col items-center text-center">
          ...
          <h3 className="mt-3 text-[15px] font-semibold text-text max-w-full truncate">
            {displayName}
          </h3>
        ...
        <div className="mt-4 space-y-2.5 border-t border-line pt-4">
          {watchedMobile && (
            <div className="flex items-center gap-2 text-[13px]">
              <FiPhone className="shrink-0 text-text-muted" />
              <span className="text-text font-medium">+91 {watchedMobile}</span>
            </div>
          )}
          {location && (
            <div className="flex items-center gap-2 text-[13px]">
              <span className="shrink-0 text-text-muted text-[14px]">📍</span>
              <span className="text-text font-medium truncate">{location}</span>
            </div>
          )}
      ...
      <div className="rounded-2xl border border-slate-200 bg-surface p-5 shadow-sm dark:bg-[#111726] dark:border-[#273244]">
        <div className="flex items-center justify-between mb-3">
          <h4 className="text-[13px] font-semibold text-text">Form Completion</h4>
          <span className={`text-[12px] font-bold ${clampedPercent === 100 ? "text-emerald-600" : "text-text-muted"}`}>
            {clampedPercent}%
          </span>
        </div>
...
const CheckItem: React.FC<{ label: string; done: boolean }> = ({ label, done }) => (
  <div className="flex items-center gap-2">
    ...
    <span
      className={`text-[12px] ${done ? "text-text font-medium" : "text-text-muted"}`}
    >
      {label}
    </span>
  </div>
...
const TipItem: React.FC<{ icon: React.ReactNode; text: string }> = ({ icon, text }) => (
  <div className="flex items-start gap-2.5">
    ...
    <p className="text-[11px] leading-[1.5] text-text-muted">{text}</p>
  </div>
);
