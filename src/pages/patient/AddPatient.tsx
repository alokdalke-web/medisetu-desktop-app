...
import type {
  AddPatientFormValues as FormValues,
  GenderOpt,
  VoicePatientForm,
} from "../../types/patient";
...
const AddPatient: React.FC = () => {
  ...
  const duplicateNameErrorRef = useRef(false);
  ...
  const [duplicateConfirmation, setDuplicateConfirmation] = useState({
    isOpen: false,
    existingPatient: null as any,
    payload: null as any,
  });
  const [familyContextVersion, setFamilyContextVersion] = useState(0);

  // Watch mobile so FamilyRelationSection can react to changes
  const mobileValue = useWatch({ control, name: "mobile" }) as string ?? "";
  const linkFamilyValue = Boolean(useWatch({ control, name: "linkFamily" }));
  const relationshipValue = useWatch({ control, name: "relationship" }) as string ?? "";

  const handleFamilyContextChange = React.useCallback(() => {
    setFamilyContextVersion((version) => version + 1);
  }, []);
  ...
  useEffect(() => {
    ...
    if (!name || !linkFamilyValue) {
      if (duplicateNameErrorRef.current) {
        duplicateNameErrorRef.current = false;
        clearErrors("name");
      }
      return;
    }
    ...
    if (duplicateFamilyMember) {
      duplicateNameErrorRef.current = true;
      setError("name", {
        type: "manual",
  ...
  // Load all cities/states on mount for voice parsing
  useEffect(() => {
    const loadCities = async () => {
      ...
      try {
        ...
        if (res.ok) {
          ...
          if (remoteData && remoteData.cities) {
            data = remoteData;
          }
        }
      } catch (e) {
        // Fallback to local data in public folder
      }
    ...
    <div className="min-h-[calc(100vh-64px)] w-full bg-slate-50/70 dark:bg-transparent">
      <div className="mx-auto w-full">
        <div className="mb-5 flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
          <div>
            <h1 className="text-[22px] font-semibold text-text sm:text-2xl">
              New Patient
            </h1>
        ...
        {(listening || transcript) && (
          <div className="mb-4 rounded-2xl border border-slate-200 bg-surface px-4 py-3">
        ...
        <div className="flex gap-5 items-start">
          <form
            ...
            dark:[&_label]:!text-text-subtle

            [&_[data-slot='input-wrapper']]:!rounded-lg
            [&_[data-slot='input-wrapper']]:!border
            [&_[data-slot='input-wrapper']]:!border-line
            [&_[data-slot='input-wrapper']]:!bg-surface
            ...
            [&_[data-slot='trigger']]:!border-line
            [&_[data-slot='trigger']]:!bg-surface
            ...
            dark:[&_[data-slot='input']]:!text-text-subtle
          "
          >
            <PatientFormSections
              control={rhfControl}
                  ...
                  render={({ field }) => {
                    ...
                    return (
                      <div className="w-full">
                        <label className="mb-2 block text-[12px] font-semibold text-text">
                          Address
                        </label>
            ...
            <div className="mt-5">
              <FamilyRelationSection
                ...
                onFamilyContextChange={handleFamilyContextChange}
                relationshipError={
            ...
            {/* Footer Buttons */}
            <div className="mt-6 flex items-center justify-between border-t border-line px-2 py-4">
              <button
                type="button"
                onClick={resetForm}
                disabled={isCreating}
      ...
      <Modal
        ...
        onOpenChange={(open) => {
          if (!open) setDuplicateConfirmation({ isOpen: false, existingPatient: null, payload: null });
        }}
        size="md"
      >
        <ModalContent>
          {(onCloseDup) => (
            <>
              <ModalHeader>Possible Duplicate Patient</ModalHeader>
              <ModalBody>
                <div className="flex flex-col gap-2">
                  <p>A patient with this phone number already exists:</p>
                  <div className="p-3 bg-default-100 rounded-md">
                    <strong>{duplicateConfirmation.existingPatient?.name}</strong> <br/>
                    {duplicateConfirmation.existingPatient?.phone}
                  </div>
                  <p>Create a new patient anyway?</p>
                </div>
              </ModalBody>
              <ModalFooter>
                <Button variant="light" onPress={onCloseDup}>
                  Cancel
                </Button>
                <AppButton
                  color="primary"
