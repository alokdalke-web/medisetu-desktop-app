...
import { FreeTrialOfferBanner } from "../../components/subscription/FreeTrialOfferBanner";
...
const UsageCard = ({ label, current, total, icon, onUpgrade, iconBg, iconColor, unit }: {
  label: string; current: number; total: number | null; icon: React.ReactNode; onUpgrade?: () => void;
  iconBg?: string; iconColor?: string;
  /**
   * When set, this card shows a plan-configured duration (e.g. data-retention months)
   * rather than an accumulating usage count — rendered as "{total} {unit}" with no
   * fraction/progress bar, since `current` has no meaningful value for these features.
   */
  unit?: string;
}) => {
  ...
  if (unit) {
    return (
      <div className="space-y-2.5">
        <span className="flex items-center gap-2 text-[11px] font-medium text-default-500">
          <span className={`flex h-6 w-6 shrink-0 items-center justify-center rounded-lg ${iconBg ?? "bg-default-100 dark:bg-default-50/50"} ${iconColor ?? "text-default-500"}`}>
            {icon}
          </span>
          {label}
        </span>
        <p className="text-[15px] font-bold text-default-900 dark:text-white">
          {unlimited ? "Unlimited" : `${total} ${unit}`}
        </p>
        <p className="text-[10px] text-default-400 font-medium">Retention window</p>
      </div>
    );
  }
...
const Subscription = () => {
  ...
  const pendingExternalPaymentRef = useRef(false);
  ...
  const [successExpiryDate, setSuccessExpiryDate] = useState<string | undefined>(undefined);
  const [successPlanName, setSuccessPlanName] = useState<string | undefined>(undefined);
  const [successIsFreeTrial, setSuccessIsFreeTrial] = useState(true);
  ...
  const eligibleForFreeTrial =
    userData?.noSubscriptionTakenTillNow === true || currentUser?.noSubscriptionTakenTillNow === true;

  const handleShowTrialSuccessModal = (expiryDate?: string | null) => {
    setSuccessExpiryDate(expiryDate || undefined);
    setSuccessPlanName(undefined);
    setSuccessIsFreeTrial(true);
    setShowSuccessModal(true);
  };

  /** Shown after a paid plan purchase completes — same modal, purchase-flavored copy. */
  const handleShowPlanSuccessModal = (planName: string, expiryDate?: string | null) => {
    setSuccessExpiryDate(expiryDate || undefined);
    setSuccessPlanName(planName);
    setSuccessIsFreeTrial(false);
    setShowSuccessModal(true);
  };
  ...
  const { data: autoRenewData, refetch: refetchAutoRenewStatus } = useGetAutoRenewStatusQuery();

  // AutoPay authorization happens in a separate Razorpay tab — refetch
  // /clinic/user (+ subscription/user) as soon as the user comes back to this
  // tab so the plan shows as active without a manual page refresh.
  useEffect(() => {
    const handleVisibilityChange = () => {
      if (document.visibilityState === "visible" && pendingExternalPaymentRef.current) {
        pendingExternalPaymentRef.current = false;
        refetchClinics();
        refetchMySubscription();
        refetchAutoRenewStatus();
        refetchUser();
      }
    };
  ...
  }, [refetchClinics, refetchMySubscription, refetchAutoRenewStatus, refetchUser]);
  ...
  const handleSelectPlan = async (plan: Plan) => {
    ...
    if (isFreePlanTarget) {
      ...
      if (success) await Promise.all([refetchMySubscription(), refetchClinics(), refetchUser()]);
    ...
    try {
      if (useAutoPay && !coupon) {
        ...
        if (res.data?.shortUrl) {
          pendingExternalPaymentRef.current = true;
        ...
        if (success) {
          const [subResult] = await Promise.all([refetchMySubscription(), refetchClinics(), refetchUser()]);
          planCheckoutModal.onClose();
          const freshExpiry = (subResult as any)?.data?.data?.subscription?.expiresAt;
          handleShowPlanSuccessModal(plan.name, freshExpiry);
        }
      }
    } catch (err: any) {
      addToast({
  ...
  const executeCheckout = async () => {
    ...
    if (cartPlan.slug.toLowerCase() !== currentSlug) {
      ...
      const [subResult] = await Promise.all([refetchMySubscription(), refetchClinics(), refetchUser()]);
      const freshExpiry = (subResult as any)?.data?.data?.subscription?.expiresAt;
      handleShowPlanSuccessModal(cartPlan.name, freshExpiry);
    }
  ...
  return (
    <div id="tour-admin-subscription-page" className="mx-auto w-full space-y-6 pb-24 sm:pb-6">
      ...
      <section className={`relative overflow-hidden rounded-2xl border px-5 py-5 shadow-sm dark:shadow-none sm:px-6 ${isFreePlan ? "border-gray-200 bg-white dark:border-[#273244] dark:bg-[#111726]" : "border-primary/20 bg-gradient-to-br from-primary/5 via-white to-white dark:from-primary/10 dark:via-[#111726] dark:to-[#111726]"}`}>
        {!isFreePlan && (
        ...
        <div className="relative flex flex-col gap-5 md:flex-row md:items-center md:justify-between">
          ...
          <div className="space-y-2.5">
            <div className="flex items-center gap-2.5">
              {!isFreePlan && (
                <span className="flex h-7 w-7 items-center justify-center rounded-full bg-amber-400/15 ring-1 ring-amber-400/30">
                  <PiCrownSimpleFill className="text-amber-500 text-[15px]" />
                </span>
              )}
          ...
          <div className="flex items-center justify-between gap-4 sm:justify-start sm:gap-5 md:flex-col md:items-end md:justify-start md:gap-2">
            ...
            {isFreePlan ? (
              <button type="button" onClick={scrollToPlans} className="inline-flex shrink-0 items-center gap-1.5 rounded-lg bg-primary px-4 py-2 text-[13px] font-medium text-white shadow-sm hover:bg-primary-active transition active:scale-[0.97]">
      ...
      {/* ═══ Free trial offer (only free-plan users who haven't taken a subscription yet) ═══ */}
      <FreeTrialOfferBanner
        showFreeOffer={isFreePlan && eligibleForFreeTrial}
        onShowSuccessModal={handleShowTrialSuccessModal}
      />

      {/* ═══ SECTION 2: Usage ═══ */}
      <section className="space-y-4 rounded-2xl border border-gray-200 bg-white p-5 shadow-sm dark:border-[#273244] dark:bg-[#111726] dark:shadow-none">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2">
            <span className="flex h-7 w-7 items-center justify-center rounded-full bg-primary/10 dark:bg-primary/15">
        ...
        <div className="grid grid-cols-2 gap-x-8 gap-y-5 sm:grid-cols-4">
          <UsageCard label="Doctors" icon={<FiUser className="text-[11px]" />} onUpgrade={scrollToPlans}
            iconBg="bg-[rgba(99,102,241,0.1)] dark:bg-[#1d2440]" iconColor="text-[#6366f1]"
          ...
          <UsageCard label="Staff" icon={<FiUsers className="text-[11px]" />} onUpgrade={scrollToPlans}
            iconBg="bg-[rgba(40,152,255,0.1)] dark:bg-[#172b48]" iconColor="text-[#2898ff]"
            current={myUsage?.staff?.current ?? 0}
            total={myUsage?.staff?.isUnlimited ? null : (myUsage?.staff?.limit ?? null)} />
          <UsageCard label="Data Retention" icon={<FiDatabase className="text-[11px]" />} unit="months"
            iconBg="bg-primary/10 dark:bg-primary/15" iconColor="text-primary"
            current={0}
            total={limitations?.limits.find((l) => l.featureKey === "storage_months")?.totalLimit ?? null} />
          <UsageCard label="Payment History" icon={<FiCalendar className="text-[11px]" />} unit="months"
            iconBg="bg-[rgba(232,155,0,0.1)] dark:bg-[#332716]" iconColor="text-[#e89b00]"
            current={0}
      ...
      {!isFreePlan && (
        <section className="space-y-5 rounded-2xl border border-gray-200 bg-white p-5 shadow-sm dark:border-[#273244] dark:bg-[#111726] dark:shadow-none">
          <div className="flex items-center gap-2">
            <span className="flex h-7 w-7 items-center justify-center rounded-full bg-[rgba(99,102,241,0.1)] dark:bg-[#1d2440]">
              <FiPackage className="h-3.5 w-3.5 text-[#6366f1]" />
            </span>
          ...
          <div className="grid grid-cols-1 gap-8 lg:grid-cols-2">
            ...
            <div className="space-y-3">
              ...
              {activeAddOns.length === 0 ? (
                <div className="rounded-lg border border-dashed border-gray-200 dark:border-[#273244] px-4 py-6 text-center">
                ...
                <div className="divide-y divide-gray-100 dark:divide-[#273244] rounded-lg border border-gray-200 dark:border-[#273244]">
                  {activeAddOns.map((addon) => (
                    <div key={addon.featureKey} className="flex items-center gap-3 px-3 py-2.5">
                      <div className="flex h-7 w-7 shrink-0 items-center justify-center rounded-lg bg-[rgba(99,102,241,0.1)] dark:bg-[#1d2440]">
                        <FiPackage className="text-[#6366f1] text-[11px]" />
                      </div>
            ...
            <div className="space-y-4">
              <div>
                ...
                <div className="rounded-lg border border-gray-200 dark:border-[#273244] px-4 py-3 space-y-2">
                  {cancelAtPeriodEnd ? (
                    ...
                    <>
                      ...
                      <div className="flex items-center justify-between border-t border-gray-100 dark:border-[#273244] pt-2 mt-1">
      ...
      {!isFreePlan && (
        <section className="space-y-4 rounded-2xl border border-gray-200 bg-white p-5 shadow-sm dark:border-[#273244] dark:bg-[#111726] dark:shadow-none">
          <div className="flex items-center gap-2">
            <span className="flex h-7 w-7 items-center justify-center rounded-full bg-[rgba(40,152,255,0.1)] dark:bg-[#172b48]">
              <FiRefreshCw className="h-3.5 w-3.5 text-[#2898ff]" />
            </span>
            <h3 className="text-[15px] font-semibold text-default-900 dark:text-white">Manage Subscription</h3>
          </div>
          <div className="grid grid-cols-2 gap-2 sm:grid-cols-3 lg:grid-cols-5">
            {[
              { key: "upgrade", label: "Upgrade Plan", desc: "Move to a higher tier", icon: <FiTrendingUp />, onClick: scrollToPlans, iconBg: "bg-[rgba(39,183,122,0.1)] dark:bg-[#16352f]", iconColor: "text-[#27b77a]" },
              { key: "addons", label: "Manage Add-ons", desc: "Adjust resources", icon: <FiPackage />, onClick: () => downgradeModal.onOpen(), iconBg: "bg-[rgba(99,102,241,0.1)] dark:bg-[#1d2440]", iconColor: "text-[#6366f1]" },
              { key: "autopay", label: "Manage AutoPay", desc: autoRenew ? "Enabled" : "Disabled", icon: <FiRefreshCw />, onClick: autoPayModal.onOpen, iconBg: "bg-primary/10 dark:bg-primary/15", iconColor: "text-primary" },
              { key: "add", label: "Add Resources", desc: "Buy add-ons", icon: <FiPlus />, onClick: addResourcesDrawer.onOpen, iconBg: "bg-[rgba(232,155,0,0.1)] dark:bg-[#332716]", iconColor: "text-[#e89b00]" },
              cancelAtPeriodEnd
                ? { key: "resume", label: "Resume Plan", desc: "Re-enable renewal", icon: <FiRefreshCw />, onClick: handleUndoCancellation, danger: false, iconBg: "bg-[rgba(39,183,122,0.1)] dark:bg-[#16352f]", iconColor: "text-[#27b77a]" }
                : { key: "cancel", label: "Cancel Plan", desc: "End at period close", icon: <FiXCircle />, onClick: () => cancelConfirmModal.onOpen(), danger: true, iconBg: "bg-[rgba(229,72,77,0.1)] dark:bg-[#332022]", iconColor: "text-[#e5484d]" },
            ].map((a: any) => (
              <button key={a.key} type="button" onClick={a.onClick}
                className="flex items-start gap-2.5 rounded-xl border border-gray-200 dark:border-[#273244] px-3 py-3 text-left transition hover:border-gray-300 hover:bg-default-50/50 dark:hover:bg-[#151e31] active:scale-[0.98]">
                <span className={`flex h-7 w-7 shrink-0 items-center justify-center rounded-lg text-sm ${a.iconBg} ${a.iconColor}`}>{a.icon}</span>
      ...
      <section className="space-y-4 rounded-2xl border border-gray-200 bg-white p-5 shadow-sm dark:border-[#273244] dark:bg-[#111726] dark:shadow-none">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2">
            <span className="flex h-7 w-7 items-center justify-center rounded-full bg-[rgba(40,152,255,0.1)] dark:bg-[#172b48]">
              <FiFileText className="h-3.5 w-3.5 text-[#2898ff]" />
            </span>
        ...
        {recentInvoices.length === 0 ? (
          <div className="rounded-lg border border-dashed border-gray-200 dark:border-[#273244] px-4 py-10 text-center">
          ...
          <>
              ...
              {recentInvoices.map((item: any) => {
                ...
                return (
                  <button
                    ...
                    className="flex w-full items-center gap-3 rounded-xl border border-gray-200 dark:border-[#273244] px-3.5 py-3 text-left transition active:scale-[0.99] active:bg-default-50/60 dark:active:bg-[#151e31]"
                  >
                    <div className="flex h-9 w-9 shrink-0 items-center justify-center rounded-lg bg-[rgba(40,152,255,0.1)] dark:bg-[#172b48]">
                      <FiFileText className="text-[#2898ff] text-[13px]" />
                    </div>
            ...
            {/* Tablet+: table */}
            <div className="hidden overflow-hidden rounded-lg border border-gray-200 dark:border-[#273244] sm:block">
              <div className="overflow-x-auto">
                <table className="min-w-full text-[14px] text-left">
                  <thead className="border-b border-gray-200 bg-gray-50/80 dark:border-[#273244] dark:bg-[#0f1728]">
                  ...
                  <tbody>
                    {recentInvoices.map((item: any, idx: number) => {
                      ...
                      return (
                        <tr
                          key={item.id}
                          className={`transition-colors hover:bg-gray-50/70 dark:hover:bg-[#151e31] ${idx !== recentInvoices.length - 1 ? "border-b border-gray-100 dark:border-[#273244]" : ""}`}
                        >
                          ...
                          <td className="px-5 py-3.5 text-[13px] font-semibold text-default-800 dark:text-default-200">{safeFormatMoney(Number(item.price - item.discount), item.currency || "INR")}</td>
                          ...
                          <td className="px-5 py-3.5 text-right">
                            <button type="button" onClick={() => handleViewInvoice(item)} className="inline-grid h-8 w-8 place-items-center rounded-lg border border-gray-200 bg-white text-default-500 shadow-sm transition hover:bg-primary/5 hover:text-primary hover:border-primary/30 dark:border-[#273244] dark:bg-[#0f1728]" aria-label="Download receipt">
      ...
      <section ref={plansRef} className="scroll-mt-6">
        <Accordion
          ...
          className="rounded-2xl border-gray-200 bg-white shadow-sm dark:border-[#273244] dark:bg-[#111726] dark:shadow-none px-0"
        >
          <AccordionItem
            key="plans"
          ...
          >
            ...
            <PlanComparison
              ...
              onSwitchToYearly={() => setBillingCycle("yearly")}
      ...
      <section>
        ...
        >
          <AccordionItem
            ...
            title={
              <div className="flex items-center gap-2">
                <span className="flex h-7 w-7 items-center justify-center rounded-full bg-[rgba(232,155,0,0.1)] dark:bg-[#332716]">
                  <FiCalendar className="h-3.5 w-3.5 text-[#e89b00]" />
                </span>
                Activity Timeline
              </div>
            }
      ...
      {autoPayModal.isOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4" onClick={autoPayModal.onClose}>
          <div className="w-full max-w-md" onClick={(e) => e.stopPropagation()}>
            <AutoPayManagement
              autoRenew={autoRenew}
              pendingAuthorization={autoPayPending}
              expiresAt={expiresAt}
              ...
              onRefreshStatus={() => { refetchAutoRenewStatus(); refetchMySubscription(); }}
            />
          </div>
        </div>
      )}
      ...
      <FreeTrialSuccessModal
        ...
        expiryDate={successExpiryDate}
        planName={successPlanName}
        isFreeTrial={successIsFreeTrial}
      />

      {/* ═══ Mobile sticky action bar ═══ */}
      {isFreePlan && (
        <div className="fixed bottom-0 inset-x-0 z-40 border-t border-gray-200 dark:border-[#273244] bg-white/95 dark:bg-[#111726]/95 backdrop-blur px-4 py-3 sm:hidden">
