...
import { BackendConfigModal } from "./components/shared/BackendConfigModal";
...
function App() {
  ...
  const isSwitchToPhoneRoute = location.pathname === "/switch-to-phone";
  const isPendingAdminOnboarding =
    userType === "Admin" && normalizeStatus(user?.userStatus) === "pending";

  const { data: clinics } = useGetAllClinicsQuery(undefined, {
    skip: userType !== "Admin" || isClinicSetupRoute || isPendingAdminOnboarding,
  });
  ...
  return (
    <AppLoaderProvider>
      <LimitationsProvider>
        ...
        <AppRoutes />
        {!isSwitchToPhoneRoute && <CookieConsentBanner />}
      </LimitationsProvider>
    </AppLoaderProvider>
  );
}
